...
- **Phase A exit gates** (7): #1 composite ≥71% ✅ · #2 heatmap floor ≥70% ❌ OPEN (see below — the floor is NOT a competence measure) · #3 0 fly ERRORs/7d ❌ RE-OPENED 2026-07-15 by R-F2622 (was a FALSE pass) · #4 quarantined DDs closed ✅ · #5 env vars set ⏳ (R-F745 closed upstash-default drift; R-F794 2026-05-22 set HARVEST_ENABLED=1 + AUTONOMOUS_ENABLED=1 + AUTONOMY_LEVEL=1 shadow; ACLED DEFERRED by operator until MVP launch per §18 — gate #5 CANNOT close by code) · #6 500-Q eval frozen ✅ · #7 ≥4 design-partner convos ⏳ (operator-owned, uncodeable).
