...
- **CURRENT BASELINE — 111 failed / 12,556 passed / 7 skipped / 2 xfailed / 28:36, SINGLE PROCESS, network guard ON by default. Measured 2026-07-30 at `a0ee0b99` (R-F3448). Full set + all 111 names: `docs/suite_baseline_2026_07_30.md`.** Command: `python -m pytest aria_service/tests/ -q --tb=no --durations=15 -p no:cacheprovider`, no env overrides. **This one is single-process, so unlike every earlier baseline it is NOT a floor** — it sees order-dependent failures.
  ...
  - **Expected next full-suite figure: 96** (99 − the 3 collateral failures the global fixtures exposed, all fixed). **NOT YET MEASURED** — three consecutive background full runs were reaped at 2%/10%/2%, and a 256-file foreground segment exceeds the 10-minute window. Do not quote 96 as measured.
