...
- **Open (deploy-sync — R-F1315, binding while the CI token is down / ARIA's CLI predates R-F1315)**: ARIA commits + pushes fine but her *running* `ci_deploy` cannot reach Fly (the GitHub `FLY_API_TOKEN` is stale, so the `[deploy]` CI path is dead; and a long-running ARIA process holds the pre-R-F1315 `ci_deploy` in memory). The symptom ARIA reports is "server online did not match" — that is the dead CI path, not bad code. So at session start, after the bridge poll, run `git fetch origin` and compare `git rev-parse --short origin/main` to the live build_rev (`curl https://aria-intel.fly.dev/health/live`). If the server is BEHIND origin: (1) compile-check every changed file (`py_compile` for .py, `node --check` for .mjs) — NEVER deploy a commit that does not compile (this is how the R-F1316 SyntaxError was caught before it shipped); (2) deploy the touched apps via `scripts/deploy.ps1` (`-Intel`/`-Wa`/`-Web` per the diff — flyctl is operator-authed locally, canary+rollback+build_rev verify); (3) confirm live build_rev advanced. This keeps the server in sync with ARIA's commits without the operator. PERMANENT fixes that retire this step: rotate `FLY_API_TOKEN` (GitHub secret) so CI deploys again, OR restart ARIA's `python -m aria_cli` so she loads the R-F1315 local-flyctl `ci_deploy`.
