...
- **CURRENT BASELINE — 111 failed / 12,556 passed / 7 skipped / 2 xfailed / 28:36, SINGLE PROCESS, network guard ON by default. Measured 2026-07-30 at `a0ee0b99` (R-F3448). Full set + all 111 names: `docs/suite_baseline_2026_07_30.md`.** Command: `python -m pytest aria_service/tests/ -q --tb=no --durations=15 -p no:cacheprovider`, no env overrides. **This one is single-process, so unlike every earlier baseline it is NOT a floor** — it sees order-dependent failures.
  - **The 15 order-dependent failures are now CLOSED (R-F3449).** A second complete single-process run measured **99 failed / 12,556→12,568 passed / 35:46**, and the failure-set diff shows exactly those 15 gone. Five root mechanisms, all leaked process-global state, all fixed — three of them in `conftest.py` rather than in the victim, because the victim was never the culprit: (a) a bare `module.func = fake` with no restore, 7 failures from just TWO tests; (b) `_auth_is_internal_var` set in the AMBIENT context — **`monkeypatch` restores the env var and cannot see a ContextVar**; (c) an inherited OPEN circuit breaker, since `web_search.py` returns `[]` BEFORE any HTTP call so nothing can wire; (d) the singleton `main.app.state.llm_provider`; (e) a **stale parent-package attribute defeating `sys.modules.pop`** — `from . import X` resolves from the package attribute, so `test_rf772` was exercising NOTHING in-suite: a green guard against an 81.6s wedge that verified nothing. Full detail: `docs/suite_baseline_2026_07_30.md`.
  - **`test_rf3449_no_unrestored_module_swaps.py` stops the class recurring** — it fails on any NEW unrestored stub assignment, with 10 remaining known-latent offenders allowlisted BY NAME in a list that may only SHRINK. Narrow it or it is useless: the un-narrowed scan reports **416 hits**, nearly all benign cache resets. And the scanner is itself tested — the first version was lowercase-only and constructor-blind, so it MISSED the `_FakeRS` swap behind four failures.
  - **Expected next full-suite figure: 96** (99 − the 3 collateral failures the global fixtures exposed, all fixed). **STILL NOT MEASURED after NINE attempts (2026-07-30); furthest reached 25%.** Do not quote 96, or any number, as measured. Five distinct causes, only ONE a product defect: (1) background reaping — run detached; (2) **R-F3459 timeout inversion — FIXED**, an inner subprocess bound above the 120s per-test cap killed the PROCESS with no summary; (3) `STATUS_CONTROL_C_EXIT 0xC000013A` — an `/IT` scheduled task shares the interactive console, so a concurrent agent's Ctrl+C kills it; (4) per-test timeout under CPU contention — use `--timeout=600` for measurement runs only, pytest.ini stays 120s for CI; (5) **ORPHANED RUNS, the dominant cause and self-inflicted** — the console isolation that fixes (3) also makes the child immune to `schtasks /end`, so "stopped" runs keep going: one survived 45 min across four attempts, starving them and writing the same log. **Kill by PID and verify none remain before relaunching.** Full detail + the four monitoring errors (a healthy run reported dead three times) in `docs/suite_baseline_2026_07_30.md`. **Completion is pytest's summary line; death is an exit code — nothing else is evidence.**
  - **Watch for this when adding a global fixture:** it WILL have collateral, and the collateral is usually a real finding. Two of the three above were tests calling `get_breaker(name, failure_threshold=1)` — which **returns the existing breaker and silently DISCARDS the threshold** — so they had only ever passed by inheriting other tests' failure counts. That silent discard is a production footgun too (first registration wins, no warning) and is deliberately NOT yet changed.
  - **ZERO failures are attributable to the R-F3446 network-guard default.** Every failure was diffed BY NAME against the 2026-07-28 set (94 named) ∪ the eight guard-ON segment sweeps (104 names) = 113 known; the 15 outside it were all order-dependent. The four tests that WERE guard-attributable are fixed (R-F3440, R-F3444) and now pass.
  - **Slowest test 46.28s = 39% of the 120s per-test timeout.** Track this: exceeding `timeout = 120` does not fail one test, it KILLS THE PROCESS with no summary (Windows thread method), so headroom is a safety property. Before R-F3443 the worst was 100.63s (84%).
  - Predecessor, kept for the audit trail: **94 failed / 11,673 passed / 11,767 tests, 2026-07-28 at `31782564` (R-F3368)**, `docs/suite_baseline_2026_07_28.md` — built from 13 FOREGROUND segments (background pytest is killed on this machine; four attempts died at 3%, 19%, 11%, 0 bytes with no summary — R-F3365/R-F3368) and therefore a **FLOOR**, blind to order-dependent failures. The line below it said *3647 tests / 72 failing* and had been ~3x understated for two months. **Diff the failure SET, never the count alone** — the count legitimately FELL 165→111 across 2026-07-29 because R-F3419/R-F3430/R-F3431/R-F3440/R-F3444/R-F3447 each removed real reds while 1,028 tests were added. Historical: **3647 tests / ~166s / 72 failing (2026-05-20, R-F747)** — was claimed as "3333/125s/27 known-fails" on 2026-05-18 but the 2026-05-20 audit re-ran `pytest -q` and found 314 more tests + 45 more failing tests than CLAUDE.md acknowledged. The 11 failing R-clusters: R-F434 (brandified hostname cap), R-F436 (page entity extraction), R-F445 (polyglot execute), R-F450 (upload magic-byte routing), R-F460 (brain absorb pause), R-F463 (memory replication patterns), R-F468 (mistake ledger no TTL), R-F513 (build_rev autoderive), R-F528 (read_document clientdisconnect), R-F574 (self-improve discard), R-F672 (lifespan silent except promoted). Refresh-baseline rule: re-run pytest after every 100 R-numbers shipped (next refresh ~R-F850) or any time a session lands ≥5 commits to aria_service/. New R-numbers must not add to the failing-test count.
...
- **`ARIA_API_TOKEN` == `ARIA_INTERNAL_TOKEN`** (both digest `15fb531c831949cc`, verified live 2026-07-29). `routes/aria.py:278-281` documents the intent — "distinct secret so internal services don't share the user-facing API key" — and it is **false in production**. Consequence, and it is the opposite of the obvious one: `_auth_is_internal_var` can never be True, so **DD report ownership scoping is ALWAYS enforced** and the duplication currently fails **SAFE**. **Rotating them apart would ACTIVATE an unrestricted cross-tenant path that has never run in production** (`routes/aria.py:1261,1406,1512`). Do not treat this as routine hygiene: pair any rotation with a test of the DD access paths under an internal token.
