...
if not errorlevel 1 (
    if exist "aria.py" (
        ...
        powershell -NoProfile -Command "try{$w=New-Object Net.WebClient;$w.DownloadFile('%SERVER%/download/aria.py', 'aria.py');Write-Host '  Downloaded aria.py' -ForegroundColor Green}catch{Write-Host '  Download failed: '+$_.Exception.Message -ForegroundColor Red;exit 1}"
        if exist "aria.py" (
            python aria.py --setup
            if not errorlevel 1 (
                python aria.py %*
                if not errorlevel 1 exit /b 0
            )
...
if not errorlevel 1 (
    if not exist "aria.py" (
        echo   Downloading ARIA Python client for better experience...
        powershell -NoProfile -Command "try{$w=New-Object Net.WebClient;$w.DownloadFile('%SERVER%/download/aria.py', 'aria.py');Write-Host '  Downloaded' -ForegroundColor Green}catch{Write-Host '  Could not download' -ForegroundColor Yellow}"
    )
    if exist "aria.py" (
        echo   Python detected. For a better experience, type: python aria.py
        echo.
    )
)
...
if /i "%input%"=="help" (
    ...
    echo    setup             Run setup wizard (get API token)
...
if /i "%input%"=="setup" (
    ...
    echo.
    echo   Or save it to config by running the Python setup wizard.
    echo.
    goto loop
)
...
if "%ARIA_API_TOKEN%"=="" (
    ...
    echo   Or run: python aria.py --setup
    echo.
    goto loop
)
