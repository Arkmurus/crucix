...
async def fetch_and_index(force: bool = False) -> dict[str, Any]:
    ...
    try:
        async with httpx.AsyncClient(timeout=120.0, follow_redirects=True) as client:  # no-breaker: crypto sanctions is best-effort; breaker would miss wallet screenings
