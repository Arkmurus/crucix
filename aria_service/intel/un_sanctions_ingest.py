...
async def fetch_and_ingest(*, max_rows: int = MAX_ROWS_DEFAULT) -> dict:
    ...
    for url in [UN_SC_URL, UN_PORTAL_URL]:
        try:
            async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:  # no-breaker: UN sanctions ingest is periodic batch; breaker would stall sanctions updates
