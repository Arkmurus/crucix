"""R-F1920/R-F1921 — deploy-reconciler: alert the operator when origin/main sits
ahead of the live build_rev past a threshold, but ONLY when the undeployed diff
actually touches aria-intel runtime (not tooling/docs/registry/web/test-only).
...
authoritative origin truth and ALERTS the operator. R-F1921 adds the
runtime-relevance filter so it doesn't nag on every chore/registry commit.

Capability tests drive the REAL reconcile functions with injected fetchers (no
network) and assert the operator-alert fires exactly when it should, once, and
only for runtime-relevant diffs.
"""
...
def _cmp(files):
    async def f(repo, base, head, token):
        return files
    return f
...
RUNTIME_DIFF = ["aria_service/main.py"]              # warrants a redeploy
TOOLING_DIFF = ["data/r_number_reservations.json", "MEMORY.md"]  # does not
TEST_ONLY_DIFF = ["aria_service/tests/test_x.py"]    # ships but no runtime change
...
def test_origin_live_clears_state():
    r = asyncio.run(dv.reconcile_origin_vs_live(
        ...
        compare_fetcher=_cmp(RUNTIME_DIFF), behind_alert_s=GRACE, now=10_000.0))
...
def test_behind_past_grace_runtime_diff_alerts_once():
    ...
    r1 = asyncio.run(dv.reconcile_origin_vs_live(
        ...
        compare_fetcher=_cmp(RUNTIME_DIFF), behind_alert_s=GRACE, now=10_700.0))
    assert r1["behind"] is True
    assert r1["runtime_relevant"] is True
    ...
    r2 = asyncio.run(dv.reconcile_origin_vs_live(
        ...
        compare_fetcher=_cmp(RUNTIME_DIFF), behind_alert_s=GRACE, now=11_000.0))
    assert r2["behind"] is True
    assert r2["alert"] is None  # already alerted for this head


def test_behind_past_grace_tooling_only_suppressed():
    """R-F1921: live behind origin but the diff is tooling/registry-only → no
    alert (no aria-intel redeploy needed). Evaluated once (alerted=True)."""
    st = {"origin_sha": "abcdef12", "first_seen_at": 10_000.0, "alerted": False}
    r = asyncio.run(dv.reconcile_origin_vs_live(
        state=st, gh_fetcher=_gh(ORIGIN), live_fetcher=_live(LIVE_BEHIND),
        compare_fetcher=_cmp(TOOLING_DIFF), behind_alert_s=GRACE, now=10_700.0))
    assert r["behind"] is True
    assert r["runtime_relevant"] is False
    assert r["alert"] is None              # suppressed
    assert r["state"]["alerted"] is True   # handled once, won't recheck this head


def test_test_only_diff_suppressed():
    """A tests-only aria_service/ change ships in the image but doesn't change
    runtime → suppressed."""
    st = {"origin_sha": "abcdef12", "first_seen_at": 10_000.0, "alerted": False}
    r = asyncio.run(dv.reconcile_origin_vs_live(
        state=st, gh_fetcher=_gh(ORIGIN), live_fetcher=_live(LIVE_BEHIND),
        compare_fetcher=_cmp(TEST_ONLY_DIFF), behind_alert_s=GRACE, now=10_700.0))
    assert r["runtime_relevant"] is False
    assert r["alert"] is None


def test_compare_unavailable_fails_safe_to_alert():
    """If the diff can't be determined (compare returns None), alert anyway —
    better a possibly-spurious alert than missing a real undeployed change."""
    st = {"origin_sha": "abcdef12", "first_seen_at": 10_000.0, "alerted": False}
    r = asyncio.run(dv.reconcile_origin_vs_live(
        state=st, gh_fetcher=_gh(ORIGIN), live_fetcher=_live(LIVE_BEHIND),
        compare_fetcher=_cmp(None), behind_alert_s=GRACE, now=10_700.0))
    assert r["runtime_relevant"] is True
...
def test_runtime_relevance_helper():
    assert dv._is_runtime_relevant(["aria_service/intel/x.py"]) is True
    assert dv._is_runtime_relevant(["aria_service/tests/test_x.py"]) is False
    assert dv._is_runtime_relevant(["data/r_number_reservations.json"]) is False
    assert dv._is_runtime_relevant(["server.mjs", "public/x.html"]) is False
    assert dv._is_runtime_relevant(["aria_service\\main.py"]) is True  # win path sep
    assert dv._is_runtime_relevant(None) is True  # fail-safe
    assert dv._is_runtime_relevant(["docs/x.md", "aria_service/main.py"]) is True


def test_via_store_persists_and_notifies():
    """The proprioception-loop glue: reads/writes state through a store and
    fires the operator_notifier when past grace AND runtime-relevant."""
    ...
    res = asyncio.run(dv.reconcile_origin_via_store(
        ...
        compare_fetcher=_cmp(RUNTIME_DIFF), behind_alert_s=GRACE,
        operator_notifier=_notify))
