# -*- coding: utf-8 -*-
"""R-F4313 — capability tests for the CLI sub-agent (spawn_subagent).

These live in aria_service/tests/ so the pre-commit capability-test guard can
find them (it scans aria_service/tests/). They drive the REAL
CoderToolbox.spawn_subagent and the REAL Agent wiring to prove the
user-visible behaviour:

1. spawn_subagent fails CLOSED when no factory is registered.
...
4. Agent.__init__ registers the sub-agent factory on its CoderToolbox.
"""
import os
import sys
...
def test_rf4313_agent_registers_subagent_factory():
    ...
    # R-F4370 (C-315) — explicit config: LLMClient() resolves from_env(),
    # which now fails closed rather than picking whichever vendor has a
    # key in the ambient environment. This test is about sub-agent wiring.
