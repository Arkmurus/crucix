...
# Read expected SHA from a file written by deploy.sh
_SHA_FILE = REPO_ROOT / ".last_deploy_sha"
_EXPECTED_SHA = ""
if _SHA_FILE.exists():
    _EXPECTED_SHA = _SHA_FILE.read_text(encoding="utf-8").strip()
...
APPS = {
    "intel": {
        ...
        "health_check": lambda d: (
            d.get("status") == "alive"
            and _EXPECTED_SHA
            and _EXPECTED_SHA in d.get("build_rev", "")
        ),
    },
    "web": {
        "url": "https://aria-web.fly.dev",
        "health_endpoint": "/healthz",
        "health_check": lambda d: d == "ok" if isinstance(d, str) else d.get("status") == "ok",
    },
    "wa": {
        "url": "https://aria-wa.fly.dev",
        "health_endpoint": "/health",
        "health_check": lambda d: d.get("status") == "connected",
    },
}
...
def check_app_health(client: httpx.Client, app_name: str, config: dict) -> bool:
...
def main() -> int:
    ...
    print(f"  expected build: {_EXPECTED_SHA}")
    ...
    with httpx.Client() as client:
        for app_name in apps_to_check:
            ...
            if not check_app_health(client, app_name, config):
                failures += 1

    print()
    if failures == 0:
        print(f"=== ALL CHECKS PASSED (commit {_EXPECTED_SHA}) ===")
        return 0
    else:
