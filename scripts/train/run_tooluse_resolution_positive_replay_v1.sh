#!/usr/bin/env bash
# R-F4078 — full-parent positive replay with balanced resolution reinforcement.
set -euo pipefail
...
REPO="$ROOT" ARIA_POD_CREATE_API=graphql ARIA_MAX_GPU_HOURLY_USD=1.60 \
  ...
  TRAINING_RECIPE_KIND=tooluse_positive_sft_diagnostic_continuation \
  ...
  REPORT_LOCAL=data/eval_reports/aria_tooluse_resolution_positive_replay_v1_eval.json \
  OUTPUT_LOCAL=data/training/checkpoints/aria_tooluse_resolution_positive_replay_v1_failed_candidate.tgz \
  INTERMEDIATE_LOCAL=data/training/checkpoints/aria_tooluse_resolution_positive_replay_v1_failed_candidate.tgz \
  INTERMEDIATE_REMOTE=/workspace/eval/aria_tooluse_dpo_adapter.tgz \
  DIAGNOSTICS_LOCAL=data/eval_reports/aria_tooluse_resolution_positive_replay_v1_diagnostics.tgz \
  STATE_FILE=data/eval_reports/.tooluse_resolution_positive_replay_v1_pod_state \
  REMOTE_DPO_OUT=/workspace/checkpoints/aria_tooluse_resolution_positive_replay_v1 \
