#!/usr/bin/env bash
# R-F4054 — evaluate the recovered R-F4049 child without retraining.
set -euo pipefail
...
TRAIN_PROOF=data/training/aria_tooluse_protected_positive_v1.jsonl
...
REPO="$ROOT" ARIA_POD_CREATE_API=graphql ARIA_MAX_GPU_HOURLY_USD=1.60 \
  ...
  TRAINING_RECIPE_KIND=tooluse_adapter_evaluation_recovery \
  PROTECTED_DPO_AXES=tooluse_adverse,tooluse_contradiction,tooluse_resolution,tooluse_news_impact \
  POD_RUNNER=scripts/train/pod_tooluse_calibration_recovery.sh \
  ...
  DIAGNOSTICS_LOCAL=data/eval_reports/aria_tooluse_protected_positive_v1_recovery_diagnostics.tgz \
  STATE_FILE=data/eval_reports/.tooluse_protected_positive_v1_recovery_pod_state \
