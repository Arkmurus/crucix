...
async def _run_defence_dd_eval(
    ...
    """Run a defence-DD eval set. Each record:
        { "question": str, "expected_keywords": [str, ...], "topic": str }
    Pass criterion: response contains at least 60% of expected_keywords
    (substring-match, case-insensitive). This is coarse; a real eval
    would use LLM-as-judge or human grading.
    """
    ...
    for q in questions:
        ...
        expected = [k.lower() for k in q.get("expected_keywords", [])]
