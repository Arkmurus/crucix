#!/usr/bin/env bash
# R-F4021 — measure the failed Phoenix adapter on novel live contradictions.
set -euo pipefail
...
QUEUE=data/training/aria_tooluse_contradiction_novel_v2.jsonl
...
test "$(hash "$QUEUE")" = e72c2202662d6fb3f8f25a9740a3a165a10f4ceb313217086e513b58000282f2
...
QUEUE="$QUEUE" EVAL_LOCAL="$EVAL" GOLDEN="$GOLDEN" \
  ...
  REPORT_LOCAL=data/eval_reports/aria_tooluse_novel_contradiction_generations.json \
  STATE_FILE=data/eval_reports/.tooluse_novel_contradiction_generation_pod_state \
