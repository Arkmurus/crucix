#!/bin/bash
# R-F1646 — v0.6 GROUNDED SFT cycle DRIVER (Stage-1 Lane-A, bigger+sharper corpus).
#
...
# Trains on the bigger+sharper v0.6 corpus (aria_grounded_v2.jsonl) and evals
# OPEN-BOOK. G1: defence-DD >= DeepSeek parity AND PI leak under the C2 set.
#
# Usage:  RUNPOD_POD_ID=<pod> bash scripts/train/run_v0_6_grounded_cycle.sh
set -uo pipefail
...
TRAIN_CORPUS="${TRAIN_CORPUS:-data/training/aria_grounded_v2.jsonl}"
EVAL_LOCAL="${EVAL_LOCAL:-data/eval_reports/aria_eval_500q_openbook.jsonl}"   # OPEN-BOOK
[ -s "$TRAIN_CORPUS" ] || { echo "[driver] FATAL: v0.6 corpus missing/empty: $TRAIN_CORPUS"; exit 1; }
[ -s "$EVAL_LOCAL" ]   || { echo "[driver] FATAL: open-book eval set missing: $EVAL_LOCAL"; exit 1; }
echo "[driver] v0.6 corpus $(wc -l < "$TRAIN_CORPUS") rows; open-book eval $(wc -l < "$EVAL_LOCAL") Q"
...
TSSH(){ timeout 75 $SSH "$@"; }   # R-F1646: hard cap so a hung SSH never freezes the loop
...
# 3. Push driver + current scripts + v0.6 corpus + open-book eval + self-stop watcher
echo "[driver] pushing driver + scripts + v0.6 corpus + open-book eval + self-stop watcher..."
...
scp -i "$KEY" -o StrictHostKeyChecking=no -P "$PORT" "$TRAIN_CORPUS" root@"$HOST":/workspace/datasets/aria_grounded_v2.jsonl  || { echo "[driver] FATAL scp corpus"; exit 1; }
...
# 4. Launch the cycle DETACHED, then AUTO-ARM the self-stop watcher (R-F1646 hardening)
echo "[driver] launching v0.6 GROUNDED cycle DETACHED on the pod..."
TSSH -p "$PORT" root@"$HOST" \
  "rm -f /workspace/eval/_cycle_status; mkdir -p /workspace/logs; \
   TRAIN_FILE=/workspace/datasets/aria_grounded_v2.jsonl \
   ...
   setsid nohup bash /workspace/v0_4_pod_run.sh > /workspace/logs/v0_6_cycle.log 2>&1 < /dev/null & echo STARTED" \
...
for i in $(seq 1 200); do
  ...
  OUT=$(TSSH -p "$PORT" root@"$HOST" \
    'printf "RC=%s\n" "$(cat /workspace/eval/_cycle_status 2>/dev/null)"; tail -1 /workspace/logs/v0_6_cycle.log 2>/dev/null' \
    2>/dev/null | tr -d '\r')
...
scp -i "$KEY" -o StrictHostKeyChecking=no -P "$PORT" root@"$HOST":/workspace/eval/aria_llm_v0_4_eval.json data/eval_reports/aria_llm_v0_6_grounded_eval.json 2>/dev/null || echo "[driver] (v0.6 report not pulled)"
...
# 6. G1 verdict
echo "[driver] === v0.6 GROUNDED CYCLE RESULT — GATE G1 (local) ==="
python - <<'PY'
import json, os
def line(tag, f):
...
a6=line("v0.6 GROUNDED (open-book)", "data/eval_reports/aria_llm_v0_6_grounded_eval.json")
ds=line("DeepSeek (open-book)", "data/eval_reports/deepseek_baseline_openbook_v06.json")
print("v0.5 prior: 0.30")
if a6 is not None:
    tgt = ds or 0.336
    print(f"G1 accuracy: {'PASS' if a6>=tgt else 'FAIL'} (v0.6 {a6} vs {tgt} parity)")
