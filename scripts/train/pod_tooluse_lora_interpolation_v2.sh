#!/usr/bin/env bash
# R-F4167 — evaluate pre-registered low weights of the protected-DPO direction.
set -uo pipefail
...
mkdir -p "$EVALD" "$LOGS" /workspace/checkpoints/interpolations_v2
rm -f "$EVALD/_cycle_status"

log(){ echo "[$(date -u +%H:%M:%S)] [lora-interpolation-v2] $*"; }
...
[ "$ALPHAS" = "0.125 0.25 0.5" ] || fail "alpha set differs from registration"
...
for alpha in $ALPHAS; do
  ...
  adapter=/workspace/checkpoints/interpolations_v2/alpha_$tag
  ...
  model="aria-tooluse-interpolation-v2-$tag"
  ADAPTER="$adapter" MODEL_NAME="$model" PORT=$PORT BASE_MODEL="$BASE_MODEL" \
    python "$SCRIPTS/serve_eval_shim.py" >"$LOGS/interpolation_v2_${tag}_shim.log" 2>&1 &
  SHIM_PID=$!
  ...
  report="$EVALD/aria_tooluse_lora_interpolation_v2_alpha_${tag}.json"
  python -m scripts.train.eval_tooluse --target "http://localhost:$PORT/v1" \
    --model "$model" --eval-file "$EVAL_FILE" --out "$report" \
    2>&1 | tee "$LOGS/interpolation_v2_${tag}_eval.log" \
