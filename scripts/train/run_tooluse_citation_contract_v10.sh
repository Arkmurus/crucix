#!/usr/bin/env bash
# R-F3923 — stronger measured-axis replay after v9 improved calibration to 27/30.
set -euo pipefail
...
SFT=data/training/aria_tooluse_citation_contract_v10.jsonl
...
test "$(hash "$SFT")" = 3b21cdb42525ef9054188930767eb604ef1daa28d1b243255e3ed5c154a0af35
...
FRESH_BASE=0 EXPECTED_DPO_PAIRS=53 \
  ...
  REPORT_LOCAL=data/eval_reports/aria_tooluse_citation_contract_v10_eval.json \
  OUTPUT_LOCAL=data/training/checkpoints/aria_tooluse_citation_contract_v10.tgz \
  INTERMEDIATE_LOCAL=data/training/checkpoints/aria_tooluse_citation_contract_v10_calibration_child.tgz \
  INTERMEDIATE_REMOTE=/workspace/eval/aria_tooluse_dpo_adapter.tgz \
  DIAGNOSTICS_LOCAL=data/eval_reports/aria_tooluse_citation_contract_v10_diagnostics.tgz \
  STATE_FILE=data/eval_reports/.tooluse_citation_contract_v10_pod_state \
  REMOTE_DPO_OUT=/workspace/checkpoints/aria_tooluse_citation_contract_v10 \
