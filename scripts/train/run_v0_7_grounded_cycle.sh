#!/bin/bash
# R-F1654 — v0.7 GROUNDED SFT cycle DRIVER (Stage-1 Lane-A, bigger+sharper corpus).
#
...
# Trains on the bigger+sharper v0.7 corpus (aria_grounded_v3.jsonl) and evals
# OPEN-BOOK. G1: defence-DD >= DeepSeek parity AND PI leak under the C2 set.
#
# Usage:  RUNPOD_POD_ID=<pod> bash scripts/train/run_v0_7_grounded_cycle.sh
set -uo pipefail
...
[ -s "$TRAIN_CORPUS" ] || { echo "[driver] FATAL: v0.7 corpus missing/empty: $TRAIN_CORPUS"; exit 1; }
[ -s "$EVAL_LOCAL" ]   || { echo "[driver] FATAL: open-book eval set missing: $EVAL_LOCAL"; exit 1; }
echo "[driver] v0.7 corpus $(wc -l < "$TRAIN_CORPUS") rows; open-book eval $(wc -l < "$EVAL_LOCAL") Q"
...
TSSH(){ timeout 75 $SSH "$@"; }   # R-F1654: hard cap so a hung SSH never freezes the loop
...
# 3. Push driver + current scripts + v0.7 corpus + open-book eval + self-stop watcher
echo "[driver] pushing driver + scripts + v0.7 corpus + open-book eval + self-stop watcher..."
...
# 4. Launch the cycle DETACHED, then AUTO-ARM the self-stop watcher (R-F1654 hardening)
echo "[driver] launching v0.7 GROUNDED cycle DETACHED on the pod..."
TSSH -p "$PORT" root@"$HOST" \
  "rm -f /workspace/eval/_cycle_status; mkdir -p /workspace/logs; \
   ...
   setsid nohup bash /workspace/v0_4_pod_run.sh > /workspace/logs/v0_7_cycle.log 2>&1 < /dev/null & echo STARTED" \
...
for i in $(seq 1 200); do
  ...
  OUT=$(TSSH -p "$PORT" root@"$HOST" \
    'printf "RC=%s\n" "$(cat /workspace/eval/_cycle_status 2>/dev/null)"; tail -1 /workspace/logs/v0_7_cycle.log 2>/dev/null' \
    2>/dev/null | tr -d '\r')
...
scp -i "$KEY" -o StrictHostKeyChecking=no -P "$PORT" root@"$HOST":/workspace/eval/aria_llm_v0_4_eval.json data/eval_reports/aria_llm_v0_7_grounded_eval.json 2>/dev/null || echo "[driver] (v0.7 report not pulled)"
...
# 6. G1 verdict
echo "[driver] === v0.7 GROUNDED CYCLE RESULT — GATE G1 (local) ==="
python - <<'PY'
import json, os
def line(tag, f):
...
a6=line("v0.7 GROUNDED (open-book)", "data/eval_reports/aria_llm_v0_7_grounded_eval.json")
ds=line("DeepSeek (open-book)", "data/eval_reports/deepseek_baseline_openbook_v06.json")
print("v0.5 prior: 0.30")
if a6 is not None:
    tgt = ds or 0.336
    print(f"G1 accuracy: {'PASS' if a6>=tgt else 'FAIL'} (v0.7 {a6} vs {tgt} parity)")
