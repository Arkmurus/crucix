...
test('the web bucket is not attributed to a backend this module never calls', () => {
  const activeSourcesBlock = SRC.slice(
    SRC.indexOf('const activeSources = ['),
    SRC.indexOf('];', SRC.indexOf('const activeSources = [')),
  );
  assert.ok(activeSourcesBlock.length > 0, 'activeSources block not found');
  assert.ok(!/webVal\.length\s*>\s*0\s*\?\s*\['Brave Search'\]/.test(activeSourcesBlock),
    "the web bucket must not be credited to 'Brave Search' — searchWeb() only "
    + 'queries SearXNG');
  assert.match(activeSourcesBlock, /webVal\.length\s*>\s*0\s*\?\s*\['SearXNG/,
    'the web bucket must name the backend that actually answered');
});
...
test('the new source label carries a weight (no silent drop to default)', () => {
  const weights = SRC.slice(SRC.indexOf('const SOURCE_WEIGHTS = {'),
                            SRC.indexOf('};', SRC.indexOf('const SOURCE_WEIGHTS = {')));
  assert.match(weights, /'SearXNG \(web meta-search\)':\s*0\.70/,
    'an unweighted label silently scores at the 0.60 default, i.e. the fix would '
