...
| **R-F2780** | **Make adverse media a first-class verdict input** — credible (official/institution/quality-press), subject-named adverse findings now ESCALATE the stored verdict (GREEN→AMBER-LIGHT, escalate-only, never downgrade); the deep adverse search now also fires on GREEN company DDs (bounded, flag-gated) so clean-looking entities are actually screened. | Never-false-clean | ✅ **implemented + capability-tested this session** (6 tests; caught & fixed an int-vs-string tier bug that would have made escalation a silent no-op) |
