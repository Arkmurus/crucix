...
| Var | Server | Status | Notes |
|---|---|---|---|
| `ACLED_EMAIL` + `ACLED_PASSWORD` | both | **BLOCKED on ACLED (not on us)** — code READY (R-F2627), creds VALID, tier lacks API entitlement (R-F2628) | ACLED conflict events (Phase A gate #5). **BOTH required; `ACLED_API_KEY` is DEAD** — it authenticated `api.acleddata.com`, a host that **no longer resolves in DNS** (verified 2026-07-15). R-F2627 migrated `conflict_tracker` to the current OAuth password grant (`POST acleddata.com/oauth/token` → Bearer → `acleddata.com/api/acled/read`). **⚠️ REGISTERING IS NOT ENOUGH.** Verified live 2026-07-15 with a real account (`aria@imaria.io`, uid 207778): OAuth returns a valid JWT (`scope:['authenticated']`) and cookie login succeeds — yet **every** data read returns `403 {"message":"Access denied"}` (both auth methods, all URL variants, with and without a token). Cause: **free "Open myACLED" registration does NOT include API access.** Per ACLED's access model, API access requires the **Research / Partner / Enterprise** level, which is **assigned by organization on request** (access@acleddata.com). ARIA is a commercial entity, so this likely needs a **paid licence** → a §6/§17 operator decision, not a code task. **Do NOT set these secrets until ACLED grants API access:** the code would then treat a known entitlement state as a repeated `source_failure` gap (noise the coder cannot fix — §21e escalates licence/financial decisions). Until then GDELT serves — by design. |
