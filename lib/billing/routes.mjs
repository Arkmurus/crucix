...
export function createBillingRouter({ requireAuth, findUserById, updateUser, listUsers }) {
  ...
  router.post('/checkout', requireAuth, async (req, res) => {
    const config = billingConfigStatus();
    if (!config.configured) {
      return res.status(503).json({ error: 'billing not fully configured', missing: config.missing });
    }
    const userId = req.user?.userId;
    if (!userId) return res.status(401).json({ error: 'auth required' });
    const user = findUserById(userId);
    if (!user) return res.status(404).json({ error: 'user not found' });
    if (user.stripeSubscriptionId && subscriptionGrantsAccess(user.subscriptionStatus)) {
      return res.status(409).json({
        error: 'an active subscription already exists — manage plan changes in the billing portal',
    ...
    try {
      await validateRecurringPrice(priceId, TIERS[requestedTier]);
      ...
      if (err.code === 'STRIPE_PRICE_MISMATCH') {
        errorTracker.record('billing', 'stripe_price_mismatch', err, null, { userId, requestedTier });
        return res.status(503).json({ error: err.message, code: err.code });
      }
  ...
  router.post('/webhook', async (req, res) => {
    ...
    try {
      await handleBillingEvent(event, { findUserById, updateUser, listUsers });
...
export async function handleBillingEvent(event, {
  ...
  updateUser,
  listUsers,
  retrieveSubscription: loadSubscription = retrieveSubscription,
}) {
  ...
  switch (type) {
    case 'checkout.session.completed': {
      ...
      if (!userId) {
        throw new Error('checkout.session.completed without crucix_user_id metadata');
      }
      const user = findUserById(userId);
      if (!user) {
        throw new Error(`checkout.session.completed for unknown user ${userId}`);
      }
      const updates = {};
    ...
    case 'customer.subscription.updated': {
      if (!obj.id) throw new Error(`${type} missing subscription id`);
      // Stripe does not guarantee webhook order. Re-read the Subscription so a
      // delayed event cannot restore stale entitlement state.
      const current = await loadSubscription(obj.id);
      const userId = current.metadata?.crucix_user_id
        || obj.metadata?.crucix_user_id
        || _userIdForCustomer(current.customer || obj.customer, { listUsers });
      if (!userId) {
        throw new Error(`${type} could not resolve crucix_user_id`);
      }
      if (!findUserById(userId)) throw new Error(`${type} resolved unknown user ${userId}`);
      const price = current.items?.data?.[0]?.price;
      const priceId = price?.id;
      const grantsAccess = subscriptionGrantsAccess(current.status);
      const tier = tierForPriceId(priceId);
      if (grantsAccess && !tier) {
        throw new Error(`${type} has unconfigured Stripe price ${priceId || '(missing)'}`);
      }
      ...
      const periodEnd = current.current_period_end
        ...
        : null;
      updateUser(userId, {
        tier: grantsAccess ? tier : DEFAULT_TIER,
        stripeCustomerId: current.customer || obj.customer || null,
      ...
      console.log(`[Billing] ${type} → user ${userId} tier=${tier || DEFAULT_TIER} status=${current.status}`);
      return;
    }

    case 'customer.subscription.deleted': {
      if (!obj.id) throw new Error('customer.subscription.deleted missing subscription id');
      const current = await loadSubscription(obj.id);
      const userId = current.metadata?.crucix_user_id
        || obj.metadata?.crucix_user_id
        || _userIdForCustomer(current.customer || obj.customer, { listUsers });
      if (!userId) throw new Error('customer.subscription.deleted could not resolve crucix_user_id');
      if (!findUserById(userId)) {
        throw new Error(`customer.subscription.deleted resolved unknown user ${userId}`);
      }
      updateUser(userId, {
        tier: DEFAULT_TIER,
        stripeSubscriptionId: null,
        stripeProductId: null,
        subscriptionStatus: current.status || 'canceled',
