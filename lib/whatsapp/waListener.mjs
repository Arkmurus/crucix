...
async function startListener() {
  ...
  sock.ev.on('messages.reaction', async (reactions) => {
    ...
    for (const r of reactions) {
      try {
        // Outer key = the reaction message itself + reactor identity.
        // Inner key (under r.reaction) = the message being reacted to.
        // The fromMe flag is unreliable in self-paired setups, so identify
        // ARIA messages by checking the target ID against our local
        // sent-message ledger instead.
        const outerKey = r.key || {};
        const innerKey = r.reaction?.key || {};
        const targetMsgId = innerKey.id || '';
        const targetChatId = innerKey.remoteJid || outerKey.remoteJid || '';
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // identified by checking the target ID against our local sent-message
      // ledger (fromMe is unreliable in self-paired setups). Empty emoji =
      // reaction removed. This block must run BEFORE the text-extraction +
      // empty-text skip because reaction messages carry no normal text.
      const reactionMsg = msg.message?.reactionMessage;
      if (reactionMsg && reactionMsg.key && reactionMsg.key.id && _sentMsgIds.has(reactionMsg.key.id)) {
