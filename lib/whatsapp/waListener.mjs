...
async function handleCommand(cmd, args, senderName, senderJid) {
  ...
  switch (cmd.toLowerCase()) {
    ...
    case 'ask': {
      ...
      const answer = await askARIA(a, '', senderName);
      return answer || 'ARIA is temporarily unavailable. Please try again shortly.';
    }
    case 'groupsummary': {
      const recent = messageStore.slice(-50).map(m => `[${m.senderName}]: ${m.text.slice(0, 200)}`).join('\n');
      if (!recent) return 'No messages stored yet.';
      const summary = await askARIA(`Summarise this group conversation. Highlight key topics, decisions, action items, and any compliance or risk mentions:\n\n${recent}`, '', senderName);
      return summary || 'ARIA is temporarily unavailable. Please try again shortly.';
    }
    case 'investigate': {
      if (!a) return 'Usage: /investigate [person or company name]';
      // Ask ARIA to investigate via chat with investigation-specific prompt
      ...
      const investigateResult = await askARIA(investigatePrompt, '', senderName);
      return investigateResult || 'ARIA investigation is temporarily unavailable. Please try again shortly.';
    }
    case 'network': {
      ...
      const networkResult = await askARIA(networkPrompt, '', senderName);
      return networkResult || 'ARIA network analysis is temporarily unavailable. Please try again shortly.';
    }
    case 'help':
      return [
  ...
  try {
    ...
    if (m.imageMessage) {
      ...
      try {
        ...
        if (!extracted) {
          // Distinguish "image is blank" from "no OCR backend installed"
          let visionConfigured = false;
          try {
            const vs = await fetch(`${ariaUrl}/api/aria/vision-status`, {
              signal: AbortSignal.timeout(5000),
            });
            if (vs.ok) {
          ...
          if (!visionConfigured) {
            await sendMessage(chatId, [
              `*ARIA* — 🖼 *Image received but local OCR is not installed on the ARIA service host.*`,
              ``,
              `I do image reading on my own — no cloud LLM needed. Install one of these on the ARIA service:`,
              ``,
              `*RECOMMENDED — EasyOCR (zero config, pure Python):*`,
              `\`pip install easyocr Pillow\``,
              `→ first image takes ~30s to download model weights, then it's instant and offline.`,
              ``,
            ...
            await sendMessage(chatId, `*ARIA* — 🖼 I looked at the image but couldn't extract any readable text. It may be blank, low-resolution, or contain only diagrams. Tell me what you're looking for and I'll try a different angle.`, msg).catch(() => {});
          }
          return null;
        }
        ...
        console.log(`[WA] OCR ${method}: ${charCount} chars from ${filename}`);
        ...
        await sendMessage(chatId, `*ARIA* — 🖼 *Image read* (${method}, ${charCount} chars):\n\n${preview}${more}${captionLine}${factsLine}`, msg).catch(() => {});
