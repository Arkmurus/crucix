...
`;
  ...
  if (isLong) {
    ...
    await sendMessage(chatId, `*ARIA* — 🔎 _On it. This involves research across multiple sources — give me up to 10 minutes. I'll come back with a structured brief and citations._`, null).catch(() => {});
...
<script>
  ...
  if (${status === 'connected' ? 'false' : 'true'}) {
    ...
    // we always show the fresh one.
    setInterval(function() {
      var img = document.getElementById('qr');
      if (img) img.src = '/api/wa-listener/qr.png' + TOK_QS + (TOK_QS ? '&' : '?') + 't=' + Date.now();
    }, 3000);
    // Reload page every 30s — picks up CAPTURED_AT updates + connection-success
