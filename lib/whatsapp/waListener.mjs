...
async function handleCommand(cmd, args, senderName, senderJid) {
  ...
  switch (cmd.toLowerCase()) {
    ...
    case 'forget': {
      ...
      //
      // Routes through the LOCAL server.mjs proxy (added 2026-04-09) instead
      // of calling fly.io directly. The listener process on seenode does not
      // always have ARIA_API_TOKEN set in its environment (past incident:
      // /forget returned 401 because the env var lived only on the
      // server.mjs process). Local proxy uses INT_TOKEN to authenticate to
      // server.mjs, which then injects the correct bearer for fly.io.
      const sid = `wa_group_${senderName.replace(/[^a-zA-Z0-9]/g, '').slice(-10)}`;
      try {
        const resp = await post('/api/aria/session/forget', { session_id: sid });
        if (resp.error) return `⚠️ /forget failed: ${resp.error}`;
        if (resp.existed) return `🧹 Conversation history wiped for ${senderName}. Next message starts fresh.`;
