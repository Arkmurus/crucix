  ...
  sock.ev.on('connection.update', ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === 'close') {
      ...
      if (logout) {
        ...
        // Session conflict — another device is using this number
        // Back off aggressively to avoid crash loop
        reconnectDelay = Math.min(reconnectDelay * 3, 300000); // max 5 min
        console.log(`[WA Listener] Session conflict (440) — another device active. Retry in ${reconnectDelay/1000}s`);
