...
async function askARIA(message, groupContext = '', sender = 'whatsapp') {
  ...
  // 2026-04-08 round 4: bumped the non-research default from 90s to 180s.
  // The 9-layer context build + LLM call + verification + honesty dispatch
  // routinely runs 60-100s on a non-trivial question even when no tool fires
  // (e.g. "who is the current defence minister of Ghana"), and 90s left no
  // headroom — the user got the timeout-diagnostic canned reply for what
  // should have been a normal answer. Research stays at 4 minutes.
  const isLong = _isLongResearchPrompt(message);
  const timeoutMs = isLong ? 240000 : 180000;  // 4min for research, 3min otherwise
