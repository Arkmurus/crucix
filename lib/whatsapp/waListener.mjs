...
function socialReplyFor(text) {
  ...
  if (/welcome/.test(t)) {
    return warm
      ? "Thank you — glad to be here. I'm ARIA, Arkmurus's AI assistant. Just tag me with @ARIA if you need anything."
      : concise
        ? "Thanks. I'm ARIA, Arkmurus's AI assistant. @ARIA to engage."
        : "Thank you. I'm ARIA, Arkmurus's AI assistant — tag me with @ARIA when you need research or a compliance check.";
  }
  if (/(nice|pleasure|great) to (meet|e-meet)/.test(t)) {
    return warm
      ? "Likewise — I'm ARIA, the Arkmurus AI assistant. I help the team with research and deal context. Happy to be in the loop."
      : "Pleased to meet you. I'm ARIA, Arkmurus's AI assistant. I help with research and record-keeping — tag me with @ARIA whenever useful.";
  }
  ...
  return warm
    ? "Hi! ARIA here — Arkmurus's AI assistant. Tag me with @ARIA when you need research, a DD check, or a compliance note."
    : "Hi — ARIA, Arkmurus's AI assistant. Tag me with @ARIA when you need anything.";
}
...
async function handleCommand(cmd, args, senderName, senderJid, notify = null) {
  ...
  switch (cmd.toLowerCase()) {
    ...
    case 'profile': {
      ...
      const syntheticMsg = `Aria, investigate ${a}`;
    ...
    case 'help':
      return [
        ...
        '',
        '*Tickets*',
        '/ticket [description] — File a dev ticket (GitHub + Airtable)',
...
//   - chat is in ARIA_MIRROR_GROUPS → silent ingest to /api/aria/ingest
            ...
            (async () => {
              try {
                const investigatePrompt =
                  ...
                  `4) Recommended Arkmurus engagement strategy and timing.`;
...
async function askARIA(message, groupContext = '', sender = 'whatsapp', progressCb = null) {
  const port = process.env.PORT || 3117;
  ...
  const timeoutMs = isLong ? 360000 : 240000;
  ...
  try {
    ...
    console.warn('[WA Listener] Streaming failed, falling back to /chat:', e.message);
    try {
...
async function handleAriaMention(msg, chatId, groupName, senderName, text) {
  ...
  if (_docMsg || _imgMsg) {
    ...
    if (recentDoc && recentDoc.text && recentDoc.text.length > 200
      ...
      // We have real extracted text — inject up to 12000 chars verbatim.
...
${recentDoc.text.slice(0, MAX_DOC_CHARS)}
...
`;
  ...
  if (isLong) {
    await sendMessage(chatId, `*ARIA* — 🔎 _On it. This involves research across multiple sources — give me up to 4 minutes. I'll come back with a structured brief and citations._`, null).catch(() => {});
  } else if ((text || '').trim().length > 25) {
    await sendMessage(chatId, `*ARIA* — 💭 _Thinking…_`, null).catch(() => {});
  }

  // 2026-04-12: Progressive status updates via SSE streaming. When the
  // backend emits a status event (e.g., "Tool: deep_research completed.
  // Generating response..."), we surface it as an interim WhatsApp message
  // so the user sees pipeline progress instead of 180s of silence.
  // Rate-limited: at most 1 status update per 15s to avoid message spam.
  let _lastStatusAt = 0;
  const progressCb = (statusMsg) => {
    const now = Date.now();
    if (now - _lastStatusAt < 15000) return; // 15s rate limit
    _lastStatusAt = now;
    sendMessage(chatId, `*ARIA* — 🔄 _${statusMsg}_`, null).catch(() => {});
  };

  const rawReply = await askARIA(enrichedText, recentMsgs, senderName, progressCb);
  // 2026-04-24 fix: never pass explicit null as fallbackMsg — JS default
  // params don't apply to explicit null, so _unwrapAriaReply was returning
  // null and the silent `if (!reply) return` below dropped the entire
  // reply on every askARIA failure for non-research prompts.
  const reply = isLong
    ? _unwrapAriaReply(rawReply, '⏱️ The deep research is still running in the background — ask me again in 60s and I should have the answer cached.')
    : _unwrapAriaReply(rawReply);
  if (!reply) return;
      ...
      try {
        ...
        if (mime.includes('pdf') || fileName.toLowerCase().endsWith('.pdf')) {
          ...
          if (pdfParse) {
            ...
            extractedText = (pdf.text || '').trim().slice(0, MAX_DOC_CHARS);
        ...
        // ── Word DOCX ────────────────────────────────────────────────────
          ...
          if (mammoth) {
            ...
            extractedText = (result.value || '').trim().slice(0, MAX_DOC_CHARS);
        ...
        // while surviving real-world supplier lists.
          ...
          if (XLSX) {
            try {
              ...
              console.log(`[WA] [${groupName}] ${senderName}: 📊 XLSX "${fileName}" (${extractedText.length} chars, ${wb.SheetNames.length} sheet(s))`);
            } catch (xe) {
              console.warn(`[WA] xlsx parse failed for "${fileName}":`, xe.message);
            }
          } else {
            console.warn(`[WA] xlsx module not available — ${fileName} will be stored as metadata only`);
          }
        }
        ...
        // ── Plain text files (txt, csv, md, json, xml) ───────────────────
          ...
          fileType = 'text';
          extractedText = buffer.toString('utf-8').trim().slice(0, MAX_DOC_CHARS);
          ...
          if (buffer && buffer.length > 0 && ariaUrl) {
            ...
            if (r && r.ok) {
              ...
              if (ocrText.length > 50) {
                extractedText = ocrText.slice(0, MAX_DOC_CHARS);
                console.log(`[WA] Backend OCR rescued "${fileName}": ${extractedText.length} chars`);
              } else {
...
export function mountWAListener(app) {
  ...
  app.post('/api/wa-listener/force-qr', _waRequireAuth, async (_req, res) => {
    ...
    if (upstashOk()) {
      try { await upstashCmd('DEL', AUTH_REDIS_KEY); redisCleared = true; }
    ...
    res.json({
      ...
      next_step: 'Watch seenode logs for "SCAN THIS QR CODE", then pair from WhatsApp Business → Linked Devices → Link a Device. The old linked-device entry on your phone can be safely removed.',
    });
  });
