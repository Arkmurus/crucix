...
`;
  ...
  if (isLong) {
    ...
    if (/https?:\/\//i.test(text)) {
      ...
      // R-F1768: progressive disclosure — quick answer first, deep-dive on request.
      // Changed from "give me up to 10 minutes" to "let me check quickly" so the
      // user gets a fast initial response instead of being told to wait.
      await sendMessage(chatId, `*ARIA* - Let me check what I know quickly — this should only take a few seconds.`, null).catch(() => {});
    }
  ...
  // R-F1760: self-healing conversation manager — tries multiple strategies
  // (fast LLM → full LLM → local-only → fallback) instead of a single
  // askARIA call that fails silently. Each strategy has its own timeout
  // and error handling. The conversation manager tracks state per chat
  // so it can handle clarifications and mid-stream interruptions.
  ...
  // If all strategies failed, build an honest fallback
  if (!reply) {
