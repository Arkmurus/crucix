  ...
  sock.ev.on('connection.update', ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === 'close') {
      ...
      if (logout) {
        ...
        console.log('[WA Listener] Session expired (440) — clearing auth for fresh QR scan...');
        try { fs.rmSync(AUTH_DIR, { recursive: true, force: true }); } catch {}
        // Also clear stale auth from Redis
        if (upstashOk()) { upstashCmd('DEL', AUTH_REDIS_KEY).catch(() => {}); }
