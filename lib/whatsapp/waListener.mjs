...
async function backupAuthToRedis() {
  ...
  try {
    ...
    for (const f of files) {
      authBundle[f] = fs.readFileSync(path.join(AUTH_DIR, f), 'utf8');
    }
    await redisSet(AUTH_REDIS_KEY, authBundle);
    console.log(`[WA Listener] Auth backed up to Redis (${files.length} files)`);
  } catch(e) {
...
async function restoreAuthFromRedis() {
  ...
  try {
    ...
    if (!authBundle || typeof authBundle !== 'object') return false;
    ...
    console.log(`[WA Listener] Auth restored from Redis (${files.length} files) — no QR scan needed`);
    return true;
  } catch(e) {
