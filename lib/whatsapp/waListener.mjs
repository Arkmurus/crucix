  ...
  switch (cmd.toLowerCase()) {
    ...
    case 'teach': {
      ...
      if (urlMatch) {
        ...
        post('/api/aria/crawl', { url, max_pages: 10, context: `Taught by ${senderName} via /teach` }).catch(err => {
          console.warn('[WA /teach crawl] post failed:', err?.message || err);
        });
...
async function sendMessage(chatId, text, quotedMsg = null) {
  ...
  const chunks = splitMessageSmart(normalised, 3800);
  let firstKey = null;
...
async function askARIA(message, groupContext = '', sender = 'whatsapp') {
  ...
  // The message field now carries ONLY the user's actual current question.
  // The group_context field is a separate structured field for the server
  // to use as a context layer (or ignore — the server-side strip handles
  // either format, so this change is rollout-safe in either order).
  ...
  // 2026-04-11 Hanwha Redback incident: Anthropic stalled, fallback to
  // DeepSeek + deep_research pushed total past 240s. Research prompts
  // now get 360s (6 min), normal chat stays at 240s. server.mjs outer
  // must exceed these — bumped to 420s / 300s respectively.
  ...
  try {
    const r = await fetch(`http://localhost:${port}/api/aria/chat`, {
      ...
      body:   JSON.stringify({
        message: fullMessage,
        session_id: sid,
        // Group context as a SEPARATE field — the server treats it as
        // an additional context layer (NOT prepended to the message
        // body). Always present even when empty so the field is
        // schema-stable. Older fly.io builds will ignore unknown fields.
...
`;
  ...
  const rawReply = await askARIA(enrichedText, recentMsgs, senderName);
