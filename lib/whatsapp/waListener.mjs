...
async function backupAuthToRedis() {
  ...
  try {
    // Only backup essential auth files — creds.json is critical, plus key files
    ...
    for (const f of files) {
      ...
      // Skip if bundle would exceed 500KB (Upstash free tier limit per key)
      if (totalSize > 500000) {
        console.warn(`[WA Listener] Auth bundle too large (${Math.round(totalSize/1024)}KB) — backing up ${Object.keys(authBundle).length} files only`);
        break;
      }
      authBundle[f] = content;
    }
    if (!Object.keys(authBundle).length) return;
    await redisSet(AUTH_REDIS_KEY, authBundle);
    console.log(`[WA Listener] Auth backed up to Redis (${Object.keys(authBundle).length}/${allFiles.length} files, ${Math.round(totalSize/1024)}KB)`);
  } catch(e) {
...
async function restoreAuthFromRedis() {
  if (!redisConfigured()) {
    console.warn('[WA Listener] Redis not configured — cannot restore auth. Check UPSTASH_REDIS_URL and UPSTASH_REDIS_TOKEN');
    return false;
  }
  try {
    console.log('[WA Listener] Attempting auth restore from Redis...');
    const authBundle = await redisGet(AUTH_REDIS_KEY);
    if (!authBundle || typeof authBundle !== 'object') {
      console.warn('[WA Listener] No auth bundle found in Redis (key:', AUTH_REDIS_KEY, ')');
      return false;
    }
    const files = Object.keys(authBundle);
    if (!files.length) {
      console.warn('[WA Listener] Auth bundle empty in Redis');
      return false;
    }
    ...
    for (const [f, content] of Object.entries(authBundle)) {
      try {
        ...
        console.warn(`[WA Listener] Failed to write auth file ${f}:`, e.message);
      }
    }
