...
const WATCHDOG_QUIET_MS    = 10 * 60 * 1000;  // 10 min of no inbound → probe
...
async function startListener() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    // Proof-of-life for the watchdog. Must bump BEFORE any filters — a history
    // sync or non-group notification is still evidence the WS is delivering
    // events. We only care about "has Baileys produced ANY inbound signal".
    lastInboundAt = Date.now();
