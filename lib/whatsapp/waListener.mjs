...
async function handleCommand(cmd, args, senderName, senderJid) {
  ...
  switch (cmd.toLowerCase()) {
    ...
    case 'ask': {
      ...
      return await askARIA(a, '', senderName);
    }
    case 'groupsummary': {
      const recent = messageStore.slice(-50).map(m => `[${m.senderName}]: ${m.text.slice(0, 200)}`).join('\n');
      if (!recent) return 'No messages stored yet.';
      return await askARIA(`Summarise this group conversation. Highlight key topics, decisions, action items, and any compliance or risk mentions:\n\n${recent}`, '', senderName);
    }
    case 'help':
      return [
...
let qrPrinted      = false;
let suppressAuthBackup = false;  // prevent stale auth backup during 440 cleanup
let messagesHeard  = 0;
let messagesSent   = 0;
let startedAt      = null;
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === 'close') {
      ...
      if (logout) {
        ...
        console.log('[WA Listener] ⚠ Logged out — clearing ALL auth for fresh QR...');
        // Delete from Redis FIRST to prevent restore loop
        if (upstashOk()) {
          try {
            await upstashCmd('DEL', AUTH_REDIS_KEY);
            console.log('[WA Listener] Stale auth deleted from Redis');
          } catch (e) { console.warn('[WA Listener] Redis auth delete failed:', e.message); }
        }
        try { fs.rmSync(AUTH_DIR, { recursive: true, force: true }); } catch {}
        if (sock) { try { sock.end(); } catch {} sock = null; }
        suppressAuthBackup = false;
        console.log('[WA Listener] Auth fully cleared — will show QR on restart');
        setTimeout(startListener, 10000);
      } else if (code === 440) {
        // Session expired/conflict — clear auth and get fresh QR
        suppressAuthBackup = true;  // prevent creds.update from re-saving stale auth
        console.log('[WA Listener] Session expired (440) — clearing ALL auth for fresh QR scan...');
        // Delete stale auth from Redis FIRST
        if (upstashOk()) {
          try {
            await upstashCmd('DEL', AUTH_REDIS_KEY);
            console.log('[WA Listener] Stale auth deleted from Redis');
        ...
        try { fs.rmSync(AUTH_DIR, { recursive: true, force: true }); } catch {}
        console.log('[WA Listener] Local auth cleared — QR scan required on next start');
        // Close existing socket to stop creds.update events
        if (sock) { try { sock.end(); } catch {} sock = null; }
        suppressAuthBackup = false;
