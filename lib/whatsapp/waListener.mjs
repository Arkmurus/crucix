    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const body = (chunks.length > 1 && i < chunks.length - 1)
        ...
        : (chunks.length > 1
           ...
           : chunks[i]);
      await sock.sendMessage(chatId, { text: body }, opts);
...
// askARIA() now returns either a string OR { error: true, message } so we
// can handle failures intelligently instead of silently swallowing them.
function _unwrapAriaReply(result, fallbackMsg = 'ARIA is temporarily unavailable. Please try again shortly.') {
  ...
  if (result && result.error) {
    ...
    return `${fallbackMsg}\n\n_Diagnostic: ${result.message?.slice(0, 200) || 'unknown error'}_`;
  }
  return fallbackMsg;
}
...
// BUG-FIX: previously this slice'd the response to 3000 chars, which is why
// users were seeing incomplete paragraphs. The smart splitter in
// sendMessage() handles long messages by breaking on sentence boundaries
// across multiple WhatsApp messages, so we should NOT pre-truncate here.
// Cap at 16000 (4 chunks) as a sanity guard against runaway responses.
