...
async function handleCommand(cmd, args, senderName, senderJid, notify = null) {
  ...
  switch (cmd.toLowerCase()) {
    ...
    case 'teach': {
      ...
      const urlMatch = a.match(/^(https?:\/\/\S+)$/i) || forceMatch;
      if (urlMatch) {
        const url = forceMatch ? forceMatch[1] : urlMatch[1];
        let host = url;
...
async function askARIA(message, groupContext = '', sender = 'whatsapp', progressCb = null) {
  ...
  try {
      ...
      for (const raw of events) {
        ...
        try {
          ...
          if (evt.type === 'chunk' && evt.text) {
            ...
            console.warn('[WA→ARIA] stream error event:', evt.message || evt.kind);
          }
    ...
    try {
      ...
      if (!r.ok) throw new Error(`ARIA fallback ${r.status}`);
      const data = await r.json();
      ...
      return { error: true, message: e2.message };
    }
  }
}
...
`;
  ...
  const progressCb = (statusMsg) => {
    ...
    _lastStatusAt = now;
    sendMessage(chatId, `*ARIA* — 🔄 _${statusMsg}_`, msg).catch(() => {});
  };
