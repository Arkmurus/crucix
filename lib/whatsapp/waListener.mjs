...
async function askARIA(message, groupContext = '', sender = 'whatsapp', progressCb = null) {
  ...
  const timeoutMs = isLong ? _longMs : _shortMs;
...
`;
  ...
  if (isLong) {
    // R-F525: bumped user-facing budget message from "4 minutes" to
    // "up to 10 minutes" to match the new 600s long budget.
    // R-F1176: if the message contains a URL, the tool runs in seconds
    // and sends preliminary results immediately. Send a shorter message.
    if (/https?:\/\//i.test(text)) {
      await sendMessage(chatId, `*ARIA* - Fetching the page now - I will post findings as they come in.`, null).catch(() => {});
    } else {
      await sendMessage(chatId, `*ARIA* - On it. This involves research across multiple sources - give me up to 10 minutes. I will come back with a structured brief and citations._`, null).catch(() => {});
    }
