...
<style>
  ...
  .meta{font-size:11px;color:rgba(255,255,255,0.35);margin-top:14px}
</style>
</head><body>
<div class="wrap">
  ...
  ${status === 'connected'
    ...
    : '<p>WhatsApp Business → Settings → Linked Devices → Link a Device. Then point your camera at the QR below.</p>' +
      ...
      '<p>QR refreshes every ~20 seconds. The page auto-reloads the image.</p>'
  }
  <div class="meta">Captured: <span id="ts">${latestQrAt ? new Date(latestQrAt).toISOString() : '—'}</span></div>
</div>
<script>
  // Re-fetch the QR image every 5s with a cache-buster — covers the
  // ~20s rotation cycle without the page going stale.
  if (${status === 'connected' ? 'false' : 'true'}) {
    setInterval(function() {
      var img = document.getElementById('qr');
      if (img) img.src = '/api/wa-listener/qr.png${tokQs}${tokQs ? '&' : '?'}t=' + Date.now();
    }, 5000);
    // Also reload the whole page every 60s in case the listener
    // transitions to connected (so the operator sees the success state).
