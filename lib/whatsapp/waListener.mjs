...
async function startListener() {
  ...
  // event instead of embedding them in `messages.upsert.message.reactionMessage`.
  // We handle both — the upsert path for older versions (still in the loop
  // below as a fallback) AND this dedicated handler for current versions.
  // Reactions arrive as an array of { key, reaction } where:
  //   key       = the message being reacted to (.id, .remoteJid, .fromMe)
  //   reaction  = { text: '👍', key, senderTimestampMs }  (text='' = removed)
  // We only care about reactions on messages ARIA sent (key.fromMe === true).
  sock.ev.on('messages.reaction', async (reactions) => {
    ...
    for (const r of reactions) {
      try {
        ...
        if (!targetKey.fromMe) continue;  // Only reactions on ARIA's own messages
        const targetMsgId = targetKey.id;
        const targetChatId = targetKey.remoteJid || '';
        if (!targetMsgId || !targetChatId) continue;
        // The reactor's identity comes from r.reaction.key.participant
        // (group context) or r.reaction.key.remoteJid (dm context)
        const reactionInner = r.reaction || {};
        const reactorJid = reactionInner.key?.participant || reactionInner.key?.remoteJid || '';
        const reactorName = reactorJid.split('@')[0] || 'unknown';
        const emoji = reactionInner.text || '';
