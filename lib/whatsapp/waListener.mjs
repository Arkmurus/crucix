  ...
  switch (cmd.toLowerCase()) {
    ...
    case 'teach': {
      ...
      if (urlMatch) {
        ...
        // Fire-and-forget: crawl can take minutes, don't block the WA reply.
        post('/api/aria/crawl', { url, max_pages: 10, context: `Taught by ${senderName} via /teach` }).catch(() => {});
        return `Crawling *${host}* — I'll ingest pages into my knowledge index. Ask me about ${host} in a minute or two and I should have something to share.`;
      }
      const sep = a.indexOf(':');
      if (sep < 1) return 'Usage: /teach [topic]: [fact]   OR   /teach <https://url>';
      const topic = a.slice(0, sep).trim();
      const fact = a.slice(sep + 1).trim();
      // Guard against URL-shaped values being misparsed (topic="https") even
      // when the user did use a colon — if topic looks like a scheme, bail.
      if (/^https?$/i.test(topic)) {
        return 'It looks like you pasted a URL. Use:  /teach https://example.com  (no colon prefix needed for URLs).';
      }
    ...
    case 'report': {
      ...
      // Generates an Arkmurus-formatted draft via /api/aria/report which
      ...
      if (!a) {
        return [
          ...
          '',
          'Drafts use any Arkmurus templates ingested as Tier D in the corpus.',
      ...
      try {
        const resp = await fetch(`${ariaServiceUrl}/api/aria/report`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ report_type: reportType, subject }),
          // Reports do investigate + LLM fill — long-running, give it 5 min
          signal: AbortSignal.timeout(300000),
        });
        if (!resp.ok) return `⚠️ Report build failed: HTTP ${resp.status}`;
        const d = await resp.json();
        if (d.error) return `⚠️ Report build error: ${d.error}`;
        const header = [
          `*ARKMURUS ${reportType.toUpperCase()} REPORT — DRAFT*`,
          `Subject: ${subject}`,
    ...
    case 'forget': {
      ...
      if (!ariaServiceUrl) return '⚠️ ARIA_SERVICE_URL not configured.';
      try {
        const resp = await fetch(`${ariaServiceUrl}/api/aria/session/forget`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ session_id: sid }),
          signal: AbortSignal.timeout(10000),
        });
        if (!resp.ok) return `⚠️ /forget failed: HTTP ${resp.status}`;
        const d = await resp.json();
        if (d.existed) return `🧹 Conversation history wiped for ${senderName}. Next message starts fresh.`;
        return `(No active session found for ${senderName} — nothing to wipe.)`;
      } catch (e) {
        return `⚠️ /forget error: ${e.message}`;
      }
    }
    case 'purgecases':
    case 'purge-cases': {
      ...
      try {
        const resp = await fetch(`${ariaServiceUrl}/api/aria/admin/purge-cases`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ dry_run: dryRun }),
          signal: AbortSignal.timeout(60000),
        });
        if (!resp.ok) return `⚠️ Purge failed: HTTP ${resp.status}`;
        const d = await resp.json();
    ...
    case 'investigate': {
      ...
      // whitespace. Without this, /investigate sees an opaque phone number
      // instead of "Arthur Switzerland".
      let target = a.replace(/@\d{6,}/g, '').replace(/\s+/g, ' ').trim();
      if (!target) return 'Usage: /investigate [name]   (WhatsApp @-mentions are anonymised by the protocol — type the name out)';
      // Cap to keep prompt manageable. If user pasted a long brief, take the
      // first line as the subject and pass the rest as context.
      let extraContext = '';
    ...
    case 'honesty': {
      ...
      try {
        if (arg.startsWith('jdg_')) {
          const r = await fetch(`${ariaServiceUrl}/api/aria/honesty/${encodeURIComponent(arg)}`, {
            signal: AbortSignal.timeout(10000),
          });
        ...
        if (arg === 'bad' || arg === 'fails' || arg === 'suspicious') {
          const r = await fetch(`${ariaServiceUrl}/api/aria/honesty/list?limit=15&bad_only=true`, {
            signal: AbortSignal.timeout(10000),
          });
        ...
        // Default: stats
        const r = await fetch(`${ariaServiceUrl}/api/aria/honesty/stats`, {
          signal: AbortSignal.timeout(10000),
        });
    ...
    case 'trace': {
      ...
      try {
        if (arg.startsWith('tr_')) {
          const r = await fetch(`${ariaServiceUrl}/api/aria/trace/${encodeURIComponent(arg)}`, {
            signal: AbortSignal.timeout(10000),
          });
        ...
        if (arg === 'recent' || arg === 'list') {
          const r = await fetch(`${ariaServiceUrl}/api/aria/trace/recent?limit=15`, {
            signal: AbortSignal.timeout(10000),
          });
        ...
        if (arg === 'bad' || arg === 'fails') {
          const r = await fetch(`${ariaServiceUrl}/api/aria/trace/recent?limit=15&bad_only=true`, {
            signal: AbortSignal.timeout(10000),
          });
          if (!r.ok) return `⚠️ Bad list failed: ${r.status}`;
          const d = await r.json();
        ...
        // Default: stats
        const r = await fetch(`${ariaServiceUrl}/api/aria/trace/stats`, {
          signal: AbortSignal.timeout(10000),
        });
    ...
    case 'cost': {
      ...
      try {
        if (arg.startsWith('call_')) {
          const r = await fetch(`${ariaServiceUrl}/api/aria/cost/call/${encodeURIComponent(arg)}`, {
            signal: AbortSignal.timeout(10000),
          });
        ...
        if (arg === 'recent') {
          const r = await fetch(`${ariaServiceUrl}/api/aria/cost/recent?limit=15`, {
            signal: AbortSignal.timeout(10000),
          });
          if (!r.ok) return `⚠️ Recent failed: ${r.status}`;
          const d = await r.json();
        ...
        if (arg === 'cumulative' || arg === 'total' || arg === 'lifetime') {
          const r = await fetch(`${ariaServiceUrl}/api/aria/cost/cumulative`, {
            signal: AbortSignal.timeout(10000),
          });
        ...
        const r = await fetch(`${ariaServiceUrl}/api/aria/cost/summary?window_hours=${windowHours}`, {
          signal: AbortSignal.timeout(10000),
        });
    ...
    case 'verify': {
      ...
      try {
        if (arg.startsWith('vfy_')) {
          const r = await fetch(`${ariaServiceUrl}/api/aria/verify/${encodeURIComponent(arg)}`, {
            signal: AbortSignal.timeout(10000),
          });
        ...
        if (validVerdicts.includes(arg)) {
          const r = await fetch(`${ariaServiceUrl}/api/aria/verify/list?limit=10&verdict=${encodeURIComponent(arg)}`, {
            signal: AbortSignal.timeout(10000),
          });
        ...
        // Default: stats
        const r = await fetch(`${ariaServiceUrl}/api/aria/verify/stats`, {
          signal: AbortSignal.timeout(10000),
        });
    ...
    case 'eval': {
      ...
      try {
        ...
        if (!arg || sub === 'run') {
          const r = await fetch(`${ariaServiceUrl}/api/aria/eval/run`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (sub === 'list' || sub === 'golden') {
          const r = await fetch(`${ariaServiceUrl}/api/aria/eval/golden`, {
            signal: AbortSignal.timeout(10000),
          });
        ...
        if (sub === 'runs' || sub === 'history') {
          const r = await fetch(`${ariaServiceUrl}/api/aria/eval/runs?limit=10`, {
            signal: AbortSignal.timeout(10000),
          });
        ...
        if (sub === 'add' || sub === 'promote') {
          ...
          const r = await fetch(`${ariaServiceUrl}/api/aria/eval/golden/promote`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (sub === 'rm' || sub === 'remove' || sub === 'delete') {
          ...
          const r = await fetch(`${ariaServiceUrl}/api/aria/eval/golden/${encodeURIComponent(rest)}`, {
            method: 'DELETE',
            signal: AbortSignal.timeout(10000),
          });
    ...
    case 'help':
      return [
        ...
        '/report [dd|compliance|market] [subject] — Branded Arkmurus draft',
        '',
        '*Compliance*',
...
async function askARIA(message, groupContext = '', sender = 'whatsapp') {
  ...
  // Dynamic timeout — aligned across the whole chain so we don't have N
  // different timeouts cutting off the request at N different points.
  //
  // 2026-04-08 round 4:  90s → 180s for non-research path
  // 2026-04-08 round 5c: server.mjs Python timeout 90s → 240s
  // 2026-04-08 round 5d: bumped non-research path to 240s to MATCH the
  //                      server.mjs ceiling. Otherwise officeholder questions
  //                      auto-trigger investigate on the Python side (which
  //                      can take 180-200s), Python returns just in time, but
  //                      the WA-side cap at 180s already aborted the request.
  //                      Same failure mode as the round-4 90s bug, one layer
  //                      down. ALL timeouts in the chain now consolidated at
  //                      240s for normal questions and 240s for research too
  //                      (the difference no longer matters — they're equal).
  const isLong = _isLongResearchPrompt(message);
  const timeoutMs = isLong ? 240000 : 240000;  // 4min everywhere — chain aligned
  ...
  try {
    ...
    // Return an object so callers can also access trace_id for snapshot
    // correlation. Stringly call sites still work via toString fallback
    // in _unwrapAriaReply (it checks typeof === 'string').
...
async function handleAriaMention(msg, chatId, groupName, senderName, text) {
  ...
  if (isLong) {
    ...
    // Quick acknowledge for normal questions. Skip for very short messages
    // (likely greetings already handled by trivial short-circuit anyway).
    await sendMessage(chatId, `*ARIA* — 💭 _Thinking…_`, msg).catch(() => {});
  }
...
async function startListener() {
  ...
  // event. The shape is INCONSISTENT across versions — sometimes the target
  // message key is at r.key, sometimes at r.reaction.key. This handler
  // reads BOTH locations and uses whichever has fromMe=true. Logs the raw
  // payload on first hit so future debugging is easier.
  ...
  sock.ev.on('messages.reaction', async (reactions) => {
    ...
    for (const r of reactions) {
      try {
        // Try to find a key with fromMe=true — that's the target (ARIA's
        // own message). Different Baileys versions put it in different
        // places, so check both.
        const outerKey = r.key || {};
        const innerKey = r.reaction?.key || {};
        let targetKey = null;
        let reactorWrapper = null;
        if (outerKey.fromMe) {
          targetKey = outerKey;
          reactorWrapper = innerKey;
        } else if (innerKey.fromMe) {
          targetKey = innerKey;
          reactorWrapper = outerKey;
        } else {
          // Neither key claims fromMe — could be a reaction on another user's
          // message (which we don't care about) OR the schema is different
          // again. Log for diagnosis but skip.
          console.log('[WA Reaction] skipping — neither key has fromMe=true. outer:',
            JSON.stringify(outerKey).slice(0, 200), 'inner:',
            JSON.stringify(innerKey).slice(0, 200));
          continue;
        }
        const targetMsgId = targetKey.id;
        const targetChatId = targetKey.remoteJid || '';
        if (!targetMsgId || !targetChatId) {
          console.log('[WA Reaction] skipping — missing target id/chat');
          continue;
        }
        const reactorJid = reactorWrapper.participant || reactorWrapper.remoteJid || '';
        ...
        const resp = await fetch(`${ariaServiceUrl}/api/aria/feedback`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
