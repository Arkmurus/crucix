  ...
  try {
    ...
    if (m.imageMessage) {
      ...
      try {
        ...
        if (!extracted) {
          // The OCR pipeline returned nothing usable. The Python side has
          // already triggered background auto-install of easyocr (if not
          // already running), so we just tell the user the image was blank
          // and that ARIA's eyes are coming online.
          const autoInst = ocrResult?.auto_installing;
          await sendMessage(chatId, [
            `*ARIA* — 🖼 I looked at the image but couldn't extract readable text from it.`,
            ``,
