...
// R-F3661 — every call in this module to `${ARIA_SERVICE_URL}/api/aria/...`
// (14 of them: /self/code, /self/staged, /self/deploy, /self/rollback,
// /student/stats, /student/quiz, /student/study, /student/curriculum,
// /independence, /vision-status x2, /ocr, /read-document x2, /read) was sent
// with NO Authorization header. Those routes are Bearer-gated — probed live
// 2026-08-03 — so all fourteen WhatsApp commands answered 401 and the user got
// "⚠️ ... failed: 401". Same defect family as R-F3655/R-F3656, caught here by
// the new auth-on-egress gate (scripts/admin/auth_egress_gate.mjs).
//
// This module already knew how to authenticate (INT_TOKEN is used on the
// localhost calls below) — the aria-intel calls simply never did it.
function _ariaFetch(url, opts = {}) {
  const token = process.env.ARIA_API_TOKEN || INT_TOKEN;
...
// ── ARIA API (calls own server.mjs endpoints) ──────────────────────────────
...
async function handleCommand(cmd, args, sender) {
  ...
  switch (cmd.toLowerCase()) {
    ...
    case 'brief': {
      const [stats, id] = await Promise.all([
        brainGet('/api/aria/brain/stats').catch(() => ({})),
      ...
      if (composite != null) {
        msg += `*Composite intelligence:* ${(composite * 100).toFixed(1)}%\n`;
      }
      const signals = stats.total_signals || stats.signals_24h;
      if (signals != null) {
        msg += `*Signals (24h):* ${signals}\n`;
      }
      msg += `\n_Use /sweep to trigger a fresh intelligence run._\n\n`;
      msg += `ARIA v${id.version||'3.0'} | ${id.markets||80} markets`;
      return msg;
    }

    case 'sweep':
      await brainPost('/api/sweep', {}).catch(() => {});
    ...
    case 'code': {
      ...
      try {
        const resp = await _ariaFetch(`${ariaServiceUrl}/api/aria/self/code`, {
          method: 'POST',
    ...
    case 'staged': {
      ...
      try {
        const resp = await _ariaFetch(`${ariaServiceUrl}/api/aria/self/staged`, {
    ...
    case 'deploy': {
      ...
      if (!ariaServiceUrl) return '⚠️ ARIA_SERVICE_URL not configured.';
      try {
        const resp = await _ariaFetch(`${ariaServiceUrl}/api/aria/self/deploy/${encodeURIComponent(id)}`, {
          method: 'POST',
    ...
    case 'student': {
      ...
      if (!ariaServiceUrl) return '⚠️ ARIA_SERVICE_URL not configured.';
      try {
        const resp = await _ariaFetch(`${ariaServiceUrl}/api/aria/student/stats`, {
          signal: AbortSignal.timeout(15000),
        });
    ...
    case 'quiz': {
      ...
      if (!ariaServiceUrl) return '⚠️ ARIA_SERVICE_URL not configured.';
      try {
        const resp = await _ariaFetch(`${ariaServiceUrl}/api/aria/student/quiz`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    case 'study': {
      ...
      if (!ariaServiceUrl) return '⚠️ ARIA_SERVICE_URL not configured.';
      try {
        const resp = await _ariaFetch(`${ariaServiceUrl}/api/aria/student/study`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    case 'curriculum': {
      ...
      if (!ariaServiceUrl) return '⚠️ ARIA_SERVICE_URL not configured.';
      try {
        const resp = await _ariaFetch(`${ariaServiceUrl}/api/aria/student/curriculum`, {
          signal: AbortSignal.timeout(15000),
        });
    ...
    case 'reasoning': {
      ...
      if (!ariaServiceUrl) return '⚠️ ARIA_SERVICE_URL not configured.';
      try {
        const resp = await _ariaFetch(`${ariaServiceUrl}/api/aria/independence`, {
          signal: AbortSignal.timeout(15000),
        });
    ...
    case 'vision-status': {
      ...
      if (!ariaServiceUrl) return '⚠️ ARIA_SERVICE_URL not configured.';
      try {
        const resp = await _ariaFetch(`${ariaServiceUrl}/api/aria/vision-status`, {
          signal: AbortSignal.timeout(10000),
        });
    ...
    case 'rollback': {
      ...
      if (!ariaServiceUrl) return '⚠️ ARIA_SERVICE_URL not configured.';
      try {
        const resp = await _ariaFetch(`${ariaServiceUrl}/api/aria/self/rollback/${encodeURIComponent(id)}`, {
          method: 'POST',
          signal: AbortSignal.timeout(30000),
        });
  ...
  async (req, res) => {
      ...
      for (let i = 0; i < numMediaInt; i++) {
        ...
        try {
          ...
          if (isImage) {
            ...
            let ocrResult = null;
            try {
              const ocrResp = await _ariaFetch(`${ariaServiceUrl}/api/aria/ocr`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                  image: b64,
            ...
            if (!extracted) {
              ...
              try {
                const vs = await _ariaFetch(`${ariaServiceUrl}/api/aria/vision-status`, {
            ...
            let factsLearned = 0;
            try {
              const docResp = await _ariaFetch(`${ariaServiceUrl}/api/aria/read-document`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    if (ariaServiceUrl) {
      ...
      for (const articleUrl of urls) {
        ...
        _ariaFetch(`${ariaServiceUrl}/api/aria/read`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          ...
          if (!r || !r.ok) {
            console.error(`[WhatsApp→ARIA] Read FAILED for ${articleUrl.slice(0, 60)}: HTTP ${r?.status || 'no response'}`);
            send(sender, `⚠️ I tried to read that article but the fetch failed (HTTP ${r?.status || 'unreachable'}). The URL may be paywalled or blocking bots.`).catch(() => {});
            return null;
          }
          return r.json();
        }).then(result => {
          if (!result) return;
          if (result.error) {
            console.error(`[WhatsApp→ARIA] Read ERROR for ${articleUrl.slice(0, 60)}: ${result.error}`);
            send(sender, `⚠️ I fetched the article but couldn't extract content: ${result.error}`).catch(() => {});
          } else if (result.facts_learned > 0) {
            ...
            console.log(`[WhatsApp→ARIA] Read: ${articleUrl.slice(0, 60)} → 0 facts (content too thin or already known)`);
            send(sender, `📖 I read that article but found no new facts to store — the content may be too thin, paywalled, or already in my knowledge base.`).catch(() => {});
          }
        }).catch(err => {
          console.error(`[WhatsApp→ARIA] Read EXCEPTION for ${articleUrl.slice(0, 60)}: ${err.message}`);
          send(sender, `⚠️ I couldn't read that URL: ${err.message?.includes('timeout') ? 'request timed out (120s)' : 'connection error'}. I'll try again on the next sweep.`).catch(() => {});
        });
      }
    }
...
router.get('/status', (_req, res) => {
  res.json({
    ...
    webhook_url: 'https://intel.arkmurus.com/api/whatsapp/incoming',
  });
});
