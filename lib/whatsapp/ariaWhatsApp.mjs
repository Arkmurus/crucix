...
if (TWILIO_SID && TWILIO_TOKEN) {
  try {
    ...
    console.log('[WhatsApp] Twilio client initialised — ARIA can send WhatsApp messages');
  } catch (e) {
    twilioError = e.message;
    console.warn('[WhatsApp] Twilio init failed:', e.message);
    console.warn('[WhatsApp] Install with: npm install twilio');
  }
} else {
  twilioError = !TWILIO_SID ? 'TWILIO_ACCOUNT_SID not set' : 'TWILIO_AUTH_TOKEN not set';
}
