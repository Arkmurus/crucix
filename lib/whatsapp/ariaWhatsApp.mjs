...
async function send(to, body) {
  ...
  try {
    ...
    const params = new URLSearchParams({ From: FROM, To: to, Body: body.slice(0, 4096) });
            ...
            if (mediaResp.ok && ariaServiceUrl) {
              ...
              if (content.length > 50) {
                fetch(`${ariaServiceUrl}/api/aria/read-document`, {
                  ...
                  body: JSON.stringify({
                    content,
                    filename: `whatsapp_${mediaType.split('/')[1] || 'doc'}`,
                  ...
                  if (result?.facts_learned > 0) {
                    console.log(`[WhatsApp→ARIA] Doc analysed → ${result.facts_learned} facts`);
                    send(sender, `📄 I analysed that document and learned ${result.facts_learned} new fact(s).`).catch(() => {});
                  }
                }).catch(() => {});
              }
            }
