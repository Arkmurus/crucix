        ...
        try {
          ...
          if (isImage) {
            ...
            if (!extracted) {
              ...
              if (!visionConfigured) {
                send(sender, [
                  `🖼 *Image received but local OCR is not installed.*`,
                  ``,
                  `ARIA does image reading on her own — she doesn't need any cloud LLM. You just need to install one of these on the ARIA service host:`,
                  ``,
                  `*RECOMMENDED — EasyOCR (zero config, pure Python):*`,
                  `\`pip install easyocr Pillow\``,
                  `→ first image takes ~30s while it downloads ~250MB of model weights, then it's instant and offline forever.`,
                  ``,
                  `*Alternative — Tesseract (smaller, needs system binary):*`,
                  `\`pip install pytesseract Pillow\``,
                  `+ install Tesseract: brew install tesseract / apt install tesseract-ocr`,
                  ``,
                  `*Best quality — Ollama vision model (local LLM):*`,
                  `\`ollama pull minicpm-v\` (or llava, bakllava, moondream)`,
                  `→ ARIA auto-detects and uses it.`,
                  ``,
                  `Run */vision-status* to verify which backend is active.`,
                ].join('\n')).catch(() => {});
              } else {
                send(sender, `🖼 I looked at *${filename}* but couldn't extract any readable text from it. The image may be blank, low-resolution, or contain only diagrams. If you need analysis, describe what you're looking for.`).catch(() => {});
              }
              continue;
            }
