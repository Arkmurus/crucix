...
export class ConversationManager {
  ...
  // ── Progressive disclosure: quick answer first, deep-dive on request ──
  // R-F1768: instead of waiting 60-180s for a complete analysis, send a
  // quick initial assessment immediately, then offer to dig deeper.
  // The user can reply "yes please" or "dig deeper" to get the full analysis.
  // This is the single biggest UX improvement for WhatsApp interactions.
  async progressiveAnswer(conv, text, sendMessageFn, askARIAFn) {
    // Step 1: Try a very fast LLM call (15s budget) for a quick summary
    const quickStrategy = { name: 'fast_llm', timeoutMs: 15000, label: 'quick answer' };
    try {
      const quickResult = await this._executeStrategy(quickStrategy, conv, text, askARIAFn);
      if (quickResult && quickResult.text && quickResult.text.length > 20) {
        // We got a quick answer — send it with an offer to dig deeper
        conv.startResponding();
        const quickAnswer = quickResult.text.slice(0, 500); // keep it brief
        return {
          text: `${quickAnswer}\n\n_I can dig deeper into this if you want — just say "yes please" or "more detail" and I will run the full analysis._`,
          quick: true,
          fullResult: quickResult,
        };
      }
    } catch (err) {
      console.warn(`[ConvManager] Quick answer failed: ${err.message}`);
    }

    // Step 2: Quick answer failed or was empty — fall back to full analysis
    return this._processWithStrategies(conv, text, sendMessageFn, askARIAFn);
  }
