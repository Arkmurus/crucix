...
const IMAP_HOST  = process.env.ARIA_EMAIL_HOST  || 'outlook.office365.com';
...
const SMTP_FROM  = process.env.ARIA_EMAIL_FROM || `Imaria Intelligence <${IMAP_USER || 'aria@imaria.io'}>`;
...
async function askARIAToCompose({ to, originalSubject, originalBody, instruction }) {
  ...
  const prompt = `You are composing a professional email reply as ARIA on behalf of Imaria.
...
Write a professional, concise email. Use the Imaria brand tone: authoritative but approachable, intelligence-focused.
Return ONLY the email body text — no subject line, no "Dear X" salutation unless appropriate, no sign-off beyond "Best regards, ARIA — Imaria Intelligence".`;
...
export function mountEmailReader(app) {
  ...
  app.post('/api/email/draft', requireEmailAuth, async (req, res) => {
    ...
    res.json({
      ok:      true,
      draft:   true,
      subject: subject || (original_subject ? `Re: ${original_subject}` : 'Imaria Intelligence Update'),
      body:    composedBody,
      to:      to || null,
    });
  });
