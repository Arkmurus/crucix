            ...
            for (const email of emails) {
              try {
                ...
                if (ariaUrl) {
                  ...
                  if (!isBackfill) {
                    // F47 fix 2026-04-27: email.body is raw and may
                    // contain quoted-printable encoding (=3F = '?',
                    // =3D = '=', trailing '=' = soft line-break). The
                    // 21:18:37 World Bank newsletter sent 4 URLs like
                    // https://t.newsletterext.worldbank.org/r/=3Fid=3D...
                    // through to /api/aria/read which all 404'd.
                    // Decode QP first, then defensively reject any URL
                    // that still has QP residue (regex didn't normalise
                    // a soft-break-spanned URL fully).
                    const decodedBody = _decodeQuotedPrintable(email.body || '');
