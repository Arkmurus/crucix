...
// 2026-04-21: previously HTML-only. Multipart emails (text/plain + text/html
// alternatives, quoted-printable encoding, MIME boundaries) arrive with raw
// body that contains part headers + boundary markers. extractText returned
// <20 chars for those, silently dropping real email content at the
// empty-body guard in the caller. Now: strip MIME noise, normalise
// quoted-printable soft line-breaks, and fall back to the raw body minus
// boundary markers if the HTML-stripped path yields nothing usable.
function extractText(body) {
  if (!body) return '';

  // ── Step 1: if this is a multipart message, try to extract the text/plain
  // part only. Look for "Content-Type: text/plain" ... "\r\n\r\n" ... "--boundary".
  let working = body;
  const plainMatch = body.match(
    /Content-Type:\s*text\/plain[^\r\n]*\r?\n(?:[^\r\n]+\r?\n)*\r?\n([\s\S]*?)(?:\r?\n--[\w=_.-]+|\r?\n\r?\nContent-Type:)/i,
  );
  if (plainMatch && plainMatch[1] && plainMatch[1].trim().length >= 20) {
    working = plainMatch[1];
  }

  // ── Step 2: decode quoted-printable soft line-breaks and =XX escapes ──
  working = working
    .replace(/=\r?\n/g, '')                                  // soft line-break
    .replace(/=([0-9A-Fa-f]{2})/g, (_, h) => {
      try { return String.fromCharCode(parseInt(h, 16)); }
      catch { return ''; }
    });

  // ── Step 3: HTML-strip (idempotent on already-plain text) ──
  let text = working
    ...
    .replace(/&quot;/g, '"')
    .replace(/&#39;/g, "'")
    // Strip MIME boundary lines + part headers that leaked through
    .replace(/^--[\w=_.-]+(?:--)?[\r\n]*/gm, '')
  ...
  // ── Step 4: if we still have nothing usable, fall back to a raw-body
  // cleanup — strip boundaries/headers from the original bytes and see if
  // any signal remains. Better a noisy fragment than silent drop.
  if (text.length < 20 && body.length >= 20) {
    const fallback = body
      ...
      .trim()
      .slice(0, 10000);
    if (fallback.length >= 20) text = fallback;
  }
