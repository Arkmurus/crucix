  ...
  return new Promise((resolve) => {
    ...
    imap.once('ready', () => {
      ...
      imap.openBox('INBOX', false, (err, box) => {
        ...
        console.log(`[Email Reader] INBOX opened — ${box.messages?.total || 0} total, ${box.messages?.unseen || 0} unseen`);

        // Search for unseen emails
        imap.search(['UNSEEN'], (err, results) => {
          if (err || !results || !results.length) {
            if (err) console.warn(`[Email Reader] Search error: ${err.message}`);
            else console.log(`[Email Reader] No unseen emails (inbox has ${box.messages?.total || 0} total)`);
            imap.end();
            resolve();
            return;
          }
          ...
          f.once('end', async () => {
            for (const email of emails) {
              try {
                ...
                brainAbsorb({
                  module: type === 'linkedin_alert' ? 'knowledge_ingestor' : 'source_verifier',
                  summary: `Email processed: [${type}] ${subject.slice(0, 120)} from ${from}`,
                  success: true,
                  confidence: 'ASSESSED',
                });
