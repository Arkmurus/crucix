  ...
  if (scored.length > 0) {
    parts.push('KNOWLEDGE BASE (verified facts from past research):\n' +
      scored.slice(0, MAX_CONTEXT_ENTRIES).map(f => {
        // Mark as accessed
        const idx = kb.facts.findIndex(x => x.id === f.id);
        if (idx >= 0) kb.facts[idx].accessCount = (kb.facts[idx].accessCount || 0) + 1;
        return `- [${f.confidence}] ${f.topic}: ${f.content}`;
      }).join('\n'));
    saveKB(kb); // persist access counts
  }
