...
export async function processLinkedInEmail(subject, from, body) {
  ...
  // Signal brain with module-specific absorb
  const totalSignals = results.appointments.length + results.competitors.length + results.growth.length + results.content.length;
  if (totalSignals > 0) {
    brainAbsorb({
      module: 'knowledge_ingestor',
      summary: `LinkedIn intel: ${results.appointments.length} appointments, ${results.competitors.length} competitor signals, ${results.growth.length} growth, ${results.content.length} content — ${subject.slice(0, 80)}`,
      success: true,
      confidence: 'ASSESSED',
    });
  }
...
async function feedSignal(content, signalType) {
  ...
  } catch (e) { console.warn('[LinkedIn] feedSignal failed:', e.message); }
}
