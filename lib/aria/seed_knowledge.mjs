...
const INVESTIGATION_FACTS = [
  ['OSINT corporate registries', 'Key corporate registries for defence investigations: Companies House (UK, free), SEC EDGAR (US, free), OpenCorporates (global aggregator), BRELA (Tanzania), ANIP (Angola), Conservatória (Mozambique/Portugal). Always check registry of incorporation AND operating jurisdictions.', 'CONFIRMED'],
  ['UBO investigation technique', 'Beneficial ownership investigation: Follow 25% ownership threshold. Check UK PSC Register, EU Anti-Money Laundering Directive registers, US FinCEN BOI database. For opaque structures: check nominee director services, trust registers, foundation registries. Red flag: ownership through multiple jurisdictions with secrecy laws (BVI, Cayman, Seychelles, Labuan).', 'CONFIRMED'],
  ['Sanctions screening depth', 'Sanctions screening must go beyond direct matches: 1) Check all name variants/transliterations, 2) Screen parent + subsidiaries, 3) Apply 50% rule (entity 50%+ owned by SDN is itself sanctioned), 4) Check directors and UBOs individually, 5) Screen 2nd-degree connections, 6) Check vessels/aircraft owned by entity. Use fuzzy matching for transliterated names.', 'CONFIRMED'],
  ['Adverse media investigation', 'Adverse media sources for defence due diligence: OCCRP (organised crime), ICIJ (offshore/tax), Global Witness (natural resources), Amnesty International (human rights), Transparency International (corruption), Stockholm Centre for Freedom, Bellingcat (open source investigation). Search in local language + English.', 'CONFIRMED'],
  ['PEP identification', 'Politically Exposed Persons in defence context: serving/former heads of state, ministers of defence/interior/foreign affairs, military chiefs of staff, heads of procurement agencies, heads of state-owned defence companies, their family members (spouse, children, parents, siblings), and close associates (business partners, advisors). PEP status requires Enhanced Due Diligence.', 'CONFIRMED'],
  ['Defence procurement red flags', 'Red flags in defence procurement: 1) Agent commission >5% of contract value, 2) Request for payment to third country, 3) Shell company intermediary with no defence track record, 4) PEP-connected agent, 5) Urgency pressure to bypass due diligence, 6) Unusual contract routing (through non-standard intermediary country), 7) Resistance to end-user documentation, 8) Vague offset proposals, 9) Cash payments, 10) No competitive tender.', 'CONFIRMED'],
  ['Cross-referencing methodology', 'Intelligence cross-referencing hierarchy: 1) Government official records (highest reliability), 2) Corporate registries, 3) Financial filings, 4) Judicial records, 5) Reputable news agencies, 6) Industry publications, 7) Social media/LinkedIn (verify independently), 8) Unattributed sources (lowest). Minimum 2 independent sources for any factual claim used in compliance decisions.', 'CONFIRMED'],
  ['Network mapping technique', 'Relationship network mapping: Start from target entity. Map outward: directors → shared directorships → those companies directors → beneficial owners. Look for: same registered address (nominee office?), same company secretary, same auditor, concurrent appointments, family surnames, military academy cohort years. Build graph: nodes = entities, edges = relationships, weight = strength of connection.', 'CONFIRMED'],
  ['Financial investigation indicators', 'Financial red flags in defence deals: company turnover inconsistent with contract size, recently incorporated company winning major contract, registered capital far below contract value, no audited accounts, multiple dormant subsidiaries, frequent name changes, registered in secrecy jurisdiction but operating elsewhere, unusual payment terms (advance payments >30%), letters of credit from sanctioned banks.', 'CONFIRMED'],
  ['Vessel and aircraft tracking', 'Defence logistics OSINT: AIS vessel tracking (MarineTraffic, VesselFinder) for arms shipments — watch for AIS gaps (transponder off = suspicious). ADS-B flight tracking (FlightRadar24, ADS-B Exchange) for military transport flights. Satellite imagery (Sentinel Hub, Google Earth) for military base changes, equipment deliveries. Port records for cargo manifests.', 'CONFIRMED'],
];
...
export function seedKnowledgeBase() {
  ...
  // Seed investigation methodology knowledge
  for (const [topic, content, confidence] of INVESTIGATION_FACTS) {
    storeFact(topic, content, 'investigation_methodology', confidence);
    count++;
  }
