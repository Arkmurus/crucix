...
import { getCompetitorContext } from './competitors.mjs';
import { getGTMContext } from './gtm_strategy.mjs';
import { recordConversation, recordThinkResponse } from './training_data.mjs';
...
5. COMMERCIAL REALISM — All recommendations must be operationally achievable. Arkmurus is a BROKER, not an OEM. We find the right supplier, assemble the deal, navigate compliance, and connect parties. Think: who supplies, who buys, why does this deal need US, and who partners with us to make it happen?
...
4. **SUPPLIER** — Which OEM(s) best fit? Can we assemble a multi-OEM package? Export compliance status?
5. **ARKMURUS VALUE-ADD** — WHY does this deal need a broker? What do we bring? (compliance nav, language bridge, offset management, multi-OEM assembly, Russian replacement expertise)
6. **PARTNERSHIP ANGLE** — Who should we partner with? (OEM teaming, local JV, agent, offset partner). For cold markets: which partner gets us into the room?
7. **COMPETITION** — Who else is chasing this? What's their angle? Can we partner with them instead of competing?
8. **DEAL ECONOMICS** — Contract value, commission potential (5-12%), financing needs, offset obligations
9. **COMPLIANCE** — Export licence requirements. Multi-jurisdiction check (UK/US/EU). End-user certificate.
10. **TIMELINE + WIN PROBABILITY** — Decision calendar. Relationship tier (INCUMBENT/ESTABLISHED/DEVELOPING/COLD). Realistic odds. What partnership would increase our chances?
...
function buildIntelContext(intelData) {
  ...
  // ACLED conflict data (previously missing — critical for procurement timing)
  const acled = intelData.acled;
  if (acled && (acled.totalEvents > 0 || acled.totalFatalities > 0)) {
    let acledStr = `CONFLICT DATA: ${acled.totalEvents || 0} events, ${acled.totalFatalities || 0} fatalities`;
    if (acled.topCountries) acledStr += ` | Top: ${acled.topCountries.slice(0, 5).map(c => c.country + '(' + c.events + ')').join(', ')}`;
    parts.push(acledStr);
  }

  // Export control alerts (previously missing)
  const ecIntel = intelData.exportControlIntel || intelData.exportControls;
  if (ecIntel?.updates?.length) {
    const critical = (ecIntel.updates || []).filter(u => u.severity === 'critical' || u.critical);
    if (critical.length) {
      parts.push(`EXPORT CONTROL ALERTS (${critical.length} critical):\n` +
        critical.slice(0, 3).map(u => `- ${u.title || u.text || ''}`).join('\n'));
    }
  }

  // Supply chain alerts (previously missing)
  const sc = intelData.supplyChain?.metrics;
  if (sc?.alerts?.length) {
    parts.push(`SUPPLY CHAIN ALERTS: ${sc.alerts.map(a => a.name + ': ' + a.severity).join(', ')}`);
  }

  // BD Brain weekly priority (previously missing — the brain's #1 recommendation)
  const brain = intelData.bdIntelligence?.brain;
  if (brain?.weeklyPriority?.action) {
    parts.push(`BRAIN TOP PRIORITY: ${brain.weeklyPriority.action} [${brain.weeklyPriority.market || ''}] — ${brain.weeklyPriority.whyNow || ''}`);
  }

  // BD Brain sales leads count
  if (brain?.salesLeads?.length) {
    parts.push(`BRAIN SALES LEADS (${brain.salesLeads.length}): ` +
      brain.salesLeads.slice(0, 3).map(l => `${l.market}: ${(l.lead || '').slice(0, 80)} [${l.urgency}]`).join(' | '));
  }
 ...
 */
// Trivial-question short-circuit — keep in sync with server.mjs trivialReply()
// and aria_service/intel/reasoning_library.trivial_reply(). Greetings /
// liveness probes / identity questions get a fixed reply with zero LLM cost.
// Past incident 2026-04-08: 'Aria, are you online?' was hitting this fallback
// and the LLM (steered by the heavy Lusophone system prompt + searchKnowledge
// returning Angola facts) returned the same Angola briefing for every greeting.
function _ariaTrivialReply(q) {
  ...
  // Background/status meta probes — keep in sync with the other 3 trivial copies.
...
export async function ariaChat(message, sessionId, llmProvider, intelData) {
  // Trivial-question short-circuit — runs before LLM check, before context
  // assembly, before knowledge lookup. Returns instantly with zero cost.
  const _trivial = _ariaTrivialReply(message);
  if (_trivial !== null) {
    return {
      response: _trivial,
  ...
  // Inject intelligence layers into prompt — budget-managed to ~3000 chars max
  const layers = [
    buildIntelContext(intelData),       // Layer 1: Live sweep data (highest priority)
    searchKnowledge(message),           // Layer 2: Knowledge base
    queryLedger(message),              // Layer 3: Intel ledger (30d)
    getContactContext(message),         // Layer 4: Contact intelligence
    getCompetitorContext(message),      // Layer 5: Competitor intelligence
    getApproachContext(message),        // Layer 6: Approach strategy
    getGTMContext(message),             // Layer 7: Go-to-market strategy
  ];
  // Cap total context to prevent blowing LLM token budget
  let totalContext = '';
  const MAX_CONTEXT_CHARS = 4500;
  for (const layer of layers) {
    if (!layer) continue;
    if (totalContext.length + layer.length > MAX_CONTEXT_CHARS) break;
    totalContext += layer;
  }
  const userPrompt = buildConversationPrompt(history, message, totalContext);
  ...
  try {
    ...
    // BUG-FIX 2026-04-12: instead of returning a dead error, assemble a
    // degraded-mode response from local knowledge layers. ARIA's knowledge
    // base, intel ledger, contacts, competitors, and approach data all work
    // without any LLM. This keeps ARIA functional when both Claude AND
    // DeepSeek hit rate limits.
    console.warn(`[ARIA] All LLMs failed (${err.message}) — assembling degraded-mode response from local knowledge`);
    degraded = true;
    const localParts = [];
    for (const layer of layers) {
      if (layer && layer.trim()) localParts.push(layer.trim());
    }
    if (localParts.length > 0) {
      responseText = `⚠️ _ARIA degraded mode — LLM unavailable, answering from local knowledge_\n\n`
        + `Here's what I found in my knowledge base for your query:\n\n`
        + localParts.join('\n\n').slice(0, 3000);
    } else {
      responseText = `⚠️ _ARIA degraded mode — LLM unavailable_\n\n`
        + `I couldn't reach any LLM provider right now, and I don't have relevant local knowledge for this query. `
        + `Try again in a minute, or use direct commands like /screen, /risk, /classify which work without an LLM.`;
    }
  }
  ...
  // Skip fact extraction in degraded mode — no LLM to verify
  if (!degraded) {
    try { autoExtractFacts(message, responseText); } catch {}
  }

  // Record to training dataset for future proprietary LLM fine-tuning
  try {
    const activeLayers = layers.map((l, i) => l ? ['intel','kb','ledger','contacts','competitors','approach','gtm'][i] : null).filter(Boolean);
    recordConversation(ARIA_SYSTEM_PROMPT, message, responseText, {
      hadIntelContext: !!intelData,
      contextLength: totalContext.length,
      layersInjected: activeLayers,
      degraded,
    });
  } catch {}
...
Please work through all 6 steps of the reasoning protocol in full.`;
  ...
  const result = parseThinkResponse(text, question, durationMs);

  // Record think mode for training (captures full reasoning chain)
  try { recordThinkResponse(question, { ...result, full_text: text }); } catch {}
