...
function buildConversationPrompt(history, currentMessage) {
 ...
 */
export async function ariaChat(message, sessionId, llmProvider) {
 ...
 * @param {import('../llm/provider.mjs').LLMProvider} llmProvider
 */
export async function ariaThink(question, context, llmProvider) {
  if (!llmProvider?.isConfigured) {
    return {
  ...
  const userPrompt = `Question for deep analysis: ${question}${contextStr}
