...
export async function ariaChat(message, sessionId, llmProvider, intelData) {
  ...
  const competitorContext = getCompetitorContext(message);      // Layer 6: Competitor intelligence
  const userPrompt = buildConversationPrompt(history, message,
    intelContext + knowledgeContext + ledgerContext + contactContext + approachContext + competitorContext);
