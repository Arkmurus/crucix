...
export function mountProactive(app) {
  ...
  app.post('/api/aria/proactive/daily', async (_req, res) => {
    const digest = await generateDailyDigest();
    if (digest) await sendTelegram(`📋 *ARIA DAILY DIGEST — ${dateStr()}*\n\n${digest}`);
    res.json({ ok: true, digest });
  });

  app.post('/api/aria/proactive/weekly', async (_req, res) => {
    const brief = await generateWeeklyBrief();
    if (brief) {
  ...
  app.post('/api/aria/proactive/battlecard', async (req, res) => {
    const { competitor } = req.body || {};
    const card = await generateCompetitorBattlecard(competitor);
    res.json({ ok: true, battlecard: card });
  });

  app.post('/api/aria/proactive/network-gap', async (_req, res) => {
    const report = await generateNetworkGapReport();
    res.json({ ok: true, report });
  });

  app.post('/api/aria/proactive/assumptions-audit', async (_req, res) => {
    const audit = await generateAssumptionsAudit();
    res.json({ ok: true, audit });
  });

  app.post('/api/aria/proactive/red-team', async (req, res) => {
    const { deal } = req.body || {};
  ...
  app.post('/api/aria/proactive/commentary', async (req, res) => {
  ...
  app.post('/api/aria/proactive/linkedin', async (req, res) => {
  ...
  app.post('/api/aria/proactive/deal-model', async (req, res) => {
    const { deal } = req.body || {};
  ...
  app.get('/api/aria/proactive/cultural-calendar', (_req, res) => {
    res.json({
  ...
  app.get('/api/aria/proactive/reengagement', async (_req, res) => {
    const nudges = await checkReengagement();
