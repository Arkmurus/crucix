...
export function mountProactive(app) {
  ...
  app.post('/api/aria/proactive/compliance-brief', requireAuth, async (_req, res) => {
    try {
      ...
      if (brief) {
        ...
        for (const email of TEAM_EMAILS) {
          try { await sendEmail({ to: email, subject: `Arkmurus Compliance Brief — ${dateStr()}`, text: brief }); } catch {}
        }
      }
      res.json({ ok: true, brief });
    } catch (e) { res.status(500).json({ error: e.message }); }
  ...
  console.log('[ARIA Proactive] Schedule: Daily 7am UTC | Weekly Mon 8am UTC | Compliance Wed 9am UTC | Monthly 1st 9am UTC | Sanctions q4h (02,06,10,14,18,22 UTC) | Hypotheses 4am UTC weekdays | Consolidation 3am UTC daily');
}
