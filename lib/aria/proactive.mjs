...
async function generateDailyEngagementPrompt() {
  ...
  try {
    ...
    const hypotheses = await brainGet('/api/aria/research/hypotheses').catch(() => null);

    // Try to find an unvalidated hypothesis to ask the team about
    const pending = (hypotheses?.hypotheses || []).filter(h =>
      h.status === 'pending' || h.status === 'unvalidated'
    );
    if (pending.length > 0) {
...
export function mountProactive(app) {
  ...
  console.log('[ARIA Proactive] Schedule: Daily 7am UTC | Engagement 8:30am UTC weekdays | Learning Dashboard Mon 8:15am UTC | Weekly Mon 8am UTC | Compliance Wed 9am UTC | Monthly 1st 9am UTC | Sanctions q4h (02,06,10,14,18,22 UTC) | Hypotheses 4am UTC weekdays | Consolidation 3am UTC daily');
}
