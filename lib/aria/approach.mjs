...
const MARKET_PROFILES = {
  ...
  'Senegal':        { language: 'French', formality: 'HIGH', greeting: 'Excellence', timezone: 'GMT', currency: 'XOF', approach: 'Former French colony — French language essential. Strong ECOWAS contributor. Nexter/Airbus historically preferred. Engage via Ministère des Forces Armées, Dakar.' },
  "Côte d'Ivoire":  { language: 'French', formality: 'HIGH', greeting: 'Excellence', timezone: 'GMT', currency: 'XOF', approach: 'Francophone market. ECOWAS rearmament active. French and Israeli OEMs historically strong. Engage via Ministère de la Défense, Abidjan.' },
  'Cameroon':       { language: 'French', formality: 'HIGH', greeting: 'Excellence', timezone: 'WAT (UTC+1)', currency: 'XAF', approach: 'Bilingual (French/English) but French dominant in military. BIR (Bataillon d\'Intervention Rapide) is key counter-terrorism unit. Active Boko Haram operations in north.' },
...
const OEM_BY_PRODUCT = {
  'armoured_vehicles': [
    ...
    { oem: 'Hanwha', country: 'KR', itar: false, price: 'MEDIUM', africa: 'LIMITED', products: 'K21 IFV, Redback — competitive in Asia/Europe' },
    { oem: 'Hyundai Rotem', country: 'KR', itar: false, price: 'MEDIUM', africa: 'LIMITED', products: 'K2 Black Panther — Poland, Egypt deals' },
  ],
  'uav_systems': [
  ...
  'helicopters': [
    ...
    { oem: 'Korea Aerospace (KAI)', country: 'KR', itar: false, price: 'MEDIUM', africa: 'LIMITED', products: 'KUH Surion, LAH — competitive pricing' },
  ],
  'radar_air_defence': [
...
function generateDraftMessage(market, product, profile, contacts) {
  ...
  if (profile.language === 'French') {
    return `Excellence,\n\nJ'ai l'honneur de m'adresser à vous au nom d'Arkmurus, société britannique spécialisée dans le conseil en défense et le courtage d'équipements militaires.\n\nNous avons pris connaissance de l'intérêt du ${authorityName} pour ${product} et souhaiterions présenter des solutions de fabricants européens et internationaux de premier plan, en pleine conformité avec les réglementations internationales de contrôle des exportations.\n\nNous sollicitons l'opportunité de présenter nos références et de discuter de la manière dont nous pouvons accompagner les besoins opérationnels de vos Forces Armées.\n\nDans l'attente de votre réponse, je vous prie d'agréer, Excellence, l'expression de ma très haute considération.\n\n[Nom]\nArkmurus Defence Advisory`;
  }

  if (profile.language === 'English/Arabic' || profile.language === 'Arabic') {
    return `Your Excellency,\n\nI have the honour to write to you on behalf of Arkmurus, a United Kingdom-based defence advisory and brokering firm with extensive experience in the Middle East and North Africa region.\n\nWe understand that the ${authorityName} of ${market} has requirements for ${product} and respectfully request the opportunity to present solutions from world-leading manufacturers, fully compliant with international export control regulations.\n\nArkmurus has established relationships with leading European, South African, and Asian defence manufacturers and offers comprehensive support including procurement advisory, export licensing, training coordination, and lifecycle sustainment.\n\nWe would be deeply honoured by the opportunity to present our credentials at your convenience.\n\nWith highest respect,\n[Name]\nArkmurus Defence Advisory`;
  }
...
export function getApproachContext(query) {
  ...
  // Try to extract market first — only trigger if a known market is mentioned
  ...
  const triggers = ['approach', 'contact', 'sell to', 'pitch', 'proposal', 'offer', 'engage', 'reach out', 'opening', 'introduce', 'how do we', 'strategy for'];
