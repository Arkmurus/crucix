...
export function ingestSweepSignals(currentData) {
  ...
  const existingTexts = new Set(ledger.signals.map(s => s.text.substring(0, 150).toLowerCase()));

  for (const sig of rawSignals) {
    if (!sig.text || sig.text.length < 20) continue;
    const textKey = sig.text.substring(0, 150).toLowerCase();
...
export function queryLedger(query) {
  ...
  // Find matching signals — pre-lowercase countries for performance
  const matches = [];
  for (const sig of ledger.signals) {
    let score = 0;
    // Check countries first (cheapest check, highest value)
    ...
    // Only do expensive text scan if no entity match yet
    if (score === 0) {
      const textLower = sig.text.toLowerCase();
      for (const w of words) { if (textLower.includes(w)) { score += 2; break; } }
    }
