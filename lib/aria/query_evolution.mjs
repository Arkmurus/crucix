...
// R-F387: dropped Upstash dual-write — state lives in EVOLUTION_FILE only.
...
function seedInitialPopulation() {
  ...
  for (let i = 0; i < GENERATION_SIZE; i++) {
    ...
    state.queries.push({
      query: `${market} ${product} ${action} ${new Date().getFullYear()}`,
      fitness: 0,
      hits: 0,
      misses: 0,
      lastRun: null,
...
function mutateQuery(parentQuery) {
  ...
  if (r < 0.25) {
    ...
    const market = MARKETS[Math.floor(Math.random() * MARKETS.length)];
    const action = ACTIONS[Math.floor(Math.random() * ACTIONS.length)];
    return `${market} ${action} ${new Date().getFullYear()}`;
  }
