...
// R-F387: dropped Upstash dual-write — training meta lives in META_FILE only.
...
export function recordConversation(systemPrompt, userMessage, ariaResponse, metadata = {}) {
  ...
  const example = {
    ...
    // Rich metadata for training quality (GAP 1.1 + 1.3 fix)
    meta: {
      ...
      layersInjected: metadata.layersInjected || [],
      confidenceTags: (ariaResponse.match(/\[(CONFIRMED|PROBABLE|ASSESSED|UNCERTAIN|SPECULATIVE)\]/g) || []),
...
export function recordBrainAssessment(context, brainOutput) {
  ...
  const example = {
    messages: [
      { role: 'system', content: 'You are ARIA, the autonomous BD intelligence brain. Produce structured strategic assessments.' },
...
// ── Record User Correction (GAP 1.2 fix) ─────────────────────────────────────
// When user says "ARIA was wrong about X, the correct answer is Y"
export function recordCorrection(originalQuery, originalResponse, correction, correctAnswer) {
  const example = {
    messages: [
      { role: 'system', content: 'You are ARIA. You previously gave an incorrect answer. Learn the correction.' },
      { role: 'user', content: `Original question: ${originalQuery}\n\nYour previous answer was wrong: ${originalResponse?.substring(0, 500)}\n\nCorrection: ${correction}` },
      { role: 'assistant', content: correctAnswer || `Acknowledged. ${correction}. I will incorporate this correction into future responses.` },
    ],
    meta: {
      ts: new Date().toISOString(),
      type: 'correction',
      correction: correction.substring(0, 300),
  ...
  appendLine(CONVERSATIONS_FILE, example);
  const meta = loadMeta();
  meta.corrections = (meta.corrections || 0) + 1;
  saveMeta(meta);
}

// ── Record Think Mode Response (GAP 1.4 fix) ──────────────────────────────────
// Captures the full 6-step reasoning chain for structured training
export function recordThinkResponse(question, thinkOutput) {
  if (!thinkOutput) return;
  const example = {
    messages: [
      { role: 'system', content: 'You are ARIA performing deep 6-step intelligence analysis. Structure your response with ORIENTATION, INVENTORY, REASONING, CHALLENGE, CONCLUSION, ACTION sections.' },
      { role: 'user', content: question.substring(0, 1000) },
      { role: 'assistant', content: thinkOutput.full_text?.substring(0, 4000) || JSON.stringify(thinkOutput).substring(0, 4000) },
    ],
    meta: {
      ts: new Date().toISOString(),
      type: 'think_mode',
      hasOrientation: !!thinkOutput.orientation,
      hasReasoning: !!thinkOutput.reasoning,
      confidence: thinkOutput.conclusion?.confidence || null,
      selfGrade: thinkOutput.metacognition?.self_grade || null,
      durationMs: thinkOutput.duration_ms || null,
    },
  };

  appendLine(CONVERSATIONS_FILE, example);
  const meta = loadMeta();
  meta.thinkResponses = (meta.thinkResponses || 0) + 1;
  saveMeta(meta);
}
