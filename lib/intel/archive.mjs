...
// Historical run archive — permanent store + 7-day trend detection
// Enables: "what changed this week", backtesting, slow-burn escalation detection.
// MAX_DAYS was 30d rolling before 2026-04-21 — operator asked for forever.
// At ~500B per summary × 1 run/day, 100 years ≈ 18 MB on disk. Safe.

import { readFileSync, writeFileSync, existsSync, mkdirSync, readdirSync, unlinkSync } from 'fs';
import { join } from 'path';

const ARCHIVE_DIR     = join(process.cwd(), 'runs', 'archive');
const MAX_DAYS        = 36500; // 100 years — effectively permanent
...
function pruneOldRuns() {
  try {
    ...
    for (const f of files) {
      ...
      try {
        const dateStr = f.replace('run_', '').replace('.json', '');
        const isoStr  = dateStr.replace(/T(\d{2})-(\d{2})-(\d{2})/, 'T$1:$2:$3');
        if (new Date(isoStr) < new Date(cutoff)) {
          unlinkSync(path);
        }
      } catch {}
    }
