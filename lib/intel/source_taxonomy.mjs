...
// Declarative source classification — maps every sweep-side `_src` tag to
// (class, appropriate_uses). As of 2026-04-26 the (tag → class) mapping
// is owned by source_registry.mjs (populated from
// source_registry_bootstrap.mjs at module load); this file owns the
// class definitions and the (region → use-case) gate logic. The
// `SOURCE_TAG_TO_CLASS` table below is kept as a fallback for
// pre-registry callers and as a safety net for tests that reset the
// registry — it is no longer the source of truth.
//
// Why this exists:
...
import { getSourceClassForTag } from './source_registry.mjs';

/** Look up the class string for a `_src` tag.
 *
 * Resolution order: registry (populated by source_registry_bootstrap.mjs)
 * first, hardcoded SOURCE_TAG_TO_CLASS table second, default to
 * 'osint_aggregator' (mixed-quality assumption).
 */
export function classifySource(srcTag) {
  if (!srcTag) return 'osint_aggregator';
  const fromRegistry = getSourceClassForTag(srcTag);
