 ...
 * compact=true → directory listing row only
 * compact=false → full profile card for browse/search view
 */
export function formatOEMCard(oem, compact = false) {
  const natoTag    = oem.nato ? '🔵 NATO' : '🟡 Non-NATO';
  const scaleEmoji = { prime: '🏭', tier1: '🔩', tier2: '⚙️', specialist: '🎯' }[oem.scale] || '🔩';
  const scaleLabel = { prime: 'Prime', tier1: 'Tier 1', tier2: 'Tier 2', specialist: 'Specialist' }[oem.scale] || '';

  if (compact) {
    let msg = `${scaleEmoji} ${oem.name}\n`;
    msg += `   ${oem.products.slice(0, 3).join(' · ')}\n`;
    return msg;
  }

  // ── Full profile card ──────────────────────────────────────────
  let msg = `━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  msg += `${scaleEmoji} *${oem.name}*\n`;
  msg += `📍 ${oem.country}  ${natoTag}  ${scaleLabel}`;
  if (oem.parentGroup && oem.parentGroup !== oem.name) msg += `\n_${oem.parentGroup}_`;
  msg += '\n\n';

  msg += `🔧 *Products:* ${oem.products.slice(0, 5).join(', ')}`;
  if (oem.products.length > 5) msg += ` _(+${oem.products.length - 5} more)_`;
  msg += `\n🎯 *Calibres:* ${oem.calibres.join(', ')}\n\n`;

  msg += `💡 ${oem.usp}\n`;
  if (oem.description) msg += `\n${oem.description}\n`;

  msg += `\n🌐 ${oem.website}\n`;
  if (oem.contact?.general) msg += `📞 ${oem.contact.general}\n`;
  if (oem.contact?.email) msg += `✉️ ${oem.contact.email}\n`;
...
export function formatOEMList(oems, title = 'OEM DIRECTORY') {
  ...
  msg += `_/oem [query] · /oem [country] · /oem [calibre] — tap Browse below for full cards_`;
  return msg;
}
