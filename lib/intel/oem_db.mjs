...
export function formatOEMCard(oem, compact = false) {
  ...
  const nato     = oem.nato ? '✅ NATO' : '⚠️ Non-NATO';
...
export function formatOEMList(oems, title = 'OEM DIRECTORY') {
  ...
  msg += `_${ts}  ·  ${oems.length} manufacturers_\n`;
  msg += '━━━━━━━━━━━━━━━━━━━━━━━━\n\n';

  for (const [country, list] of Object.entries(groups).sort()) {
    const flag = FLAG[country] || '🌍';
    const natoMark = list[0].nato ? '✅' : '⚠️';
    msg += `${flag} *${country}*  ${natoMark}  (${list.length})\n`;
    for (const oem of list) {
      const ico = SCALE_EMOJI[oem.scale] || '⚙️';
      msg += `  ${ico} ${oem.name}  ·  ${oem.products.slice(0, 2).join(' · ')}\n`;
    }
    msg += '\n';
  }

  msg += `_/oem [country] · /oem [calibre] · /oem [product] — tap Browse for full cards_`;
  return msg;
}
