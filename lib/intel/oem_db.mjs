...
/**
 * Format a compact card for a single OEM (for Telegram).
 */
export function formatOEMCard(oem, compact = false) {
  ...
  msg += `📍 ${oem.country} · ${natoTag}`;
  if (oem.parentGroup && oem.parentGroup !== oem.name) msg += ` · ${oem.parentGroup}`;
  msg += '\n';

  if (!compact) {
    msg += `\n*Products:* ${oem.products.slice(0, 4).join(', ')}${oem.products.length > 4 ? ' (+more)' : ''}\n`;
    msg += `*Calibres:* ${oem.calibres.join(', ')}\n`;
    msg += `\n*USP:* ${oem.usp}\n`;
    msg += `\n🌐 ${oem.website}\n`;
    if (oem.contact?.general) msg += `📞 ${oem.contact.general}\n`;
    if (oem.contact?.email) msg += `✉️ ${oem.contact.email}\n`;
    if (oem.exportContact) msg += `📤 Export: ${oem.exportContact}\n`;
  } else {
