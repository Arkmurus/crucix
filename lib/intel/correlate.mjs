...
const _COMMERCIAL_DEFENCE_SOURCE_TAGS = new Set([
  'defence_news', 'arms_transfer', 'procurement', 'export_control',
]);
const _GEOPOLITICAL_CONTENT_KEYWORDS = [
  'hormuz', 'strait', 'blockade', 'embargo', 'ceasefire', 'truce',
  'escalation', 'sanctions', 'evacuation', 'airstrike', 'missile strike',
  'assassination', 'invasion', 'war powers', 'declaration of war',
  'red line', 'casus belli', 'naval blockade',
];
function sourceQualityMultiplier(signal, srcTag) {
  if (!_COMMERCIAL_DEFENCE_SOURCE_TAGS.has(srcTag)) return 1;
  const text = [signal.text, signal.title, signal.headline, signal.content, signal.summary]
    .filter(Boolean).join(' ').toLowerCase();
  if (!text) return 1;
  const isGeopolitical = _GEOPOLITICAL_CONTENT_KEYWORDS.some(kw => text.includes(kw));
  return isGeopolitical ? 0.5 : 1;
}
...
export function correlate(data) {
  ...
  for (const signal of allSignals) {
    ...
    // Source-quality gate: halve score for commercial-defence-feed signals
    // making geopolitical claims (Damen-feed elevation issue, 2026-04-25).
    const qMult = sourceQualityMultiplier(signal, signal._src || '');
    const combinedMult = rMult * qMult;
    const adjustedScore = combinedMult < 1
      ? Math.round(scored.score * combinedMult)
      : scored.score;
    ...
    for (const region of regions) {
      ...
      correlations[region].signals.push({ ...signal, ...scored, _dampened: dampened });
    ...
    return {
      ...
      topSignals:  c.signals
        ...
        .map(s => {
          ...
          // so it doesn't elevate single-source geopolitical claims from
          // commercial defence feeds to CRITICAL editorial classification.
          const sourceLabel = s._dampened
            ? `${s._src} (commercial defence feed; geopolitical content downweighted — treat as MARKET-VIEW commentary, not primary geopolitical reporting)`
            : s._src;
          return {
