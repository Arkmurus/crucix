...
const EMAIL_FROM   = process.env.EMAIL_FROM ||
  (_ARIA_FALLBACK && EMAIL_USER
    ? `Imaria Intelligence <${EMAIL_USER}>`
    : 'Imaria Intelligence <noreply@imaria.io>');
        ...
        <tr>
          <td style="background:${BG_PANEL};padding:20px 36px;">
            <p style="margin:0 0 6px;font-size:11px;color:${TEXT_DIM};text-align:center;line-height:1.7;">
              This message was sent by the Imaria Intelligence Platform. Do not reply to this email.<br/>
              If you did not request this, you may safely disregard it.
            </p>
            <p style="margin:0;font-size:10px;color:${TEXT_DIM};text-align:center;">
              © ${new Date().getFullYear()} Imaria · All Rights Reserved
            </p>
          </td>
        </tr>
...
export async function sendVerificationEmail(email, fullName, code) {
  ...
  const body = `
    <div style="max-width:480px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
      <div style="background:linear-gradient(135deg,#7c3aed,#4f46e5);border-radius:12px 12px 0 0;padding:32px;text-align:center;">
        <h1 style="margin:0;font-size:20px;font-weight:700;color:#fff;">Imaria</h1>
        <p style="margin:8px 0 0;font-size:13px;color:rgba(255,255,255,0.8);">Verify your email address</p>
      </div>
      <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;padding:32px;">
        <p style="margin:0 0 16px;font-size:14px;color:#1f2937;line-height:1.6;">
          Hi <strong>${name}</strong>,
        </p>
        <p style="margin:0 0 20px;font-size:14px;color:#4b5563;line-height:1.6;">
          Please use the following security code to verify your Imaria account:
        </p>
        ...
        <p style="margin:0 0 12px;font-size:12px;color:#6b7280;line-height:1.5;">
          Only enter this code on the official Imaria website or app.
          <strong>Never share it with anyone.</strong> We will never ask for it outside an official platform.
        </p>
        <p style="margin:0 0 12px;font-size:12px;color:#6b7280;line-height:1.5;">
        ...
        <p style="margin:0;font-size:11px;color:#9ca3af;line-height:1.5;">
          Imaria Intelligence<br>
          <a href="https://imaria.io" style="color:#7c3aed;text-decoration:none;">imaria.io</a>
        </p>
      </div>
    </div>`;

  if (!isConfigured) console.log(`[EMAIL] Verification code for ${email}: ${code}`);
  return sendMail(email, 'Security code for your Imaria account', wrapHtml('Verify Your Email', body));
}
...
export async function sendVerificationSuccessEmail(email, fullName) {
  ...
  const body = `
    <div style="max-width:480px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
      <div style="background:linear-gradient(135deg,#059669,#10b981);border-radius:12px 12px 0 0;padding:32px;text-align:center;">
        <h1 style="margin:0;font-size:20px;font-weight:700;color:#fff;">Email Verified</h1>
        <p style="margin:8px 0 0;font-size:13px;color:rgba(255,255,255,0.8);">Your email has been confirmed</p>
      </div>
      <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;padding:32px;">
        <p style="margin:0 0 16px;font-size:14px;color:#1f2937;line-height:1.6;">
          Hi <strong>${name}</strong>,
        ...
        <p style="margin:0 0 20px;font-size:14px;color:#4b5563;line-height:1.6;">
          ...
          administrator approval. You will receive a notification once your access is activated.
        </p>
        <p style="margin:0 0 20px;font-size:14px;color:#4b5563;line-height:1.6;">
  ...
  return sendMail(email, 'Email verified — Imaria', wrapHtml('Email Verified', body));
}
...
export async function sendPendingApprovalEmail(email, fullName) {
  ...
  const body = `
    ...
    <p style="margin:0 0 20px;font-size:14px;color:${TEXT_MUTED};line-height:1.75;">
      Thank you for your interest in the <strong style="color:${TEXT_MAIN}">Imaria Intelligence Platform</strong>.
  ...
  return sendMail(
    email,
    'Imaria — Your Access Request is Under Review',
...
export async function sendWelcomeEmail(email, fullName) {
  ...
  const body = `
    ...
    <p style="margin:0 0 24px;font-size:14px;color:${TEXT_MUTED};line-height:1.75;">
      ...
      <strong style="color:${TEXT_MAIN}">Imaria Intelligence Platform</strong>
  ...
  return sendMail(
    email,
    'Imaria — Your Account Has Been Approved',
...
export async function sendPasswordResetEmail(email, fullName, code) {
  ...
  const body = `
    ...
    <p style="margin:0 0 24px;font-size:14px;color:${TEXT_MUTED};line-height:1.6;">
      ...
      Imaria Intelligence Platform password. Use the code below to set a new password:
    </p>
    ${codeBox(code)}
  ...
  return sendMail(email, 'Imaria — Password Reset Request', wrapHtml('Password Reset', body));
}
...
export async function sendPasswordChangedNotification(email, fullName, requestIp = '') {
  ...
  const body = `
    ...
    <p style="margin:0 0 18px;font-size:14px;color:${TEXT_MUTED};line-height:1.6;">
      ...
      Imaria Intelligence Platform account was reset successfully a moment ago.
  ...
  return sendMail(email, 'Imaria — Your password was just changed', wrapHtml('Password changed', body));
}
...
export async function sendSuspensionEmail(email, fullName) {
  ...
  const body = `
    ...
    <p style="margin:0 0 20px;font-size:14px;color:${TEXT_MUTED};line-height:1.75;">
      Your Imaria Intelligence Platform account has been temporarily suspended by an administrator.
  ...
  return sendMail(email, 'Imaria — Account Suspended', wrapHtml('Account Suspended', body));
}
...
export async function sendReactivationEmail(email, fullName) {
  ...
  const body = `
    ...
    <p style="margin:0 0 24px;font-size:14px;color:${TEXT_MUTED};line-height:1.75;">
      Your Imaria Intelligence Platform account has been reactivated.
      You can now sign in and access all platform features.
    </p>
    ${ctaButton('Sign In to Your Dashboard', loginUrl)}`;

  return sendMail(email, 'Imaria — Account Reactivated', wrapHtml('Account Reactivated', body));
}
