...
// R-F2032 (2026-06-27): also read the SMTP_USER / SMTP_PASS namespace. The
// live aria-web secrets hold the working mailbox credentials under SMTP_USER /
// SMTP_PASS, but this module only read EMAIL_* → ARIA_EMAIL_* — so auth failed
// with "Invalid login: wrong user/password" against ox.livemail.co.uk while the
// correct creds sat unread. SMTP_* is a purely-additive MIDDLE priority: checked
// after the dedicated EMAIL_* vars but before the ARIA inbound-bridge fallback,
// so it can never override a working EMAIL_* config. Host/port are left as-is —
// the failure was auth-only (host/TLS already connect), so SMTP_* user/pass is
// the surgical fix; do not widen to host/port without re-verifying the live env.
const EMAIL_HOST   = process.env.EMAIL_HOST || process.env.ARIA_SMTP_HOST || process.env.ARIA_EMAIL_HOST;
const EMAIL_USER   = process.env.EMAIL_USER || process.env.SMTP_USER || (_ARIA_FALLBACK ? process.env.ARIA_EMAIL_USER : undefined);
const EMAIL_PASS   = process.env.EMAIL_PASS || process.env.SMTP_PASS || (_ARIA_FALLBACK ? process.env.ARIA_EMAIL_PASS : undefined);
...
const APP_URL      = process.env.APP_URL || 'https://intel.arkmurus.com';
  ...
  const body = `
    <div style="max-width:480px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
      <div style="background:linear-gradient(135deg,#7c3aed,#4f46e5);border-radius:12px 12px 0 0;padding:32px;text-align:center;">
        <h1 style="margin:0;font-size:20px;font-weight:700;color:#fff;">Arkmurus</h1>
      ...
      <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;padding:32px;">
        ...
        <p style="margin:0 0 20px;font-size:14px;color:#4b5563;line-height:1.6;">
          Please use the following security code to verify your Arkmurus account:
        </p>
        ...
        <p style="margin:0 0 12px;font-size:12px;color:#6b7280;line-height:1.5;">
          Only enter this code on the official Arkmurus website or app.
        ...
        <p style="margin:0;font-size:11px;color:#9ca3af;line-height:1.5;">
          Arkmurus Research Intelligence<br>
          <a href="https://arkmurus.com" style="color:#7c3aed;text-decoration:none;">arkmurus.com</a>
        </p>
      </div>
    </div>`;

  if (!isConfigured) console.log(`[EMAIL] Verification code for ${email}: ${code}`);
  return sendMail(email, 'Security code for your Arkmurus account', wrapHtml('Verify Your Email', body));
}
...
export async function sendVerificationSuccessEmail(email, fullName) {
  ...
  return sendMail(email, 'Email verified — Arkmurus', wrapHtml('Email Verified', body));
}
