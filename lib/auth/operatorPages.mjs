...
//                 clear-all, user management, and platform internals).
//
...
/** Customer tools that require a login but no privileged operator role. */
export const AUTHENTICATED_PAGES = Object.freeze([
  // R-F3559 — pairing is owner-scoped by the WA proxy/listener and capped at one
  // connection per owner. Making the page admin-only prevented ordinary users
  // from reaching their own isolated connection manager.
...
export function operatorPageFor(route) {
  ...
  const authenticated = AUTHENTICATED_PAGES.find(([r]) => pageKey(r) === clean);
  if (authenticated) return { file: authenticated[1], roles: ['viewer', 'support', 'poweruser', 'admin'] };
