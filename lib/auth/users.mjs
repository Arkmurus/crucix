...
// USERS_FILE is overridable via USERS_FILE_OVERRIDE so capability tests can
// point a child Node process at a tmp users.json without colliding with the
// running app's real store. Production deploys leave the env var unset.
const USERS_FILE = process.env.USERS_FILE_OVERRIDE || join(RUNS_DIR, 'users.json');
