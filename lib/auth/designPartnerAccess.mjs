...
//     one being fixed.
//
//  2. The caller is TOLD what happened to the email. Live SMTP on aria-web
//     cannot authenticate — EMAIL_USER and EMAIL_PASS hold the same value
//     (identical fly secret digests 49bb8a67b557e235), which R-F3289 made
//     report honestly as isConfigured=false, so sendMail returns
//     {sent:false} and logs to stdout. A fix that only emailed the credential
//     would have produced a second silent nothing for Ray. So the send
//     OUTCOME is returned, and the temp password comes back to the operator
//     once so they can deliver it by hand (CLAUDE.md §19e, §22).
