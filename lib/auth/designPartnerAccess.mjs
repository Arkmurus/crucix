...
export async function provisionDesignPartnerAccess(entry, deps = {}) {
  ...
  try {
    ...
    updateUser(created.id, {
      ...
      // R-F3332 — this password was generated by us, shown on an admin screen
      // and emailed in clear, so it is known to at least one other party by
      // construction. The account does nothing but rotate it until they do
      // (enforced in requireAuth via lib/auth/passwordRotation.mjs).
