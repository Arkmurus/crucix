...
// R-F3167 — paths where the tenant is pinned for EVERYONE, admin included.
//
// The see-all exemption above is right for DD: an operator legitimately reviews
// every company report. It is wrong for vetting, which by design has NO admin
// see-all (see routes/vetting.py) — a vetting case holds criminal-conviction
// data about a named individual, so a wildcard reached by accident is
// categorically worse than for a company report.
//
// The live symptom that exposed this: an admin's request carried no `user_id`
// at all, so the brain's deliberately-strict tenant check refused it and
// /vetting.html reported "Could not load cases (HTTP 400)". Admin is still an
// IDENTITY — pinning gives them their own tenant, which is the correct answer,
// not a lesser one.
const ALWAYS_PINNED_PREFIXES = ['/api/aria/vetting'];

function alwaysPinned(originalUrl) {
  const path = String(originalUrl || '').split('?')[0].toLowerCase();
  return ALWAYS_PINNED_PREFIXES.some(
    (p) => path === p || path.startsWith(`${p}/`),
  );
}

export function pinNonAdminUserId(originalUrl, user) {
  if (isPrivileged(user) && !alwaysPinned(originalUrl)) return originalUrl;
  // For an always-pinned path an admin is pinned to their OWN id, same as
  // anyone else. `user.id` covers the internal-token shape (`aria-internal`),
  // which otherwise has no userId and would pin to '' — refused downstream.
  const uid = (user && (user.userId || user.id)) || '';
  try {
