...
// Supported languages. Portuguese (pt) is REQUIRED for the Lusophone
// Africa moat (Angola / Moçambique / Cabo Verde / Guiné-Bissau) and
// Brazil. Arabic (ar) covers the Gulf market. The locale files may
// initially be English placeholders for any strings not yet translated —
// the LLM side uses _detect_language_hint in aria_engine.py so user
// messages already get replies in their written language. The UI
// fallback is English, not a crash.
const SUPPORTED_LOCALES = ['en', 'fr', 'pt', 'ar'];
...
function loadLocale(lang) {
  ...
  if (!existsSync(localePath)) {
    // R-F1370: BASE CASE. When the DEFAULT locale file is itself missing
    // (the locales/ dir has never existed in the repo or the deployed image),
    // the old "fall back to en" path called loadLocale('en') from
    // loadLocale('en') — infinite recursion — RangeError — and /api/locales
    // answered 500 in production. English strings are built-in fallbacks
    // everywhere (t() returns the key, getLLMPrompt() returns ''), so an
    // empty locale object is the designed degraded mode, not an error.
    if (lang === DEFAULT_LOCALE) {
      console.warn(`[i18n] Default locale file missing: ${localePath} — using built-in fallbacks`);
      const empty = {};
      localeCache.set(lang, empty); // cache so we warn once, not per call
      return empty;
    }
