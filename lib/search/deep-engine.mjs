...
async function webSearch(query, limit = 6) {
  // R-F373 (2026-05-12): Brave Search removed (independence directive).
  // Pre-fix this function would call Brave if BRAVE_API_KEY was set, otherwise
  // return []. Now always returns []. Callers either fall back to other
  // searchers or accept the empty result. This was already the dominant
  // production path (audit found no fallback was wired here — webSearch
  // simply returned [] on Brave failures), so removing it is a no-op for
  // most code paths but removes the dependency surface entirely.
  return [];
}
