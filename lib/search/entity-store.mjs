...
function save() {
  try {
    ...
    redisSet('crucix:entity_graph', _store).catch(() => {});
  } catch (e) {
    console.warn('[EntityStore] Save failed:', e.message);
  }
}

export async function initEntityStore() {
  if (existsSync(STORE_PATH)) return;
  const remote = await redisGet('crucix:entity_graph');
  if (remote !== null && remote.version === 2) {
    _store = remote;
    try {
      mkdirSync(dirname(STORE_PATH), { recursive: true });
      writeFileSync(STORE_PATH, JSON.stringify(_store, null, 2));
      console.log('[Persist] Restored entity graph from Redis');
    } catch {}
  }
}
