...
import { brainAbsorb } from './learning_store.mjs';
import { shouldSkip, recordFailure, recordSuccess } from '../util/throttle.mjs';

const BRAIN_URL          = process.env.BRAIN_SERVICE_URL || 'http://localhost:5001';

// R-F3655 — EVERY brain call in this module was unauthenticated.
// Live evidence (15-cycle log sweep, 2026-08-03): repeated
// `GET /api/aria/curiosity HTTP/1.1" 401 Unauthorized` from an internal
// fdaa: 6PN address. Probed directly: /api/aria/curiosity and
// /api/aria/identity both return 401 without a Bearer token.
//
// The consequence was NOT a noisy failure — it was silence. The 401 threw,
// the catch called recordFailure(_BRAIN_CIRCUIT), and after 2 consecutive
// failures the circuit OPENED, after which every tick logged
// "Brain circuit open — skipping run". So a permanent AUTH bug presented as
// an intermittently unreachable brain (§22: the log named the wrong cause),
// and ARIA's whole curiosity-exploration loop has been dead — never fetching
// threads, never calling /think, never resolving a thread, never signalling.
//
// Token chain mirrors lib/self/learning_store.mjs:420, the working sibling in
// this directory. Both vars carry the same value in production (§18).
const BRAIN_TOKEN = (
...
/** fetch() against the brain with auth attached. Use this for EVERY brain call
 *  in this module so a new call site cannot silently forget the token. */
function brainFetch(path, opts = {}) {
  const headers = { ...(opts.headers || {}) };
  if (BRAIN_TOKEN) headers['Authorization'] = `Bearer ${BRAIN_TOKEN}`;
  return fetch(`${BRAIN_URL}${path}`, { ...opts, headers });
}
...
// R-F23 2026-05-01: brain-fetch failures cluster around fly.io
// aria-intel slowdowns (anthropic-billing 400 races, redeploys, etc).
// Without a circuit breaker, every 3h tick logs `Could not fetch
// curiosity threads: fetch failed` even when the brain has been
// unreachable for hours. Skip after 2 consecutive failures for 30 min.
const _BRAIN_CIRCUIT = 'explorer:brain_fetch';
const _BRAIN_CIRCUIT_OPTS = { failureThreshold: 2, cooldownMs: 30 * 60 * 1000 };
...
async function runCuriosityExploration(redis, notifyFn) {
  ...
  // R-F23: short-circuit if brain has been unreachable on recent ticks.
  if (shouldSkip(_BRAIN_CIRCUIT, _BRAIN_CIRCUIT_OPTS)) {
    console.log('[ExplorerScheduler] Brain circuit open — skipping run, will retry after cooldown');
    return;
  }
  let threads = [];
  try {
    const resp = await brainFetch('/api/aria/curiosity', { signal: AbortSignal.timeout(10000) });
    if (!resp.ok) throw new Error(`Brain returned ${resp.status}`);
    const data = await resp.json();
    threads    = (data.open_threads || []).filter(t => !t.resolved);
    recordSuccess(_BRAIN_CIRCUIT);
  } catch (e) {
    recordFailure(_BRAIN_CIRCUIT, _BRAIN_CIRCUIT_OPTS);
...
async function generateQueriesForThread(thread) {
  ...
  try {
    const resp = await brainFetch('/api/aria/think', {
      method:  'POST',
...
async function resolveThread(question, answer) {
  try {
    await brainFetch('/api/aria/curiosity/resolve', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function storeToARIAMemory(question, answer, findings) {
  try {
    await brainFetch('/api/aria/brain/signal', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({
    ...
    // Module-specific brain absorb
    brainAbsorb({
      module: 'osint_sweep',
      summary: `Explorer finding: ${question.slice(0, 100)} → ${findings.length} sources, ${answer.length} chars`,
      success: true,
      confidence: 'ASSESSED',
    });
  } catch (e) {
...
export async function triggerReactiveExploration(acledEvent, redis, notifyFn) {
  ...
  try {
    ...
    await brainFetch('/api/aria/think', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({
