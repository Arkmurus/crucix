...
import { logSelfUpdate, getSourcesToReview } from './learning_store.mjs';
...
function gitCommit(message) {
  try {
    ...
    // R-F1867 (audit DD-11): pass the message as a spawnSync argument array,
    // NOT an execSync template literal. The old form escaped only quotes +
    // newlines, so shell metacharacters (backticks, $(), ;, |) in `message`
    // — which carries a caller-controllable moduleName — executed as shell.
    // An argument array never invokes a shell, so injection is impossible.
    const commit = spawnSync('git', ['commit', '-m', String(message)], {
      cwd: ROOT, stdio: 'pipe', timeout: 15000, encoding: 'utf8',
    });
    if (commit.status !== 0) {
      throw new Error(`git commit exited ${commit.status}: ${(commit.stderr || '').slice(0, 200)}`);
    }
    console.log('[Updater] Git commit OK:', String(message).substring(0, 60));
    return true;
  } catch (err) {
...
// ── Test a staged module by actually running briefing() ──────────────────────
// Writes staged code to a temp path, spawns a child Node process, validates output schema.
// Returns { ok, updates, signals, ms } or { ok: false, error }
export function testStagedModule(moduleName, timeoutMs = 45000) {
  ensureDirs();
  const stagePath = join(STAGING_DIR, `${moduleName}.mjs.staged`);
  if (!existsSync(stagePath)) return { ok: false, error: `No staged file: ${moduleName}` };

  const code     = readFileSync(stagePath, 'utf8');
  const testPath = join(STAGING_DIR, `${moduleName}.mjs.testrun`);
  writeFileSync(testPath, code, 'utf8');

  const fileUrl  = 'file://' + testPath.replace(/\\/g, '/');
  const script   = `
import { briefing } from ${JSON.stringify(fileUrl)};
const t = Date.now();
try {
  const r = await briefing();
  if (!r || typeof r !== 'object') throw new Error('briefing() must return an object');
  process.stdout.write(JSON.stringify({ ok: true, updates: (r.updates||[]).length, signals: (r.signals||[]).length, ms: Date.now()-t }));
} catch(e) {
  process.stdout.write(JSON.stringify({ ok: false, error: (e.message||'unknown').substring(0,200) }));
}`.trim();
  ...
  try {
    ...
    return out.ok !== undefined ? out : { ok: false, error: result.stderr?.substring(0, 300) || 'parse error' };
  } catch {
    return { ok: false, error: (result.stderr || result.error?.message || 'unknown').substring(0, 300) };
  }
}

// ── Auto-deploy all staged modules that pass testing ──────────────────────────
// Safe: tests each module in isolation before writing to sources/
export async function autoDeployStaged() {
  ensureDirs();
  const stagedFiles = existsSync(STAGING_DIR)
    ? readdirSync(STAGING_DIR).filter(f => f.endsWith('.mjs.staged')).map(f => f.replace('.mjs.staged', ''))
    : [];

  if (stagedFiles.length === 0) return [];

  const results = [];
  for (const moduleName of stagedFiles) {
    console.log(`[Updater] Auto-deploy: testing ${moduleName}...`);
    const testResult = testStagedModule(moduleName);

    if (!testResult.ok) {
      console.log(`[Updater] Auto-deploy skip ${moduleName}: ${testResult.error}`);
      logSelfUpdate('auto_deploy_skip', { moduleName, reason: testResult.error });
      results.push({ moduleName, deployed: false, reason: testResult.error });
      continue;
    }

    console.log(`[Updater] Auto-deploying ${moduleName} — test passed (${testResult.updates} updates, ${testResult.ms}ms)`);
    const deployResult = await deployModule(moduleName);
    logSelfUpdate('auto_deploy', { moduleName, testResult, success: deployResult.success });
    results.push({ moduleName, deployed: deployResult.success, testResult, deployResult });
  }

  return results;
}

// ── Auto-disable persistently failing sources ─────────────────────────────────
// Only fires when a source has ≥ minSweeps recorded AND fail rate ≥ maxFailRate
export function autoDisableDegradedSources(maxFailRate = 0.90, minSweeps = 20) {
  const criticals = getSourcesToReview().filter(s => s.status === 'critical');
  const disabled  = [];

  for (const source of criticals) {
    const total    = (source.totalOk || 0) + (source.totalFail || 0);
    if (total < minSweeps) continue;
    const failRate = (source.totalFail || 0) / total;
    if (failRate < maxFailRate) continue;

    console.log(`[Updater] Auto-disabling ${source.name} (${Math.round(failRate * 100)}% failure rate, ${total} sweeps)`);
    const result = disableSource(source.name);
    disabled.push({ name: source.name, failRate: Math.round(failRate * 100), total, ...result });
  }
