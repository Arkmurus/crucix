...
import { saveExplorerFindings, logSelfUpdate, brainAbsorb } from './learning_store.mjs';
import { searxngInstances } from '../search/engine.mjs';   // R-F2209 — sovereign instance resolver
...
const EXPLORATION_QUERIES = [
  ...
  // Competitor intelligence — who is winning and where
  'defence broker competitor arms dealer Africa contract win 2025 2026',
  'Turkey defence export Africa contract signed 2025 2026',
  'China military export Africa arms deal 2025 2026',
  'Russia arms export Africa bypass sanctions 2025 2026',
  'Israel defence export Africa UAV drone deal 2025 2026',
  // Sanctions & export control updates
  'ITAR EAR export control update 2025 2026 new regulation',
  'UK ECJU export licence arms embargo update 2025 2026',
  'EU arms embargo update dual use regulation 2025 2026',
  'sanctions evasion arms procurement Africa shell company 2025 2026',
  // Contact & relationship intelligence
  'Africa defence minister appointed new 2025 2026',
  'Lusophone Africa military leadership change 2025 2026',
  'Angola Mozambique Guinea-Bissau defence ministry appointment 2025 2026',
  // Supply chain & OEM health
  'defence manufacturer bankruptcy merger acquisition 2025 2026',
  'ammunition shortage supply chain disruption military 2025 2026',
  'armoured vehicle production delay backlog defence 2025 2026',
];
...
// R-F373 (2026-05-12): searchBrave removed (independence directive,
// mirrors Python R-F320). SearXNG → Google News RSS serve the load.
// Stub kept so any forgotten caller returns [] cleanly instead of
...
async function searchSearXNG(query) {
  // R-F2209 — sovereign aria-searxng.internal instead of dead public instances.
...
async function runQueries(queries) {
  // R-F373 (2026-05-12): braveKey param removed; was conditional in the
  // batch loop but always fell through to SearXNG anyway.
  const allResults = [];
  ...
  for (let i = 0; i < queries.length; i += 3) {
    ...
    const batchResults = await Promise.allSettled(
      batch.map(async q => {
        // R-F373: SearXNG first now that Brave is gone.
        let results = await searchSearXNG(q);
        if (results.length === 0) {
...
export async function runExploration(llmProvider = null, opts = {}) {
  // R-F373: braveKey lookup removed; Brave is gone. Kept the export
  ...
  console.log(`[WebExplorer] Starting exploration — ${queries.length} queries`);
  ...
  // Brain hook: feed OSINT sweep to ARIA's learning system
  brainAbsorb({
    module: 'osint_sweep',
    summary: `OSINT sweep: ${findings.insights.length} insights, ${findings.newSources.length} new sources, ${findings.salesIdeas.length} sales ideas`,
    detail: (findings.insights || []).slice(0, 5).map(i => typeof i === 'string' ? i.slice(0, 100) : JSON.stringify(i).slice(0, 100)).join('; '),
    extra_topics: ['osint', 'market_intel'],
    confidence: 'ASSESSED',
  });
...
export async function exploreQuery(llmProvider, query) {
  // R-F373: Brave removed. SearXNG → Google News RSS.
...
export function formatExplorerFindingsForTelegram(findings) {
  ...
  msg += `_/explore <topic> for targeted search · Automatic pushes only when new high-signal findings appear_`;
  return msg;
}

const EXPLORER_TELEGRAM_WINDOW_DAYS = Number(process.env.EXPLORER_TELEGRAM_DEDUP_DAYS) || 5;

function _explorerTelegramStatePath() {
  return process.env.EXPLORER_TELEGRAM_STATE_PATH
    || join(process.cwd(), 'data', 'explorer_telegram_seen.json');
}

function _loadExplorerTelegramSeen() {
  try {
    const today = new Date().toISOString().slice(0, 10);
    const raw = JSON.parse(readFileSync(_explorerTelegramStatePath(), 'utf8'));
    const entries = Array.isArray(raw?.entries) ? raw.entries : [];
    return entries.filter(e => {
      const ageDays = Math.round((Date.parse(today) - Date.parse(e.d || today)) / 86_400_000);
      return e && e.k && ageDays < EXPLORER_TELEGRAM_WINDOW_DAYS;
    });
  } catch {
    return [];
  }
...
function _saveExplorerTelegramSeen(entries) {
  try {
    const p = _explorerTelegramStatePath();
    mkdirSync(dirname(p), { recursive: true });
    writeFileSync(p, JSON.stringify({ entries: entries.slice(-500) }, null, 0), 'utf8');
    return true;
  } catch (e) {
    console.warn('[WebExplorer] Telegram novelty-state persist failed:', e.message);
    return false;
  }
}

function _telegramKey(item) {
  return String(item?.sourceUrl || item?.title || item?.market || item?.buyer || '')
    .toLowerCase()
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 180);
...
function _strongInsight(item) {
  if (!item || typeof item !== 'object') return false;
  const title = String(item.title || '').trim();
  const summary = String(item.summary || '').trim();
  const relevance = String(item.relevance || '').toUpperCase();
  const hasSource = /^https?:\/\//i.test(String(item.sourceUrl || ''));
  const hasSpecifics = !!(item.dealSize || item.timeline || item.region);
  return title.length >= 20 && summary.length >= 40 && relevance === 'HIGH' && hasSource && hasSpecifics;
}

function _strongSalesIdea(item) {
  if (!item || typeof item !== 'object') return false;
  const urgency = String(item.urgency || '').toUpperCase();
  const title = String(item.title || '').trim();
  const nextStep = String(item.nextStep || '').trim();
  const buyer = String(item.buyer || '').trim();
  const product = String(item.productCategory || '').trim();
  return title.length >= 20
    && nextStep.length >= 25
    && buyer.length >= 6
    && product.length >= 4
    && ['HIGH', 'MEDIUM'].includes(urgency);
}

export function selectTopExplorerFindings(findings, opts = {}) {
  const maxInsights = opts.maxInsights ?? 2;
  const maxIdeas = opts.maxIdeas ?? 2;
  const seen = new Set(_loadExplorerTelegramSeen().map(e => e.k));
  const fresh = item => {
    const key = _telegramKey(item);
    return key && !seen.has(key);
  };
  const insights = (findings?.insights || []).filter(i => _strongInsight(i) && fresh(i)).slice(0, maxInsights);
  const salesIdeas = (findings?.salesIdeas || []).filter(i => _strongSalesIdea(i) && fresh(i)).slice(0, maxIdeas);
  return { ...findings, insights, salesIdeas };
}

export function formatExplorerFindingsForTelegramIfTop(findings, opts = {}) {
  const selected = selectTopExplorerFindings(findings, opts);
  const keys = [...(selected.insights || []), ...(selected.salesIdeas || [])].map(_telegramKey).filter(Boolean);
  if (keys.length === 0) {
    return { shouldSend: false, reason: 'no new high-signal findings', text: '', keys: [] };
  }
  return { shouldSend: true, reason: 'new high-signal findings', text: formatExplorerFindingsForTelegram(selected), keys };
}

export function recordExplorerTelegramPost(keys) {
  const arr = Array.isArray(keys) ? keys : [keys];
  const entries = _loadExplorerTelegramSeen();
  const today = new Date().toISOString().slice(0, 10);
  for (const k of arr) {
    if (k) entries.push({ k, d: today });
  }
  return _saveExplorerTelegramSeen(entries);
}
