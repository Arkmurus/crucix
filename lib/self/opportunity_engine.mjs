...
function scoreOpportunity(market, conflictData, blockers, devFinanceCount, marketIntel) {
  ...
  // Base score from market priority — REDUCED from 18/10/5 to 10/6/3.
  // Market priority alone should not inflate a score to MEDIUM tier.
  // Signal quality must carry the score.
  score += market.priority === 'HIGH' ? 10 : market.priority === 'MEDIUM' ? 6 : 3;
  ...
  // Lusophone advantage: language + relationship edge (8pts — reduced from 12)
  if (market.lusophone) score += 8;
