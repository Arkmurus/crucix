...
const TARGET_MARKETS = [
  ...
  // Europe (NATO/EU members — modernisation budgets post-2022)
...
// Extract market mention counts from explorer findings + OSINT signals
function _extractExplorerSignals(currentData) {
  const counts = {};
  const texts = [
    ...(currentData.signals || []).map(s => s.text || ''),
    ...(currentData.news || []).map(n => (n.title || '') + ' ' + (n.summary || '')),
  ];
  for (const text of texts) {
    const lower = text.toLowerCase();
    for (const market of TARGET_MARKETS) {
      if (lower.includes(market.name.toLowerCase()) || lower.includes((market.iso2 || '').toLowerCase())) {
...
export function detectOpportunities(currentData) {
  ...
  // Build a lookup of explorer/OSINT signals per market for non-ACLED signal detection
  const explorerSignals = _extractExplorerSignals(currentData);
  ...
  for (const market of TARGET_MARKETS) {
    ...
    const matchedOEMs = matchOEMs(
      procurementNeeds.length > 0 ? procurementNeeds : ['small arms', 'ammunition', 'communications']
    );
    ...
    opportunities.push({
      ...
      notes: !hasConflict
        ? `Pre-positioning: ${market.priority} priority market — no active conflict events${marketSignals > 0 ? `, ${marketSignals} intel signal(s) detected` : ''}`
        : blockers.length > 0
