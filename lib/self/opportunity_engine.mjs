...
import { saveOpportunities, getAdaptiveScoringWeights, brainAbsorb } from './learning_store.mjs';

// ── Signal amplification: region-to-market mapping ───────────────────────────
const REGION_MARKET_MAP = {
  'Lusophone Africa': ['Angola', 'Mozambique', 'Guinea-Bissau', 'Cape Verde', 'São Tomé and Príncipe'],
  'West Africa': ['Nigeria', 'Ghana', 'Senegal', 'Cameroon', "Côte d'Ivoire"],
  'East / Central Africa': ['Kenya', 'Ethiopia', 'Uganda', 'Rwanda', 'Tanzania'],
  'East Asia': ['Philippines', 'Indonesia', 'Vietnam', 'Bangladesh'],
  'South Asia': ['Bangladesh', 'Indonesia'],
  'Middle East': ['UAE', 'Jordan', 'Saudi Arabia'],
  'Eastern Europe': ['Bulgaria', 'Romania', 'Poland', 'Greece'],
  'Latin America': ['Brazil', 'Colombia', 'Peru'],
  'Energy / Maritime': ['Angola', 'Nigeria', 'Mozambique', 'UAE', 'Saudi Arabia'],
  'Defence Procurement': [],  // global — boosts all markets slightly
};

function getCorrelationBoost(marketName, correlations) {
  if (!correlations || !correlations.length) return 0;
  let boost = 0;
  for (const c of correlations) {
    const markets = REGION_MARKET_MAP[c.region] || [];
    // Defence Procurement correlation boosts all markets
    const matches = markets.length === 0 ? (c.region === 'Defence Procurement') : markets.includes(marketName);
    if (!matches) continue;
    const sourceCount = c.topSignals?.length || 1;
    boost += sourceCount * 4;
    if (c.severity === 'critical') boost += 15;
    else if (c.severity === 'high') boost += 10;
    else if (c.severity === 'medium') boost += 5;
  }
  return Math.min(30, boost);
}

// ── Energy price momentum ────────────────────────────────────────────────────
const OIL_DEPENDENT_MARKETS = new Set([
  'Angola', 'Nigeria', 'Saudi Arabia', 'UAE', 'Brazil', 'Colombia', 'Peru',
  // Note: Mozambique is GAS-dependent (Rovuma LNG), not oil — excluded from oil correlation
]);
...
export const TARGET_MARKETS = [
  ...
  // North Africa
  { name: 'Morocco',        iso2: 'MA', lusophone: false, priority: 'MEDIUM', riskLevel: 1,
    strategicNeeds: ['fighter aircraft', 'armoured vehicles', 'naval systems', 'surveillance UAV', 'air defense'],
    procurementPortal: '',
    stratContext: 'FAR modernisation; regional power; Sahara security; active defence importer (F-16V, Apache, Abrams)' },
  { name: 'Algeria',        iso2: 'DZ', lusophone: false, priority: 'MEDIUM', riskLevel: 2,
    strategicNeeds: ['armoured vehicles', 'naval systems', 'air defense', 'fighter aircraft', 'ammunition'],
    procurementPortal: '',
    stratContext: 'Largest African defence budget; traditionally Russian-equipped; diversifying post-2022; ANP modernisation' },
  { name: 'Egypt',          iso2: 'EG', lusophone: false, priority: 'MEDIUM', riskLevel: 2,
    strategicNeeds: ['naval systems', 'armoured vehicles', 'fighter aircraft', 'missiles', 'surveillance'],
    procurementPortal: '',
    stratContext: 'Regional power; French/US/Russian diversified supply; EAF modernisation; Sinai security operations' },
  { name: 'Libya',          iso2: 'LY', lusophone: false, priority: 'LOW',    riskLevel: 5,
    strategicNeeds: ['border security', 'surveillance', 'small arms', 'communications'],
    procurementPortal: '',
    stratContext: 'Post-conflict; split governance; UN arms embargo (review needed); maritime migration security' },
  { name: 'Tunisia',        iso2: 'TN', lusophone: false, priority: 'LOW',    riskLevel: 2,
    strategicNeeds: ['border security', 'counter-terrorism', 'surveillance', 'patrol vehicles'],
    procurementPortal: '',
    stratContext: 'Libya border security; US partner; moderate defence budget; EU border cooperation' },
  // Middle East (export-permissible)
  { name: 'Qatar',          iso2: 'QA', lusophone: false, priority: 'MEDIUM', riskLevel: 1,
    strategicNeeds: ['air defense', 'naval systems', 'special forces equipment', 'cyber'],
    procurementPortal: '',
    stratContext: 'QEAF modernisation (Rafale, F-15QA); small but high-spend; regional hub; DIMDEX' },
  { name: 'Oman',           iso2: 'OM', lusophone: false, priority: 'LOW',    riskLevel: 1,
    strategicNeeds: ['maritime surveillance', 'border security', 'armoured vehicles', 'communications'],
    procurementPortal: '',
    stratContext: 'Strait of Hormuz security; traditional UK partner; modest budget; balanced diplomacy' },
  { name: 'Kuwait',         iso2: 'KW', lusophone: false, priority: 'LOW',    riskLevel: 1,
    strategicNeeds: ['armoured vehicles', 'air defense', 'surveillance', 'ammunition'],
    procurementPortal: '',
    stratContext: 'Post-1991 build-up mature; modernisation cycle; US FMS primary route' },
  { name: 'Iraq',           iso2: 'IQ', lusophone: false, priority: 'MEDIUM', riskLevel: 4,
    strategicNeeds: ['counter-terrorism', 'armoured vehicles', 'surveillance UAV', 'ammunition', 'communications'],
    procurementPortal: '',
    stratContext: 'ISF reconstruction; diverse suppliers (US/Iran/Russia/Turkey); ISR deficit; border security' },
  ...
  // Turkey (key defence exporter — Arkmurus Measure Group partner)
  { name: 'Turkey',         iso2: 'TR', lusophone: false, priority: 'HIGH',   riskLevel: 2,
    strategicNeeds: ['co-production', 'technology transfer', 'export facilitation', 'joint ventures'],
    procurementPortal: 'https://www.ssb.gov.tr/',
    stratContext: 'SSB/Baykar/ASELSAN/ROKETSAN — major exporter to Africa/Asia; Measure Group relationship; no ITAR components on many platforms' },
  // Asia (additional)
  { name: 'South Korea',    iso2: 'KR', lusophone: false, priority: 'MEDIUM', riskLevel: 1,
    strategicNeeds: ['armoured vehicles', 'artillery', 'naval systems', 'fighter aircraft'],
    procurementPortal: '',
    stratContext: 'K2 tank, K9 SPH global sales; DAPA; competing with European/US OEMs in Africa/SE Asia' },
  { name: 'India',          iso2: 'IN', lusophone: false, priority: 'MEDIUM', riskLevel: 2,
    strategicNeeds: ['armoured vehicles', 'naval systems', 'fighter aircraft', 'missiles', 'surveillance'],
    procurementPortal: '',
    stratContext: 'Make in India; DRDO/HAL; diversifying from Russian supply; massive defence budget; active exporter (BrahMos)' },
  { name: 'Japan',          iso2: 'JP', lusophone: false, priority: 'LOW',    riskLevel: 1,
    strategicNeeds: ['missile defense', 'naval systems', 'surveillance', 'cyber'],
    procurementPortal: '',
    stratContext: 'ATLA; relaxing arms export policy; GCAP (next-gen fighter); Indo-Pacific security' },
...
function getProcurementNeeds(conflictData, market) {
  ...
  // R-F2012: do NOT fall back to market.strategicNeeds (hand-authored static
  // lists). procurementNeeds must come from REAL conflict/signal data only — a
  // templated wishlist shown as "detected need" is fabrication. Empty is honest.
...
function scoreOpportunity(market, conflictData, blockers, devFinanceCount, marketIntel) {
  let score = 0;

  // R-F2012: NO editorial base from market.priority. The score must reflect
  // REAL observed signals only (conflict, fatalities, procurement signals, dev
  // finance). A static priority constant baked into a "/100" opportunity score
  // is fabrication — it made strategically-interesting-but-quiet markets look
  // like live opportunities. Strategic priors are not observed opportunity.

  // Conflict intensity (max 30pts — reduced from 40)
  score += Math.min(30, conflictData.total * 2);

  // Fatalities indicate severity (max 10pts — reduced from 15)
  score += Math.min(10, Math.floor(conflictData.fatalities / 15));

  // Procurement type diversity (max 12pts — reduced from 15)
  score += Math.min(12, Object.keys(conflictData.byType).length * 3);

  // Development finance signals = budget available (8pts — reduced from 10)
  if (devFinanceCount > 0) score += 8;

  // R-F2012: removed the editorial Lusophone +8 bonus — a strategic prior, not
  // an observed signal. (Lusophone advantage still informs win-probability /
  // relationship scoring elsewhere; it does not belong in the observed-opportunity score.)

  // PROCUREMENT SIGNAL QUALITY — new dimension.
  // Markets with actual procurement sources (not just conflict/news) score higher.
  if (marketIntel) {
    const procSources = (marketIntel.sources || []).filter(s => s.isProcurement);
    score += Math.min(20, procSources.length * 8); // up to 20pts for real procurement signals
  }

  // R-F2012: removed the editorial market.riskLevel penalty (a static prior).
  // Real, observed penalties (sanctions/export-control blockers) still apply below.

  // Sanctions/blocker penalty
...
// R-F2015 (ARIA opportunity audit, Phase 1): a news/signal item only counts as a
// real market SIGNAL if it is actually defence/security/procurement relevant.
// Pre-fix, ANY article merely mentioning a country name was counted — so "Cape
// Verde extend World Cup dream" and "Ronaldo plays Colombia" were logged as
// defence signals. This lexicon gates that: no defence relevance -> noise, skip.
const DEFENCE_RELEVANCE_KW = [
  'defence', 'defense', 'military', 'army', 'navy', 'air force', 'armed forces',
  'soldier', 'troop', 'weapon', 'arms ', 'ammunition', 'missile', 'artillery',
  'rifle', 'firearm', 'grenade', 'fighter jet', 'warplane', 'helicopter', 'drone',
  'uav', 'radar', 'frigate', 'warship', 'patrol vessel', 'patrol boat', 'armoured',
  'armored', ' tank', 'apc', 'ifv', 'mrap', 'coast guard', 'border security',
  'counter-terror', 'counterterror', 'counter-insurgency', 'counterinsurgency',
  'insurgenc', 'militant', 'jihadist', 'procurement', 'tender', 'rfp', 'rfq',
  'contract award', 'arms deal', 'weapons deal', 'defence deal', 'fms', 'dsca',
  'embargo', 'sanction', 'peacekeep', 'airstrike', 'ministry of defence',
  'ministry of defense', 'defence budget', 'defense budget', 'military spending',
  'modernisation programme', 'modernization program', 'acquisition programme',
];

function _isDefenceRelevant(text) {
  const t = (text || '').toLowerCase();
  return DEFENCE_RELEVANCE_KW.some(kw => t.includes(kw));
}
...
function _extractMarketIntel(currentData) {
  ...
  for (const item of candidates) {
    ...
    const combined = (title + ' ' + (item.summary || '')).toLowerCase();

    // R-F2015 — defence-relevance gate: a country-mentioning article is only a
    // signal if it's actually about defence/security/procurement. Drops sports/
    // entertainment/general noise that merely names the country.
    if (!_isDefenceRelevant(combined)) continue;

    // Find ALL matching markets, then determine the PRIMARY one.
    // The primary market is the one that appears first in the TITLE
    // (headlines put the subject country first). If no country appears
    // in the title, fall back to first mention in the full text.
    // This prevents "Somalia cancels UAE agreements" from filing under UAE.
    const matchedMarkets = TARGET_MARKETS.filter(m =>
      combined.includes(m.name.toLowerCase())
    );

    if (matchedMarkets.length === 0) continue;

    // Determine primary market: earliest position in title wins
    let primaryMarket = matchedMarkets[0];
    let bestPos = Infinity;
    for (const m of matchedMarkets) {
      const nameL = m.name.toLowerCase();
      const posInTitle = title.indexOf(nameL);
      const pos = posInTitle >= 0 ? posInTitle : (combined.indexOf(nameL) + 10000);
      if (pos < bestPos) {
        bestPos = pos;
        primaryMarket = m;
      }
    ...
    // Only file under the PRIMARY market (the subject of the headline)
    const key = primaryMarket.name.toLowerCase();
...
export async function detectOpportunities(currentData) {
  ...
  // Trend-driven timing boost (Unit 5) + Energy momentum (Unit 6)
  let trendBoost = 0;
  let energyBoostMap = {};
  try {
    const { analyzeTrends } = await import('../intel/archive.mjs');
    const trends = analyzeTrends();
    if (trends?.signals?.trend?.direction === 'rising') trendBoost += 5;
    if (trends?.criticalChanges?.trend?.direction === 'rising') trendBoost += 3;
    if (trends?.signals?.trend?.direction === 'falling') trendBoost -= 3;
    // Energy momentum for oil-dependent markets
    const wtiDir = trends?.wti?.trend?.direction;
    const vixDir = trends?.vix?.trend?.direction;
    for (const m of OIL_DEPENDENT_MARKETS) {
      energyBoostMap[m] = 0;
      if (wtiDir === 'rising') energyBoostMap[m] += 8;
      else if (wtiDir === 'falling') energyBoostMap[m] -= 5;
    }
    if (vixDir === 'rising') {
      for (const m of TARGET_MARKETS) energyBoostMap[m.name] = (energyBoostMap[m.name] || 0) + 3;
    }
  } catch {}
  ...
  for (const market of TARGET_MARKETS) {
    ...
    // R-F2014 (completes R-F2012): match OEMs to the REAL detected needs only.
    // The old fallback recommended OEMs off a hand-authored/hardcoded needs list
    // when none were actually detected — fabricated OEM recs surfaced in Telegram
    // summaries + the API. No real need -> no OEM recommendation. Honest.
    ...
    const marketKey = market.name.toLowerCase();
    const marketIntelData = marketIntel[marketKey] || null;
    let score = scoreOpportunity(market, conflictData, blockers, devFinanceCount, marketIntelData);
    ...
    // R-F2015 — MIN-EVIDENCE GATE (ARIA audit Problem 2). A market with NO real
    // evidence (no defence signal, no conflict, no dev-finance, no procurement
    // source) must NOT appear. Pre-fix, correlationBoost/trend/energy inflated
    // ~32 zero-signal markets to scores of 17-30 — opportunities with nothing
    // behind them. Boosts AMPLIFY real evidence; they don't manufacture it.
    const _sourceCount = (marketIntelData?.sources || []).length;
    const hasRealSignal = conflictData.total > 0 || marketSignals > 0
                          || devFinanceCount > 0 || _sourceCount > 0;
    if (!hasRealSignal) continue;

    // Signal amplification: multi-source correlation boost (Unit 3)
    const correlationBoost = getCorrelationBoost(market.name, currentData.correlations || []);
    score += correlationBoost;

    // Trend-driven timing boost (Unit 5)
    score += trendBoost;

    // Energy price momentum for oil-dependent markets (Unit 6)
    score += (energyBoostMap[market.name] || 0);

    // Apply outcome-learned multiplier — always active (default 1.0 = neutral)
    const multiplier = adaptiveWeights?.regionMultipliers?.[market.name] ?? 1.0;
    score = Math.min(100, Math.max(0, Math.round(score * multiplier)));

    // R-F2012: NO "always show HIGH-priority markets" floor. A market with no
    // real signal must NOT appear — surfacing it as a WATCH card misled users
    // into seeing a non-existent opportunity. Only markets that clear the
    // threshold on REAL observed signal score are shown. An empty page is honest.
    if (score < 5) continue;

    const hasConflict = conflictData.total > 0;
    // Tier assignment — score-driven only. No automatic floor for HIGH priority
    // markets. A market with no signals should not appear as MEDIUM just because
    // it's strategically important — that misleads the user into thinking there's
    // an active opportunity when there isn't.
    let calcTier = score >= 65 ? 'HIGH' : score >= 35 ? 'MEDIUM' : 'WATCH';
    opportunities.push({
      ...
      // R-F2012: "CLEAR" falsely implied a PASSED compliance screen — we only
      // checked a static blocker keyword list, not a real sanctions/export-control
      // screen. Report the honest state (no automated blocker hit != screened clean).
      complianceStatus: blockers.length === 0 ? 'NOT_SCREENED' : 'REVIEW_REQUIRED',
      ...
      notes: blockers.length > 0
          ...
          : hasConflict
            ...
            : `${marketSignals} intel signal(s) detected`,  // R-F2012: no stratContext fallback (hand-authored prose shown as analysis)
      adaptiveMultiplier: multiplier ?? null,
      correlationBoost,
      trendBoost,
      energyBoost: energyBoostMap[market.name] || 0,
  ...
  // Brain hook: feed opportunity detection to ARIA's learning system
  const high = sorted.filter(o => o.tier === 'HIGH');
  brainAbsorb({
    module: 'opportunity_detector',
    summary: `Opportunity scan: ${sorted.length} markets scored, ${high.length} HIGH tier. Top: ${sorted.slice(0, 3).map(o => `${o.market}(${o.score})`).join(', ')}`,
    detail: sorted.slice(0, 5).map(o => `${o.market}: score=${o.score}, tier=${o.tier}, window=${o.procurementWindow || 'N/A'}`).join('; '),
    extra_topics: ['market_intel', 'competitor_intel'],
    confidence: 'ASSESSED',
  });
