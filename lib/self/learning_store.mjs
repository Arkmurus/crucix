...
// surface (the second auth check needs ARIA_API_TOKEN matching what fly
// expects — easy to misconfigure). The one-line fix: prefer the direct fly
// URL via BRAIN_DIRECT_URL, with a hardcoded production fallback. Localhost
// stays as the dev-only last resort.
const BRAIN_URL = (
  process.env.BRAIN_DIRECT_URL ||      // explicit direct-to-fly override
  process.env.BRAIN_SERVICE_URL ||      // legacy: explicit brain URL
  'https://aria-intel.fly.dev'         // production fallback (never goes via seenode self-proxy)
).replace(/\/+$/, '');
...
export async function brainAbsorb({ module, summary, detail, entity_name, success, extra_topics, source_id, confidence }) {
