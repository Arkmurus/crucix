...
// Returns null if not enough data yet (< 20 resolved outcomes).
export function getAdaptiveScoringWeights() {
  ...
  for (const [region, stats] of Object.entries(byRegion)) {
    ...
    // Scale 0.3× (0% accurate) → 1.5× (100% accurate), linear
    regionMultipliers[region] = parseFloat(Math.max(0.3, Math.min(1.5, 0.3 + accuracy * 1.2)).toFixed(3));
  }
