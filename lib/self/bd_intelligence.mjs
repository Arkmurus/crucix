...
function parseLlmJson(text) {
  ...
  try {
    ...
    try { return JSON.parse(clean(m[0])); }
      ...
      try { return JSON.parse(repaired); } catch {}
      // Fix 4: truncated JSON — close any open braces/brackets
      let opens = 0, closesNeeded = '';
      for (const ch of repaired) {
        if (ch === '{') opens++;
        else if (ch === '}') opens--;
      }
      while (opens > 0) { closesNeeded += '}'; opens--; }
      if (closesNeeded) {
        try { return JSON.parse(repaired + closesNeeded); } catch {}
      }
