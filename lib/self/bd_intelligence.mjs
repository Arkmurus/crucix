...
import { logSelfUpdate, recordAlertOutcome, getAdaptiveScoringWeights, brainAbsorb } from './learning_store.mjs';
import { redisGet, redisSet, redisLockAcquire, redisLockRelease } from '../persist/store.mjs';
...
// Exported (with underscore prefix) so the R-F355 unit tests can pin
// the new Fix 6/7 strategies without standing up the full LLM stack.
export function _parseLlmJson(text) {
  return parseLlmJson(text);
}
function parseLlmJson(text) {
  let s = (text || '').trim();
  // Strip BOM, zero-width chars, and other invisible prefixes
  s = s.replace(/^\uFEFF/, '').replace(/^[\u200B\u200C\u200D\uFEFF\u00A0]+/, '');
  // Strip markdown code fences (single or triple backtick, with or without json label)
  s = s.replace(/^`{1,3}(?:json)?\s*\n?/i, '').replace(/\n?\s*`{1,3}\s*$/i, '');
  // Strip wrapping quotes if the entire response is "{ ... }"
  if (s.startsWith('"') && s.endsWith('"')) { try { s = JSON.parse(s); } catch {} }
  // Strip any leading prose/text before the first {
  const firstBrace = s.indexOf('{');
  if (firstBrace < 0) throw new Error('No JSON object found in LLM response');
  ...
  function clean(str) {
    ...
    c = c.replace(/^```(?:json)?\s*\n?/gm, '').replace(/\n?```\s*$/gm, '');
    // Strip single-line JS/C++ comments — ONLY at line-start (with optional indent),
    // never anywhere a `://` could appear inside a URL string value. The previous
    // regex was greedy and destroyed every URL in JSON values like
    // "url": "https://example.com" → "url": "https:" → JSON parse failure.
    // This was losing 5+ LLM-generated weekly priorities per day.
    c = c.replace(/^[ \t]*\/\/[^\n]*$/gm, '');
  ...
  try {
    ...
    // Pull the outermost { ... } block if both braces are present; otherwise
    // (truncation cut before any `}`) feed the whole string into the repair
    // chain — Fix 4 can rebuild missing closers.
    const m = s.match(/\{[\s\S]*\}/);
    const candidate = m ? m[0] : (s.includes('{') ? s : null);
    if (!candidate) throw new Error('No JSON object found in LLM response');
    try { return JSON.parse(clean(candidate)); }
    catch (e) {
      // 2026-04-12: multi-stage JSON repair instead of blanket newline replace
      let repaired = clean(candidate);
      // Fix 1: escape newlines ONLY inside string values (between quotes)
      repaired = repaired.replace(/"([^"\\]*(?:\\.[^"\\]*)*)"/g, (match) => {
        return match.replace(/\n/g, '\\n').replace(/\r/g, '\\r').replace(/\t/g, '\\t');
      });
      try { return JSON.parse(repaired); } catch {}
      // Fix 2: unquoted keys — "key": → key: (common DeepSeek quirk)
      repaired = repaired.replace(/(?<=[\{,]\s*)([a-zA-Z_]\w*)(?=\s*:)/g, '"$1"');
      try { return JSON.parse(repaired); } catch {}
      // Fix 3: single quotes → double quotes, but ONLY outside existing
      // double-quoted strings. The blanket replace used to corrupt apostrophes
      // inside string values (e.g. "Angola's defence space" → "Angola"s defence
      // space"), which then defeated Fix 4's truncation repair downstream.
      {
      ...
      try { return JSON.parse(repaired); } catch {}
      // Fix 4: truncated JSON — stack-based repair. Walks the string tracking
      // string state and the opener stack, then closes in correct nesting order.
      // 2026-04-26: previous version only counted `{}`, never `[]`, and didn't
      // handle truncation inside an unterminated string (mid-value cut). The BD
      // strategy call hit exactly that mode when the model ran out of token
      // budget mid-`"product"` value.
      {
        ...
        if (inString) suffix = '"';
          ...
          // Trailing `:` means a key with no value yet — fill with null so JSON
          // is valid; trailing `,` is just a dangling separator — drop it.
          if (/:\s*$/.test(pre)) suffix = 'null';
          else pre = pre.replace(/,\s*$/, '');
        }
        while (stack.length) suffix += stack.pop();
        if (suffix) {
          try { return JSON.parse(pre + suffix); } catch {}
        }
      }
      // Fix 5: nuclear option — strip everything except the JSON core
      const nuclear = repaired
        .replace(/[\x00-\x1f]/g, ' ')
        .replace(/\n/g, ' ')
        .replace(/\s+/g, ' ');
      try { return JSON.parse(nuclear); } catch {}

      // R-F355 (2026-05-12) — Fix 6: unescaped embedded quotes in string
      // values. Recurring BD failure 06:41:48 — LLM emitted a string
      // containing literal `"` characters that weren't escaped (e.g.
      // "Kenya is a major non-"NATO" ally"). Parser sees the inner `"`
      // as a string terminator and chokes on the next token. Walk the
      // string, track in/out-of-string state, and when we see a `"`
      // while inString=true, look ahead: if the NEXT non-whitespace
      // char is a structural delimiter (`,`, `}`, `]`, `:`, or EOF),
      // it's a real closer; otherwise it's an embedded quote → escape it.
      {
        let result = '';
        let inString = false;
        let escape = false;
        for (let i = 0; i < repaired.length; i++) {
          ...
          if (ch === '"') {
            if (inString) {
              ...
              const nextChar = j < repaired.length ? repaired[j] : '';
      ...
      // R-F355 Fix 7: last-resort position-based truncation. If we still
      // can't parse, slice to the last `,` or `{` before the JSON error
      // position and close the JSON cleanly using the Fix 4 stack logic.
      // Sacrifices content after the error position but produces parseable
      // JSON instead of throwing. Better than zero output.
      try { JSON.parse(repaired); } catch (parseErr) {
        const posMatch = (parseErr.message || '').match(/position (\d+)/);
        if (posMatch) {
          const errPos = parseInt(posMatch[1], 10);
          const lastComma = repaired.lastIndexOf(',', errPos);
          const lastOpenBrace = repaired.lastIndexOf('{', errPos);
          const safePoint = Math.max(lastComma, lastOpenBrace);
          if (safePoint > 0) {
            let truncated = repaired.substring(0, safePoint + 1).replace(/,\s*$/, '');
            // Reuse Fix 4 stack-close logic
            let inString = false, escape = false;
            const stack = [];
      ...
      try { return JSON.parse(nuclear); }
      catch (e2) {
        console.error('[BD] JSON repair failed after 7 strategies. Input preview:', s.slice(0, 300));
        throw new Error('JSON parse failed after 7 repair strategies: ' + e2.message);
      }
    }
  }
}
...
// Phrases indicating coverage ABOUT a tender/deal, not an actionable opportunity.
// Includes cancellations, criticism, investigations, and negative sentiment.
const NEGATIVE_COVERAGE_KW = [
  ...
  'halted', 'cancelled', 'cancels', 'canceled', 'cancel ', 'cancellation',
  'suspended', 'suspends', 'scrapped', 'scraps', 'overpriced', 'inflated',
  'audit finding', 'whistleblow', 'leaked document',
  'terminat', 'revoked', 'revokes', 'withdrew', 'withdraws', 'withdrawal',
  'collapsed', 'collapses', 'failed', 'failure', 'abandoned', 'abandons',
  'rejected', 'rejects', 'denied', 'denies', 'blocked', 'blocks',
  'threatens', 'warning', 'warns', 'disputed', 'disputes',
];
...
// ── Source verification tier — official portals vs news coverage ─────────────
// Official procurement portals produce real, actionable tenders.
// News sources produce coverage ABOUT procurement — useful context but not leads.
const OFFICIAL_PROCUREMENT_DOMAINS = [
  'ted.europa.eu', 'sam.gov', 'contractsfinder.service.gov.uk',
  'ungm.org', 'devbusiness.com', 'dgmarket.com',
  'afdb.org', 'worldbank.org/procurement', 'ebrd.com/work-with-us',
  'nspa.nato.int', 'merx.com', 'buyandsell.gc.ca',
  'etenders.gov.ie', 'contracts.gov.za',
  // African procurement portals
  'e-procurement.gov.za', 'ifmis.go.ke', 'ppda.go.ug',
  'procurement.gov.ng', 'ppb.gov.mz',
];
...
function getSourceTier(url) {
  ...
  try {
    ...
    if (OFFICIAL_PROCUREMENT_DOMAINS.some(d => domain.includes(d))) return 'OFFICIAL';
    if (['janes.com', 'shephard.co.uk', 'defensenews.com', 'breakingdefense.com',
         'armyrecognition.com', 'defenceweb.co.za', 'defense-aerospace.com'].some(d => domain.includes(d)))
      return 'INDUSTRY';
    if (['reuters.com', 'bbc.com', 'ft.com', 'aljazeera.com', 'france24.com',
         'lusa.pt', 'dw.com', 'rfi.fr'].some(d => domain.includes(d)))
      return 'QUALITY_PRESS';
    return 'GENERAL';
  } catch { return 'UNVERIFIED'; }
}

function hasDefenseRelevanceStrong(text) {
  // Require at least 2 defense keywords for stronger relevance signal
  const lower = (text || '').toLowerCase();
  const hits = DEFENSE_RELEVANCE_KW.filter(k => lower.includes(k)).length;
  return hits >= 2;
}
...
function extractRealTenders(procurementTenders, targetMarkets) {
  ...
  // De-dupe feed buckets — same item used to enter the pipeline 2-3 times
  // via .updates / .lusophone / .africa, which skewed scoring toward Africa.
  const allUpdates = [];
  const _seen = new Set();
  const _dedupeKey = (it) => {
    const u = (it.url || it.link || '').toLowerCase().trim();
    if (u) return 'u:' + u;
    return 't:' + (it.title || '').toLowerCase().trim().substring(0, 120);
  };
  for (const bucket of [procurementTenders.updates, procurementTenders.lusophone, procurementTenders.africa]) {
  ...
  for (const item of allUpdates) {
    ...
    // FIRST: reject negative coverage immediately — before any scoring.
    // Past bug: "Somalia cancels UAE agreements" was scored as a tender
    // because keyword scoring ran before negative coverage check.
    if (isNegativeCoverage(text, itemUrl)) continue;

    // Skip non-defense sources ONLY if text lacks STRONG defense relevance
    // (require 2+ defense keywords, not just "police" or "security")
    const srcLower = (item.source || '').toLowerCase();
    const isNonDefSrc = NON_DEFENSE_SOURCES.some(s => srcLower.includes(s));
    if (isNonDefSrc && !hasDefenseRelevanceStrong(text)) continue;
    ...
    // Require STRONG defense relevance (2+ keywords) to prevent
    // development/humanitarian projects from slipping in
    if (!hasDefenseRelevanceStrong(text)) continue;

    // Match to PRIMARY target market — STRICT rules to prevent incidental
    // mentions from hijacking attribution. Past bug: "Top 10 African
    // Countries..." got tagged as India because "India" appeared once in
    // the body as a comparison.
    //
    // Rules:
    //   1. A market qualifies ONLY if its name is in the TITLE, or appears
    //      ≥2 times in the full text (body-only single mentions are
    //      treated as incidental).
    //   2. ISO2 codes only count inside the title (word-boundary) — body
    //      ISO2 matches are too noisy.
    //   3. When multiple markets qualify, the title mention wins. If none
    //      are in the title, prefer the one with the most body mentions.
    //   4. If nothing qualifies, the item is tagged 'Global' rather than
    //      force-matched to the first country found.
    const titleLower = (item.title || '').toLowerCase();
    const bodyLower  = ((item.content || item.text || item.summary || '') + '').toLowerCase();
    // M6 fix: word-boundary counter. Raw substring was matching "Mali"
    // inside "Somalia", "Iran" inside "Iranian" is fine but the bug was
    // that 2 body mentions of "mali" inside "somalia"+"somalia" triggered
    // Mali attribution on a Somalia story. Word boundary kills that.
    const _escapeRe = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const _countWB = (hay, needle) => {
      if (!needle) return 0;
      const re = new RegExp(`\\b${_escapeRe(needle)}\\b`, 'g');
      const m = hay.match(re);
      return m ? m.length : 0;
    };
    const _firstWBPos = (hay, needle) => {
      if (!needle) return -1;
      const re = new RegExp(`\\b${_escapeRe(needle)}\\b`);
      const m = re.exec(hay);
      return m ? m.index : -1;
    };
    const qualified = [];
    for (const m of targetMarkets) {
      ...
      const titlePos = _firstWBPos(titleLower, nameL);
      const inTitle = titlePos >= 0;
      const iso2InTitle = m.iso2 && new RegExp(`\\b${m.iso2.toLowerCase()}\\b`).test(titleLower);
      const bodyMentions = _countWB(bodyLower, nameL);
      // Rule 1: title match, OR ≥2 body mentions, OR ISO2 in title
      if (inTitle || iso2InTitle || bodyMentions >= 2) {
        qualified.push({
          market: m,
          inTitle: inTitle || iso2InTitle,
          bodyMentions,
          titlePos: inTitle ? titlePos : Infinity,
        });
      }
    }
    let market = null;
    if (qualified.length > 0) {
      // Prefer title matches, tie-break by earliest title position,
      // then by most body mentions.
      qualified.sort((a, b) => {
        if (a.inTitle !== b.inTitle) return a.inTitle ? -1 : 1;
        if (a.titlePos !== b.titlePos) return a.titlePos - b.titlePos;
        return b.bodyMentions - a.bodyMentions;
      });
      market = qualified[0].market;
    }
    // market = null → falls through as 'Global' at line ~351

    const type = tScore > 0 ? 'TENDER' : cScore > 0 ? 'CONTRACT' : 'BUDGET';

    // Source tier determines verification status
    const sourceTier = getSourceTier(itemUrl);

    // Lead quality — MUCH stricter thresholds.
    // HOT = only from official procurement portals with strong keyword match.
    // WARM = industry/quality press with good keyword match.
    // WATCH = everything else (news coverage, general sources).
    const hasUrl = !!itemUrl;
    const isStrategicMarket = market?.priority === 'HIGH' || market?.lusophone;
    let leadQuality;
    if (sourceTier === 'OFFICIAL' && tScore >= 2) {
      leadQuality = 'HOT';  // Real tender from official portal
    } else if (sourceTier === 'OFFICIAL' && total >= 2) {
      leadQuality = 'WARM'; // Official source, contract/budget signal
    } else if ((sourceTier === 'INDUSTRY' || sourceTier === 'QUALITY_PRESS') && tScore >= 3 && isStrategicMarket) {
      leadQuality = 'WARM'; // Industry source with strong tender signal in strategic market
    } else {
      leadQuality = 'WATCH'; // News coverage — context, not a lead
    }

    // Score: signal-quality + source-tier only. Market-bias bonuses
    // (lusophone +8, HIGH priority +5) were removed because they produced
    // an Africa-only pipeline — source tier is now the single quality
    // multiplier, so every region competes on the same footing.
    const sourceTierBonus = sourceTier === 'OFFICIAL' ? 20 : sourceTier === 'INDUSTRY' ? 10 : sourceTier === 'QUALITY_PRESS' ? 5 : 0;
    const qualityScore = total * 10 + sourceTierBonus;

    // Verification status — tells the user how much to trust this
    const verified = sourceTier === 'OFFICIAL' ? 'VERIFIED' :
                     (sourceTier === 'INDUSTRY' && total >= 3) ? 'CORROBORATED' :
                     'UNVERIFIED';
...
function buildStrategicIdeas(currentData, targetMarkets, existingTenderMarkets) {
  ...
  // Index all items by PRIMARY target market (first country in title wins)
  const marketSignals = {};
  for (const item of allItems) {
    const titleL = (item.title || '').toLowerCase();
    const combined = (titleL + ' ' + (item.text || '')).toLowerCase();
    const matched = targetMarkets.filter(m => combined.includes(m.name.toLowerCase()));
    if (matched.length === 0) continue;

    // Determine primary market — earliest position in title
    let primary = matched[0];
    if (matched.length > 1) {
      let bestPos = Infinity;
      for (const m of matched) {
        const nameL = m.name.toLowerCase();
        const posTitle = titleL.indexOf(nameL);
        const pos = posTitle >= 0 ? posTitle : (combined.indexOf(nameL) + 10000);
        if (pos < bestPos) { bestPos = pos; primary = m; }
      }
    }

    const key = primary.name;
    if (!marketSignals[key]) marketSignals[key] = { market: primary, items: [], stratScore: 0, budgetScore: 0 };
  ...
  for (const ins of explorerInsights) {
    const titleL = ((ins.title || '')).toLowerCase();
    const combined = (titleL + ' ' + (ins.summary || '')).toLowerCase();
    const matched = targetMarkets.filter(m => combined.includes(m.name.toLowerCase()));
    if (matched.length === 0) continue;
    // Primary market — earliest in title
    let primary = matched[0];
    if (matched.length > 1) {
      let bestPos = Infinity;
      for (const m of matched) {
        const pos = titleL.indexOf(m.name.toLowerCase());
        if (pos >= 0 && pos < bestPos) { bestPos = pos; primary = m; }
      }
    }
    const key = primary.name;
    if (!marketSignals[key]) marketSignals[key] = { market: primary, items: [], stratScore: 0, budgetScore: 0 };
...
async function llmStrategicAnalysis(llmProvider, tenders, ideas, currentData) {
  ...
  const context = [
    ...
    '\n=== MARKET CONTEXT (live signals only) ===',
    // R-F2002: removed a hardcoded global-military-spending / top-importers
    // literal that was presented to the model as current market data — it was
    // static, undated, and would silently go stale (a fabrication vector). Only
    // genuinely live values (VIX/energy) belong here.
...
Be specific, but ONLY using facts present in the intelligence above. Name a company, ministry, or person ONLY if it appears in the provided signals — if it does not, say "not in current signals" rather than inventing one. Do NOT fabricate names, numbers, or deals. No generic advice.
Each string field MUST be ≤ 200 characters — concise summaries, not paragraphs.
Output STRICT JSON only (no prose, no markdown, no comments) with this exact shape:
{
  "topPriority":     { "deal": "", "buyer": "", "product": "", "oem": "", "firstAction": "" },
  "strategicMoves":  [ { "move": "", "rationale": "" } ],
  "riskWatch":       [ { "risk": "", "impact": "" } ]
}`;

  try {
    const result = await llmProvider.complete(systemPrompt, context, { maxTokens: 3000, timeout: 60000 });
...
function scoreWinProbability(tender, markets) {
  ...
  // ── 4. DEAL QUALITY SIGNALS (source-tier-aware) ──
  // Official procurement portal URL = real tender, major boost.
  // News article URL = context only, minimal boost.
  const sTier = tender.sourceTier || getSourceTier(tender.url);
  if (sTier === 'OFFICIAL') score += 15;        // Real procurement portal
  else if (sTier === 'INDUSTRY') score += 8;    // Industry press (Jane's, DefenceWeb)
  else if (sTier === 'QUALITY_PRESS') score += 3; // Quality journalism
  else if (tender.url) score += 1;              // Any URL, minimal credit

  if (tender.leadQuality === 'HOT') score += 8;
  else if (tender.leadQuality === 'WARM') score += 4;

  // PENALTY for unverified items — don't inflate win % on news coverage
  if (tender.verified === 'UNVERIFIED') score -= 10;
  ...
  // Priority market boost (reduced from +6 to +3 — signal quality matters more)
  if (marketDef?.priority === 'HIGH') score += 3;
...
async function runAutonomousBrain(llmProvider, tenders, ideas, pipeline, currentData, learningData) {
  ...
  const benchmarks = `
ILLUSTRATIVE REFERENCE RANGES — these are STATIC industry rules-of-thumb, NOT live, NOT verified, and NOT specific to any current deal. Use only for rough order-of-magnitude sizing, and label any figure derived from them as an estimate. NEVER present these as confirmed deal values or verified sales.
CONTRACT VALUE RANGES (rough order-of-magnitude only):
...
OEM EXPORT PATTERNS (general industry knowledge, NOT deal-specific or verified — do not cite as a confirmed sale):
...
${Object.entries(adaptiveWeights.regionMultipliers).map(([r, m]) => `  ${r}: ${m}x multiplier`).join('\n')}`
  ...
  const systemPrompt = `You are ARIA — the autonomous BD intelligence brain of Arkmurus, a specialist defence brokering firm.
...
CRITICAL — NO FABRICATION (overrides everything below): every buyer, ministry, OEM, person, product, value, and deadline you state MUST be present in, or directly inferable from, the CURRENT INTELLIGENCE provided below. If a fact is not supported by the provided signals, write "not in current signals" for that field and lower the lead to WARM/COLD — do NOT invent a ministry name, company, dollar figure, or date to make a lead look actionable. An honest "insufficient signal" is REQUIRED over a confident fabrication. Any dollar figure you give must be flagged as an estimate (e.g. "~$X-YM, est.") and may only use the illustrative ranges below for rough sizing.

LEAD QUALIFICATION RULES — a lead is only "actionable" (HOT) if ALL of these are GROUNDED in the provided signals:
1. SPECIFIC BUYER — the buying ministry/directorate as named in a signal (do NOT invent one; if only the country is known, the lead is at most WARM)
2. ESTIMATED VALUE — only if a value/budget signal exists or an illustrative range clearly applies; label it "est."
3. PRODUCT MATCH — 1-2 products supported by the signal text
4. OEM RECOMMENDATION — a real OEM whose fit is supported by the signal (general export patterns are context, not a confirmed match)
5. FIRST CONTACT — a specific, lawful 48-hour action
6. TIMING — why NOW, citing the signal that drives it
If any of the 6 is not grounded, classify WARM or COLD, never HOT — and never fill the gap by inventing.
${benchmarks}
...
{"weeklyPriority":{"action":"string","market":"string","whyNow":"string","firstStep":"string"},"salesLeads":[{"market":"string","lead":"string","type":"TENDER or STRATEGIC or PROACTIVE","estimatedValue":"string","procurementAuthority":"string","oemRecommendation":"string","urgency":"HOT or WARM or COLD","nextStep":"string"}],"actionPlan":[{"deal":"string","market":"string","steps":["string"],"deadline":"string","winProb":50}],"knowledgeGaps":[{"question":"string","howToFind":"string","urgency":"high or medium or low"}],"marketIntelligence":[{"market":"string","signal":"string","recommendation":"string"}],"competitiveThreats":[{"market":"string","threat":"string","counter":"string"}],"selfLearning":{"patternsObserved":["string"],"strategyAdjustment":"string","nextSweepFocus":"string"},"confidence":0.7}`;
  ...
  try {
    const raw = await llmProvider.complete(
      ...
      { maxTokens: 4000, temperature: 0.12, timeout: 120000 }  // 120s — BD Brain prompt is large
    );
    _rawText = (typeof raw === 'string' ? raw : raw?.text || '');
    const parsed = parseLlmJson(_rawText);
    ...
    if (_rawText) {
      console.warn('[BD Brain] Raw response start:', JSON.stringify(_rawText.substring(0, 300)));
      // 2026-04-26: when the parse error reports a position, dump the
      // surrounding window so we can root-cause the failing token instead of
      // guessing. parseLlmJson re-throws as `JSON parse failed after 5
      // repair strategies: <inner>` where <inner> includes "at position N".
      const posMatch = err.message.match(/position (\d+)/);
      if (posMatch) {
        const pos = parseInt(posMatch[1], 10);
        const start = Math.max(0, pos - 120);
        const end = Math.min(_rawText.length, pos + 120);
        console.warn(
          `[BD Brain] Raw response window @ position ${pos} `
          + `(len=${_rawText.length}): ${JSON.stringify(_rawText.substring(start, end))}`
        );
      }
    }
    return null;
...
// ── Main entry point ──────────────────────────────────────────────────────────
const _BD_LOCK_KEY = 'crucix:bd:run_lock';
const _BD_LOCK_TTL_S = 180;  // matches per-task timeout_seconds in tasks.yaml

export async function runBDIntelligence(currentData, targetMarkets, llmProvider = null) {
  // H7: cross-task advisory lock. Multiple staggered autonomous crons
  // (DAILY-PROC-ANGOLA at 06:00, DAILY-LUSO-MOZAMBIQUE at 06:10, ...)
  // were reading bd_pipeline.json / bd_strategy.json, mutating in
  // memory, and writing — the second writer overwrote the first
  // writer's new tenders. Redis SET NX EX serialises the critical
  // section. If Redis is unreachable the lock fails closed and the
  // caller gets a skipped result (safer than concurrent overwrites).
  const _locked = await redisLockAcquire(_BD_LOCK_KEY, _BD_LOCK_TTL_S);
  if (!_locked) {
    return {
      tenders: [],
      ideas: [],
      ...
      skipped: 'bd_run_lock_held',
      counts: { activeTenders: 0, contractAwards: 0, strategicIdeas: 0, pipelineDeals: 0 },
    };
  }
  try {
  ...
  // Brain hook: feed BD intelligence run to ARIA's learning system
  brainAbsorb({
    module: 'signal_generator',
    summary: `BD intel run: ${result.counts?.tenders || 0} tenders, ${result.counts?.strategicIdeas || 0} ideas, ${result.counts?.pipelineDeals || 0} pipeline deals, brain=${!!result.brain}`,
    detail: (result.tenders || []).slice(0, 5).map(t => `${t.title || t.market}: ${t.type}`).join('; '),
    extra_topics: ['finance', 'compliance', 'market_intel', 'procurement'],
    confidence: 'ASSESSED',
  });

  return result;
  } finally {
    await redisLockRelease(_BD_LOCK_KEY);
  }
}
