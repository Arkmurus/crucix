...
// Political / opinion / news-coverage domains — NOT procurement portals
const NON_PROCUREMENT_DOMAINS = [
  'da.org.za', 'anc.org.za', 'eff.org.za', 'acdp.org.za',
  'dailymaverick.co.za', 'news24.com', 'timeslive.co.za', 'businessday.co.za',
  'dailymail.co.uk', 'theguardian.com', 'reuters.com', 'bbc.com', 'bbc.co.uk',
  'aljazeera.com', 'bloomberg.com', 'politico.com', 'defenceweb.co.za',
  'breakingdefense.com', 'defensenews.com', 'janes.com',
];
...
function isNegativeCoverage(text, url) {
  ...
  if (NON_PROCUREMENT_DOMAINS.includes(domain)) return true;
...
Active tenders: ${JSON.stringify(topTenders)}
...
Global context: VIX ${currentData.fred?.find(f=>f.id==='VIXCLS')?.value||'N/A'}, Brent $${currentData.energy?.brent||'N/A'}

TASK: Produce a complete autonomous assessment. Think step by step before answering.
...
Respond ONLY with valid JSON:
{
  ...
  try {
    const raw = await llmProvider.complete(systemPrompt, { maxTokens: 2000, temperature: 0.15 });
    let text = (typeof raw === 'string' ? raw : raw?.text || '').trim();
    if (text.startsWith('```')) text = text.replace(/^```(?:json)?\n?/, '').replace(/\n?```$/, '');
    const result = JSON.parse(text);
    // Store the brain's reasoning for learning
    const L = getlearning();
    L.lastThought = { ts: new Date().toISOString(), result };
    saveLearning(L);
    return result;
  } catch (err) {
