...
function extractRealTenders(procurementTenders, targetMarkets) {
  ...
  for (const item of allUpdates) {
    ...
    // Match to PRIMARY target market (first country in title wins).
    // IMPORTANT: ISO2 codes must be word-boundary matched — "NG" inside
    // "pushing" is NOT Nigeria. Use regex \bXX\b for ISO2.
    const titleLower = (item.title || '').toLowerCase();
    const matched = targetMarkets.filter(m => {
      if (text.includes(m.name.toLowerCase())) return true;
      // ISO2 word-boundary match only (prevents "ng" matching in "pushing")
      if (m.iso2) {
        const iso2Re = new RegExp(`\\b${m.iso2.toLowerCase()}\\b`);
        if (iso2Re.test(text)) return true;
      }
      return false;
    });
    let market = matched[0] || null;
    if (matched.length > 1) {
      let bestPos = Infinity;
      for (const m of matched) {
        const nameL = m.name.toLowerCase();
        const posTitle = titleLower.indexOf(nameL);
        const pos = posTitle >= 0 ? posTitle : (text.indexOf(nameL) + 10000);
        if (pos < bestPos) { bestPos = pos; market = m; }
      }
    }
    ...
    // Score: reduce market bias, increase signal quality weight.
    // Source tier bonus replaces the blunt Lusophone/priority inflation.
    const sourceTierBonus = sourceTier === 'OFFICIAL' ? 20 : sourceTier === 'INDUSTRY' ? 10 : sourceTier === 'QUALITY_PRESS' ? 5 : 0;
    const marketBonus = (market?.lusophone ? 8 : 0) + (market?.priority === 'HIGH' ? 5 : 0);
    const qualityScore = total * 10 + sourceTierBonus + marketBonus;
