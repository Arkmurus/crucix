...
// Tolerant JSON parser — strips trailing commas, code fences, and common LLM artifacts
function parseLlmJson(text) {
  ...
  s = s.replace(/,(\s*[}\]])/g, '$1');
  // Strip single-line JS comments (DeepSeek sometimes adds // comments in JSON)
  s = s.replace(/\/\/[^\n]*/g, '');
  ...
  try {
    ...
    catch (e) { throw new Error('JSON parse failed after cleanup: ' + e.message); }
  }
}
...
function buildStrategicIdeas(currentData, targetMarkets, existingTenderMarkets) {
  ...
  // Defence news
  for (const d of (currentData.defenseNews?.updates || currentData.defenseNews || [])) {
    allItems.push({ title: d.title || '', text: d.content || d.description || d.summary || '', url: d.url || d.link || '', src: d.source || 'DefenseNews' });
  }
  // Defence events
  for (const e of (currentData.defenseEvents?.updates || currentData.defenseEvents || [])) {
