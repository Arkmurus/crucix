...
import { savePatterns, brainAbsorb } from './learning_store.mjs';
...
export async function analyzePatterns(llmProvider = null) {
  ...
  // Brain hook: feed pattern analysis to ARIA's learning system
  brainAbsorb({
    module: 'signal_generator',
    summary: `Pattern analysis: ${merged.length} patterns (${rulePatterns.length} rules, ${llmPatterns.length} LLM) from ${runs.length} runs`,
    detail: merged.slice(0, 5).map(p => `${p.id}: ${p.description || p.type || ''}`).join('; '),
