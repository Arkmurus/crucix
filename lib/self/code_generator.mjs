...
import { redisGet, redisSet, redisDel } from '../persist/store.mjs';

// R-F16 2026-05-01: staged modules used to live only on disk under
// /usr/src/app/runs/staged/, which is container-ephemeral on seenode —
// every restart wiped pending operator-review modules. Mirror the
// staging area into Redis under one key (JSON map of moduleName →
// { code, meta }) so:
//   1. on stageModule(), Redis is updated alongside disk
//   2. on hydrateStagedFromRedis(), a fresh container reconstructs disk
//      from Redis at startup
//   3. on discard, Redis entry is removed
const STAGED_REDIS_KEY = 'crucix:staged_modules';
const STAGED_REDIS_TTL_DAYS = 90;

// ICU-free Europe/London timestamp
...
export function stageModule(moduleName, code, metadata = {}) {
  ...
  // R-F16: mirror to Redis so a container restart doesn't wipe pending
  // review work. Fire-and-forget — disk is the source of truth for the
  // running process; Redis is the persistence backstop for next startup.
  (async () => {
    try {
      const raw = await redisGet(STAGED_REDIS_KEY);
      const map = raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : {};
      map[moduleName] = { code, meta };
      await redisSet(STAGED_REDIS_KEY, JSON.stringify(map), STAGED_REDIS_TTL_DAYS * 24 * 3600);
    } catch (err) {
      console.warn(`[CodeGenerator] Redis mirror failed for ${moduleName}: ${err.message}`);
    }
  })();
...
// R-F16: rebuild disk staging area from Redis on startup. Call this
// from server boot before the autonomous loop kicks in. Safe to call
// multiple times — only writes files that don't already exist on disk.
export async function hydrateStagedFromRedis() {
  ensureStaging();
  let map;
  try {
    const raw = await redisGet(STAGED_REDIS_KEY);
    if (!raw) return { hydrated: 0 };
    map = typeof raw === 'string' ? JSON.parse(raw) : raw;
  } catch (err) {
    console.warn(`[CodeGenerator] Redis hydrate read failed: ${err.message}`);
    return { hydrated: 0, error: err.message };
  }
  if (!map || typeof map !== 'object') return { hydrated: 0 };
  let hydrated = 0;
  for (const [moduleName, entry] of Object.entries(map)) {
    const stagePath = join(STAGING_DIR, `${moduleName}.mjs.staged`);
    if (existsSync(stagePath)) continue;  // disk already has it; trust disk
    if (!entry || !entry.code) continue;
    try {
      writeFileSync(stagePath, entry.code, 'utf8');
      writeFileSync(stagePath + '.meta.json', JSON.stringify(entry.meta || {}, null, 2), 'utf8');
      hydrated++;
    } catch (err) {
      console.warn(`[CodeGenerator] Redis hydrate write failed for ${moduleName}: ${err.message}`);
    }
  }
  if (hydrated > 0) {
    console.log(`[CodeGenerator] Hydrated ${hydrated} staged module(s) from Redis`);
  }
  return { hydrated };
}

// R-F16: clear a module's Redis entry. Called from the discard
// endpoint so a discarded module doesn't get re-hydrated on next boot.
export async function discardStagedFromRedis(moduleName) {
  try {
    const raw = await redisGet(STAGED_REDIS_KEY);
    if (!raw) return;
    const map = typeof raw === 'string' ? JSON.parse(raw) : raw;
    if (!map || typeof map !== 'object' || !(moduleName in map)) return;
    delete map[moduleName];
    if (Object.keys(map).length === 0) {
      await redisDel(STAGED_REDIS_KEY);
    } else {
      await redisSet(STAGED_REDIS_KEY, JSON.stringify(map), STAGED_REDIS_TTL_DAYS * 24 * 3600);
    }
  } catch (err) {
    console.warn(`[CodeGenerator] Redis discard failed for ${moduleName}: ${err.message}`);
  }
}
...
export function formatStagedForTelegram(staged) {
  ...
  for (const s of staged) {
    ...
    msg += `  Staged: ${s.stagedAt ? londonTs(new Date(s.stagedAt)) : 'unknown'} London\n\n`;
  }
