...
export async function redisSet(key, value, ttlSeconds = 90 * 24 * 3600) {
  if (!redisConfigured()) return;
  try {
    // Upstash REST: POST / with body ["SET", key, value, "EX", seconds]
    // Using the generic command endpoint — works reliably with all value types.
    // Default TTL = 90 days (long-running BD state). Pass a smaller TTL for
    // short-lived caches (e.g. RSS proxy fallback caches at 30 min).
...
// Acquire a short-lived advisory lock using Redis SET NX EX. Returns true
// if the caller now owns the lock, false if another holder has it.
// Used by BD sweep tasks to prevent concurrent writes from staggered crons
// overwriting each other's pipeline JSON files.
export async function redisLockAcquire(key, ttlSeconds = 60) {
  if (!redisConfigured()) {
    // Fail-CLOSED when Redis is down — caller should skip the
    // critical section rather than risk concurrent writes.
    return false;
  }
  try {
    const res = await fetch(`${REDIS_URL()}`, {
      ...
      body: JSON.stringify(['SET', key, '1', 'NX', 'EX', String(Math.max(5, ttlSeconds))]),
      signal: AbortSignal.timeout(5000),
    });
    if (!res.ok) return false;
    const data = await res.json();
    // Upstash returns {result: "OK"} on success, {result: null} when NX blocked
...
export async function redisPush(key, value) {
  if (!redisConfigured()) return;
  try {
    // Upstash REST: POST / with body ["RPUSH", key, value]
    await fetch(`${REDIS_URL()}`, {
      method:  'POST',
      headers: {
        Authorization:  `Bearer ${REDIS_TOKEN()}`,
