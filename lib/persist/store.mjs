  ...
  async init() {
    if (existsSync(this.path)) {
      try {
        ...
        // R-F830 (2026-05-23): DO NOT silently start fresh on corrupt
        // JSON. For critical stores (users.json, sessions.json) the
        // pre-R-F830 behaviour wiped every account / session on the
        // first parse error — silent total data loss.
        //
        // New policy:
        //   1. Quarantine the corrupt file to <path>.corrupt-<ts> so
        //      forensics can recover it.
        //   2. THROW from init() so boot fails loudly. The operator
        //      then either restores from snapshot (Fly volume daily
        //      snapshots) or sets PERSIST_ALLOW_ZERO=1 to accept the
        //      reset.
        //
        // Opt-out via env var for non-critical stores or first-boot:
        //   PERSIST_ALLOW_ZERO=1  → preserve the old behaviour
        const allowZero = (process.env.PERSIST_ALLOW_ZERO || '').toLowerCase() === '1';
        try {
          const ts = new Date().toISOString().replace(/[:.]/g, '-');
          const quarantine = `${this.path}.corrupt-${ts}`;
          renameSync(this.path, quarantine);
          console.error(
            `[Persist] ${this.key}: file CORRUPT — quarantined to ${quarantine}. `
            + `Parse error: ${parseErr.message}. `
            + (allowZero
                ? 'PERSIST_ALLOW_ZERO=1 set → starting fresh.'
                : 'Refusing to silently zero the store (set PERSIST_ALLOW_ZERO=1 to override; or restore from Fly volume snapshot).'),
          );
        } catch (renameErr) {
          console.error(
            `[Persist] ${this.key}: corrupt + couldn't quarantine: ${renameErr.message}`,
          );
        }
        if (!allowZero) {
          throw new Error(
            `[Persist] ${this.key}: corrupt file at ${this.path} (R-F830 halt-don't-zero). `
            + `Set PERSIST_ALLOW_ZERO=1 or restore from snapshot.`,
          );
        }
      }
    }
