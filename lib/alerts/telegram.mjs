...
export class TelegramAlerter {
  constructor({ botToken, chatId, port = 3117 }) {
    ...
    // Persist lastAlertTime to disk so server restarts don't break the 3-hour gap
  ...
  _saveLastAlertTime(t) {
    try {
      writeFileSync(this._alertTimePath(), JSON.stringify({ t }), 'utf8');
    } catch {}
  }
    ...
    if (message.length <= MAX_LENGTH) {
      try {
        ...
        if (!res.ok) {
          ...
          console.error(`[Telegram] Send failed (HTTP ${res.status}):`, errText);
  ...
  async onSweepComplete(data) {
    ...
    // Rate-limited to 3/hour. Does NOT mark as seen — that happens in the
    // digest dedup below so the signal still appears in the next 3-hour digest.
    ...
    // ── 3-hour cadence check — BEFORE dedup ──────────────────────────────────
    // Critical: dedup must run AFTER this gate. If dedup ran before, every
    // signal found between digests would be marked as seen and silently dropped,
    // leaving the 3-hour digest empty even when new intel was available.
    ...
    // Runs here (post-gate) so signals accumulate across sweeps and are
    // included in the digest once the 3-hour window opens.
    let newSignals;
    try {
    ...
    msg += `━━━━━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `_For full analysis: /brief · /full · /osint · /supply · /predict_`;
