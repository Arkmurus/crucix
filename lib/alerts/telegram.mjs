...
export class TelegramAlerter {
  ...
  async _pollUpdates() {
    ...
    try {
      ...
      for (const update of data.result) {
        ...
        // in the linked discussion group (or DM the bot) with SCREEN/[COUNTRY]/PRO/
        // DEMO/HELP — non-slash keywords from NON-operator users, which the operator
        // allow-list gate below would drop. Handle recognised keywords here first
        // (rate-limited, cheap — SCREEN hits the sanctions API, not the LLM). Falls
        // through unchanged for non-keyword messages from non-allowed users.
  ...
  async _handleChannelKeyword(msg, chatId, userId) {
    try {
      const text = (msg.text || '').trim();
      if (!text || text.startsWith('/')) return false;  // slash = operator command
      // Only from the channel's linked discussion group OR a private DM.
      const discussionId = process.env.TELEGRAM_CHANNEL_DISCUSSION_ID || '';
      const isDiscussion = discussionId && String(chatId) === String(discussionId);
      const isDM = msg.chat?.type === 'private';
      if (!isDiscussion && !isDM) return false;

      const { parseReply } = await import('../telegram/replyKeywordRouter.mjs');
      const parsed = parseReply(text);
      // Answer ONLY explicit conversion keywords — ignore chatter so the bot
      // never spams the discussion group.
      ...
      const resp = await handleReply(text, userId);
      if (resp?.text) {
