...
export class TelegramAlerter {
  ...
  async _sendOEMPage(chatId, results, query, offset) {
    ...
    for (const oem of slice) {
      text += formatOEMCard(oem, false) + '\n\n';
    }
    text += `_Card ${offset + 1}–${Math.min(offset + PAGE, total)} of ${total}_`;

    const navRow = [];
    if (offset > 0)            navRow.push({ text: '← Prev', callback_data: `oem:${offset - PAGE}:${query}` });
    if (offset + PAGE < total) navRow.push({ text: 'Next 2 →', callback_data: `oem:${offset + PAGE}:${query}` });
