...
export class TelegramAlerter {
  ...
  async _handlePredict(query) {
    try {
      const data = await this._getCachedData();
      const markets = data?.polymarket?.markets || data?.sources?.Polymarket?.markets || [];
  ...
  async evaluateAndAlertWithHistory(previousData, currentData) {
    try {
      ...
      if (timeSinceLastAlert < this._alertIntervalMs) {
        ...
        console.log(`[Telegram] Auto-alert suppressed — next in ${minsLeft} min`);
        return false;
      }

      const prevTexts = (previousData?.tg?.urgent || []).map(s => (s.text || s).substring(0, 50)).join('|');
      const newSignals = (currentData?.tg?.urgent || []).filter(s => !prevTexts.includes((s.text || s).substring(0, 50)));
      const prevCritical = (previousData?.correlations || []).filter(c => c.severity === 'critical').length;
      const currCritical = (currentData?.correlations || []).filter(c => c.severity === 'critical').length;

      if (newSignals.length === 0 && currCritical <= prevCritical) {
        console.log('[Telegram] Auto-alert suppressed — no new intelligence since last alert');
        return false;
      }

      console.log(`[Telegram] Sending alert — ${newSignals.length} new signals, ${currCritical} critical correlations`);
  ...
  async _handlePredict(query) {
    try {
      ...
      const markets = data?.polymarket?.markets || data?.sources?.Polymarket?.markets || [];
      if (!query || !query.trim()) {
        if (!markets.length) return 'No prediction market data yet. Wait for next sweep.';
        let msg = 'PREDICTION MARKETS

';
        for (const m of markets.slice(0, 8)) {
          const label = m.yesProb >= 70 ? 'HIGH' : m.yesProb <= 30 ? 'LOW' : 'MID';
          const vol = m.volume24h >= 1000000 ? (m.volume24h/1000000).toFixed(1)+'M' : m.volume24h >= 1000 ? Math.round(m.volume24h/1000)+'K' : m.volume24h;
          msg += m.yesProb + '% YES [' + label + '] ' + m.question + '
';
          msg += '   Vol: 
...
';
      ...
      const q = query.toLowerCase();
      const matches = markets.filter(m => m.question.toLowerCase().includes(q));
      if (!matches.length) return 'No markets found for  + query + .';
      let msg = 'MARKETS: ' + query.toUpperCase() + '

';
      for (const m of matches.slice(0, 5)) {
        const vol = m.volume24h >= 1000 ? Math.round(m.volume24h/1000)+'K' : m.volume24h;
        msg += m.yesProb + '% YES — ' + m.question + '
';
        msg += '   Vol 24h: 
    this._commandHandlers[command.toLowerCase()] = handler;
...
';
      ...
      return msg;
    } catch (e) { return 'Predict error: ' + e.message; }
  }

  async _handleRisk(query) {
    if (!query?.trim()) return 'COUNTERPARTY RISK SCORING
...
• Cyber exposure (15%)';
    try {
      const { handleRiskCommand } = await import('../apis/sources/counterparty_risk.mjs');
      return await handleRiskCommand(query);
    } catch(e) { return 'Risk scoring unavailable: ' + e.message; }
  }
