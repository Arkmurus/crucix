...
export class TelegramAlerter {
  ...
  async onSweepComplete(data) {
    ...
    // Critical signals — full text
    if (criticals.length > 0) {
      msg += `🚨 *CRITICAL INTELLIGENCE (${criticals.length})*\n`;
      msg += `━━━━━━━━━━━━━━━━━\n`;
      for (const s of criticals.slice(0, 4)) {
        const text = s.text.trim().replace(/\n+/g, ' ');
    ...
    // High signals
    if (highs.length > 0) {
      msg += `⚠️ *HIGH PRIORITY (${highs.length})*\n`;
      msg += `━━━━━━━━━━━━━━━━━\n`;
      for (const s of highs.slice(0, 4)) {
        const text = s.text.trim().replace(/\n+/g, ' ');
