...
export class TelegramAlerter {
  ...
  async evaluateAndAlert(llmProvider, delta, memory, context = {}) {
    try {
      ...
      let message = `*CRUCIX INTELLIGENCE UPDATE*\n`;
      message += `_${new Date().toISOString().slice(0, 19).replace('T', ' ')} UTC_\n\n`;

      // Direction / delta summary
      if (delta?.summary) {
        const dir = { 'risk-off': '📉', 'risk-on': '📈', 'mixed': '↔️' }[delta.summary.direction] || '↔️';
        message += `${dir} *Direction:* ${delta.summary.direction.toUpperCase()}`;
        message += ` | ${delta.summary.totalChanges} changes`;
        if (delta.summary.criticalChanges > 0) message += ` | *${delta.summary.criticalChanges} critical*`;
        message += `\n\n`;
      }
      ...
      if (vix || oil.wti) {
        message += `📊 *Markets*\n`;
        message += `VIX: ${vix?.value || '--'} | WTI: $${oil.wti || '--'} | Brent: $${oil.brent || '--'}`;
        if (oil.natgas) message += ` | NatGas: $${oil.natgas}`;
        message += `\n\n`;
      }
      ...
      if (urgent.length > 0) {
        ...
        for (const s of urgent.slice(0, 3)) {
          ...
          const text = (s.text || '').substring(0, 220);
          message += `• ${ch}${text}\n\n`;
        }
        if (urgent.length > 3) message += `_...and ${urgent.length - 3} more. Use /osint for all._\n\n`;
      }

      // Latest news headlines
      if (newsFeed.length > 0) {
        message += `📰 *Latest Feed*\n`;
        for (const n of newsFeed.slice(0, 3)) {
          message += `• ${n.headline} _(${n.source})_\n`;
        }
        message += `\n`;
      }

      // Defense contracts if any
      if (contracts.length > 0) {
        message += `🔒 *Defense Contracts (${contracts.length})*\n`;
      ...
      message += `_/brief · /osint · /full · /sweep_`;

      const result = await this.sendMessage(message);
      if (result.ok) {
        this._addAlert('sweep', `Alert sent: ${urgent.length} OSINT signals, ${newsFeed.length} news items`, 'high');
