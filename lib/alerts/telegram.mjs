...
export class TelegramAlerter {
  ...
  async _handleOEM(query) {
    try {
      const { searchOEMs, formatOEMCard, formatOEMList, OEM_DATABASE } = await import('../intel/oem_db.mjs');

      // No query → full directory grouped by country
      if (!query?.trim()) {
        return formatOEMList(OEM_DATABASE);
      }
      ...
      if (results.length === 1) {
        const oem = results[0];
        let msg = formatOEMCard(oem, false);
        msg += `\n*Description:*\n${oem.description}`;
        if (oem.contact?.address) msg += `\n\n📮 ${oem.contact.address}`;
        return msg;
      }

      // Multiple results → compact list
      if (results.length <= 5) {
        let msg = `🏭 *OEM RESULTS for "${q}"* — ${results.length} found\n━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
        for (const oem of results) {
          msg += formatOEMCard(oem, false) + '\n';
        }
        return msg;
      }

      // Many results → directory view
      return formatOEMList(results, `OEM RESULTS — "${q}"`);
    } catch (err) {
  ...
  async onSweepComplete(data) {
    ...
    for (const s of candidates) {
      ...
      // Normalise whitespace before dedup key — collapses \n\n and double-spaces
      const textKey = text.toLowerCase().replace(/\s+/g, ' ').trim().substring(0, 120);
      ...
      try {
        ...
        if (newUrgent.length > 0) {
          ...
          for (const s of newUrgent) {
            ...
            const displayText = smartTruncate(s.text.trim().replace(/\n+/g, ' '), 380);
            urgentMsg += `${icon} *[${s.source}]* ${displayText}\n\n`;
          }
