...
export class TelegramAlerter {
   ...
   */
  /**
   * R-F3617 — ONE dispatch decision, shared by BOTH transports.
   *
   * Telegram delivers updates by polling OR by webhook, never both. This routing —
   * "is the sender the operator, or a member of a public surface, or nobody?" — used
   * to live INLINE in the polling loop, and `/webhook` (server.mjs) called
   * `_handleMessage` directly. So the webhook silently skipped `_handleChannelKeyword`
   * entirely: switch this deployment to a webhook and R-F3610 comes straight back
   * through the other door, with every subscriber reply dropped again.
   *
   * It is dormant today — getWebhookInfo reports no webhook set, and
   * TELEGRAM_WEBHOOK_SECRET is unset so the endpoint refuses everything in production
   * — which is exactly why it would not have been noticed. This is the CLAUDE.md §13
   * stream-bypass rule (aria_chat / aria_chat_stream): when one surface is a fork of
   * another, a hook added to one must exist in both. The durable fix is to remove the
   * fork, so there is nothing to keep in sync.
   *
   * Security is UNCHANGED and still enforced twice: the allow-list below decides who
   * reaches `_handleMessage`, and `_handleMessage` re-checks it independently
   * (R-F1821) so neither transport can be the weak one.
   *
   * @returns {Promise<{handled: boolean, via?: string, reason?: string}>}
   */
  async dispatchMessage(msg) {
    if (!msg?.text) return { handled: false, reason: 'no_text' };
    ...
    // R-F2302 — PUBLIC reply-keyword conversion path. Channel subscribers reply in
    // the public channel/supergroup, its linked discussion group, or a DM (R-F3610)
    // with SCREEN/[COUNTRY]/PRO/DEMO/HELP — non-slash keywords from NON-operator
    // users, which the operator allow-list below would drop. Handle recognised
    // keywords first (rate-limited, cheap — SCREEN hits the sanctions API, not the
    // LLM). Falls through for non-keyword messages from non-allowed users.
    if (!isGroup && !isWhitelistedUser) {
      try {
        if (await this._handleChannelKeyword(msg, chatId, userId)) {
          return { handled: true, via: 'channel_keyword' };
        }
      } catch (kwErr) { console.warn('[Telegram] channel keyword path error:', kwErr.message); }

      // R-F3610 — this drop used to be completely silent, which is how a dead public
      // reply surface went unnoticed: nothing logged, no outcome recorded, so "no one
      // ever replies" and "every reply is discarded" looked identical. Log ONCE PER
      // CHAT so a mis-set destination is discoverable without turning ordinary channel
      // chatter into log spam.
      //
      // Only for an UNRECOGNISED chat. A recognised surface reaching here just means
      // the keyword handler declined this particular message (chatter, a slash
      // command, another bot) — that is normal and correctly silent. Warning on it
      // would blame the destination config, which is already right.
      const knownSurface = this._publicKeywordSurface(chatId, msg);
      this._droppedChatsSeen ??= new Set();
      if (!knownSurface && !this._droppedChatsSeen.has(chatId)) {
      ...
      return { handled: false, reason: knownSurface ? 'declined_by_keyword_router' : 'sender_not_allowed' };
    }

    await this._handleMessage(msg);
    return { handled: true, via: 'operator_command' };
  }
