...
export class HybridProvider extends LLMProvider {
  constructor(config) {
    ...
    // Cost/usage tracking
    this.stats = {
      deepseekCalls: 0, ollamaCalls: 0, fallbackCalls: 0,
      deepseekTokens: 0, ollamaTokens: 0,
  ...
  async complete(systemPrompt, userMessage, opts = {}) {
    ...
    if (useDeepSeek) {
      try {
        ...
        this.stats.deepseekTokens += (result.usage?.inputTokens || 0) + (result.usage?.outputTokens || 0);
    ...
    if (ollamaReady) {
      try {
        ...
        this.stats.ollamaTokens += (result.usage?.inputTokens || 0) + (result.usage?.outputTokens || 0);
   ...
   */
  getStats() { return { ...this.stats, totalCalls: this.stats.deepseekCalls + this.stats.ollamaCalls + this.stats.fallbackCalls }; }
