...
export async function pushToAll(payload, filter = null) {
  const wp = await getWebPush();
  if (!wp) { console.debug('[Push] web-push not available — skipping'); return { sent: 0, total: 0, reason: 'web-push not installed' }; }
  if (!vapidKeys) { console.debug('[Push] VAPID keys not configured — skipping'); return { sent: 0, total: 0, reason: 'no VAPID keys' }; }

  const subs = loadSubs();
  const entries = Object.values(subs).filter(e => filter ? filter(e) : true);
  if (entries.length === 0) {
    // 2026-04-12: log clearly WHY 0/0 happens — no browser clients subscribed
    console.log(`[Push] pushToAll — 0/0 delivered (no subscriptions registered. Users must enable push in browser.)`);
    return { sent: 0, total: 0, reason: 'no subscriptions' };
  }
  ...
  return { sent, total: entries.length, failed: failed.length };
}
