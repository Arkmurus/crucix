...
// R-F2562 (#9) — web push is a DEPRECATED, redundant alert channel: there is NO
// browser subscription flow in the UI (0 subscribers is structural, not a bug), and
// every alert already fans out via Telegram / email / WhatsApp. It is OFF unless
// PUSH_ENABLED is explicitly set, so a send is a quiet no-op — NOT a scary
// "CRITICAL alert NOT delivered by push" warning on every flash. To revive push,
// set PUSH_ENABLED=1 AND build the missing service-worker + subscribe frontend.
const PUSH_ENABLED = ['1', 'true', 'yes', 'on'].includes(
  (process.env.PUSH_ENABLED || '').trim().toLowerCase(),
);

export async function pushToUser(userId, payload) {
  if (!PUSH_ENABLED) return { sent: false, reason: 'push_deprecated' };
  const wp = await getWebPush();
...
export async function pushToAll(payload, filter = null, opts = {}) {
  // R-F2562 (#9) — deprecated channel: quiet no-op (alerts go via Telegram/email/WA).
  if (!PUSH_ENABLED) return { sent: 0, total: 0, subscribers: 0, reason: 'push_deprecated' };
  const wp = await getWebPush();
