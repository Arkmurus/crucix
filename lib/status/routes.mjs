...
export function createStatusRouter({ requireAdmin, getBrainBridgeVerdict }) {
  ...
  router.get('/', (req, res) => {
    ...
    //
    // R-F975 (2026-05-28): "operational" here means SERVICE AVAILABILITY only —
    // it reflects filed incidents + brain-bridge reachability, NOT intelligence
    // quality (composite / grounding / source-health). A green "All systems
    // operational" banner can be misread as overall quality; the `measures`
    // fields in the response make that scope explicit to every consumer.
    let overall = 'operational';
    let summary = 'No active incidents — service availability nominal.';
    if (open.length) {
    ...
    res.json({
      overall,
      summary,
      // R-F975: explicit scope so "operational" is never read as a quality claim.
      measures: 'service_availability',
      measuresNote: 'Availability only (incidents + brain-bridge reachability). Not a measure of intelligence quality — see /aria-brain for composite, grounding and source-health.',
