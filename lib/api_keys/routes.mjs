...
// ── R-F3150: ONE budget path for every protocol that accepts a crx_ key ────
//
// Extracted from the /api/v1 middleware because MCP (R-F3140) authenticated a
// key and then called tools WITHOUT either of these checks — so an MCP client
// paid no per-minute rate limit and consumed no daily quota, while the exact
// same operation over /api/v1 did both. That asymmetry is the worse half of a
// rate limiter: the surface most likely to hammer it was the one exempt from
// it, because an MCP client is typically an LLM in a loop rather than a human
// clicking.
//
// Both layers are kept and both still matter: the per-minute cap shapes the
// curve, the daily quota is the actual budget. Returns
// {allowed, reason} — never throws, so a quota-store failure cannot take the
// API down.
export async function consumeApiKeyBudget(keyRecord, user) {
  if (!await _perKeyRateOk(keyRecord.id)) {
    return { allowed: false, reason: 'rate limit exceeded — 60 req/min per key' };
  }
  const verdict = await checkAndConsume(
    user.id, user.tier || DEFAULT_TIER, 'message');
  if (!verdict.allowed) {
