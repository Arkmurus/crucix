...
// The browser hashes the script text produced by the HTML parser. The parser's
// preprocessing stage normalises CRLF and bare CR to LF before CSP evaluates
// the block. We therefore scan at boot for the files actually being served and
// apply that same newline normalisation before hashing.
//
// ── FAILURE MODE, STATED ─────────────────────────────────────────────────────
...
export function sha256Source(bodyLatin1) {
  // HTML parsing normalises both CRLF and bare CR before the browser evaluates
  // CSP hashes. Hashing the raw Windows checkout bytes instead blocks the
  // script even though the server's policy appears to contain its hash.
  const browserText = bodyLatin1.replace(/\r\n?/g, '\n');
  const digest = createHash('sha256').update(Buffer.from(browserText, 'latin1')).digest('base64');
