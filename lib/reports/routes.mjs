...
export function createReportsRouter({ requireAuth, findUserById, brainBaseUrl, brainInternalToken }) {
  ...
  router.post('/pdf', requireAuth, async (req, res) => {
    ...
    // R-F742 (2026-05-20) — must match the frontend slug derivation
    // (aria.html:638). Prefer EMAIL > USERNAME > id so the guard works
    // across seenode redeploys that regenerate user.id but keep email
    // stable. Without this match, every PDF export after a deploy
    // would 403 even for the legitimate owner.
