...
function _verdictColour(v) {
  return _VERDICT_COLOURS[String(v || '').toUpperCase()] || MUTE;
}
...
function addVerdictBlock(doc, risk, readiness) {
  const v = String(risk || 'UNKNOWN').toUpperCase();
  const col = _verdictColour(v);
  ...
  doc.roundedRect(MARGIN + 12, y + 14, 96, 34, 3).fill(col);
  doc.font('Helvetica-Bold').fontSize(15).fillColor(WHITE)
    .text(v, MARGIN + 12, y + 24, { width: 96, align: 'center' });

  const rx = MARGIN + 124;
  doc.font('Helvetica-Bold').fontSize(11).fillColor(cleared ? GREEN : ORANGE)
    .text(status.replace(/_/g, ' '), rx, y + 16, { width: CONTENT_W - 140, lineBreak: false });
  ...
  doc.font('Helvetica').fontSize(8.5).fillColor(INK_SOFT)
    .text(_pdfSafe(bits.join('  |  ')) || '-', rx, y + 34, { width: CONTENT_W - 140, lineBreak: false });
...
const _DD_LAYER_PLAN = [
  ...
  ['compliance', 'Compliance and sanctions',
    ['country_risk', 'export_control', 'sanctions_regimes', 'licence_path']],
  ['network', 'Ownership and control network',
    ['controlled_by', 'ubo_chain', 'pep_connections', 'sanctions_network', 'cross_linked_entities']],
  ['verification', 'Verification',
    ['grounded_rate', 'unverified_claim_count', 'independent_corroboration_rate', 'conflicts']],
  ['digital', 'Digital footprint',
    ['press_coverage', 'procurement_history', 'people', 'exhibition_presence']],
...
export function ddReportSections(report = {}) {
  ...
  if (kf.length) {
    out.push({
      title: 'Key findings', status: '', error: '', facts: [],
      findings: [...kf].sort((a, b) => _sevRank(a.severity) - _sevRank(b.severity)),
    });
  }
  ...
  for (const [key, title, facts] of _DD_LAYER_PLAN) {
    ...
    const findings = Array.isArray(layer.findings) ? layer.findings.filter(Boolean) : [];
    const factPairs = facts
      .map((k) => [k, layer[k]])
      .filter(([, v]) => v != null && v !== '' && !(Array.isArray(v) && v.length === 0));
    if (!factPairs.length && !findings.length && !status) continue;   // nothing to show → omit
    out.push({
...
function addReportSection(doc, sec) {
  ...
  if (sec.status) {
    const col = sec.status === 'ok' ? GREEN : (sec.status === 'error' ? RED : ORANGE);
    doc.font('Helvetica-Bold').fontSize(8).fillColor(col)
      .text(sec.status.toUpperCase(), MARGIN, doc.y, { width: CONTENT_W });
    doc.moveDown(0.2);
    if (sec.error) {
  ...
  for (const [k, v] of sec.facts) {
    ...
    else if (typeof v === 'object') val = '(present)';
    else val = String(v);
    addKeyValue(doc, label, val);
  }
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (blocking.length) {
        ...
        for (const b of blocking) addBullet(doc, b);
      }
