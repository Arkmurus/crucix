...
function addCustomerWatermark(doc, metadata, signature) {
  ...
  const ident = email ? `Issued to ${email} | Report ${reportId}` : `Report ${reportId}`;

  for (let i = pages.start; i < pages.start + pages.count; i++) {
    doc.switchToPage(i);

    // Visible per-page identification line, just above the standard footer
    doc.fontSize(7).fillColor(GREY)
      .text(ident, 40, doc.page.height - 42, { width: doc.page.width - 80 });

    // Faint diagonal watermark — survives photocopying
    doc.save();
