...
export function ddReportSections(report = {}) {
  ...
  for (const [key, title, facts] of _DD_LAYER_PLAN) {
    ...
    if (key === 'compliance') {
      ...
      if (Array.isArray(screen.verified_sources)) {
        sanctionsSources = screen.verified_sources.filter(Boolean);
        sanctionsDate = screen.screened_at || report.generated_at || '';
      }
    }
    if (!factPairs.length && !findings.length && !status && !sanctionsSources.length) continue;
    out.push({
      title, status, error: meta.error ? String(meta.error) : '',
      facts: factPairs,
      findings: [...findings].sort((a, b) => _sevRank(a.severity) - _sevRank(b.severity)),
      sanctionsSources, sanctionsDate,
    });
  }
  return out;
}
