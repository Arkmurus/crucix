...
function _adverseMediaSummary(am, digital) {
  ...
  if (credible || findings.length) {
    const n = credible || findings.length;
...
function _adverseMediaLines(am) {
  ...
  const findings = Array.isArray(am.findings) ? am.findings : [];
  out.push({ text: `Subject-named items returned: ${findings.length}` });
  ...
  for (const f of findings.slice(0, 8)) {
    ...
    out.push({ text: title.slice(0, 100) || '(untitled item)', url: url || '', indent: true });
  }
  if (findings.length > 8) out.push({ text: `… and ${findings.length - 8} more`, indent: true });
...
export function ddReportSections(report = {}) {
  ...
  for (const [key, title, facts] of _DD_LAYER_PLAN) {
    ...
    if (key === 'digital') {
      ...
      const _amSum = _adverseMediaSummary(report.adverse_media, layer);
      adverseMedia = [
        { text: _amSum.headline, bold: true, warn: _amSum.severity !== 'info' },
        { text: 'Concern: ' + _amSum.concern },
        { text: 'Advice: ' + _amSum.advice },
        ..._adverseMediaLines(report.adverse_media),
      ];
    }
    if (!factPairs.length && !findings.length && !status && !sanctionsSources.length
        && !people.length && !evidence.length && !adverseMedia.length) continue;
    out.push({
