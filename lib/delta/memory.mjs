...
export class MemoryManager {
  ...
  // Async Redis restore — call once on startup after constructor
  async initFromRedis() {
    if (existsSync(this.hotPath)) return; // file present, nothing to restore
    const remote = await redisGet('crucix:memory_hot');
    if (remote && Array.isArray(remote.runs) && typeof remote.alertedSignals === 'object') {
      this.hot = remote;
      this._saveHot(); // write to disk so _loadHot finds it next time
      console.log('[Persist] Restored hot memory from Redis');
    }
  }
  ...
  _saveHot() {
    ...
    try {
      ...
      // Async Redis backup — only alertedSignals matters for cross-restart continuity
      redisSet('crucix:memory_hot', { runs: [], alertedSignals: this.hot.alertedSignals }).catch(() => {});
    } catch (err) {
