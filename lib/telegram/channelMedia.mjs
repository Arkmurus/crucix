...
/**
 * R-F3609 — rasterise an SVG card and POST IT ONCE, caption included.
 *
 * This function was `uploadSvgAsPhoto`, and the name was a lie about its behaviour:
 * the Telegram Bot API has NO upload-without-send method. `sendPhoto` *publishes*.
 * Both call sites read the name, believed they were holding an unpublished file_id,
 * and sent it AGAIN to attach a caption — so every carded post published the image
 * TWICE:
 *
 *   1. `uploadSvgAsPhoto` -> sendPhoto (multipart)  -> card posted, full size, NO caption
 *   2. `sendPhoto(fileId, { caption })`             -> SAME card posted again, captioned
 *   3. `_channelSend(text)`                         -> the actual post
 *
 * Worse, step 2 re-sent `data.result.photo[0].file_id` — Telegram returns PhotoSize
 * ascending, so `[0]` is the SMALLEST thumbnail. The duplicate was also the degraded
 * one, and it is the one carrying the caption.
 *
 * Live-confirmed on the public channel without sending anything: the R-F2717 outcome
 * ledger's recorded text message_id advanced by exactly 3 per Grade-A delivery
 * (10017 -> 10020 -> ... -> 10038, eight consecutive) and by 1 per Grade-B delivery,
 * which is text-only. 2 photos + 1 text.
 *
 * Neither the name nor the tests could catch it: `test/channel-card-evidence-rf2903`
 * exercises this function in ISOLATION and asserts the PNG bytes, and the publish-path
 * tests count only `/sendMessage`, never `/sendPhoto`. Both stayed green throughout.
 *
 * So the fix is the function itself, not the two callers: one send, caption carried in
 * the same multipart request, honestly named. The old name is DELETED rather than
 * aliased — a comment cannot stop the next caller re-adding a second send, absence of
 * the symbol can (the R-F2936 precedent).
 *
 * @param {object} bot — Telegram bot config { botToken, chatId|channelId }.
 * @param {string} svgContent — SVG markup.
 * @param {object} [opts]
 * @param {string} [opts.caption] — Caption, sent WITH the photo (Telegram caps at 1024).
 * @param {string} [opts.parseMode='Markdown'] — Caption parse mode.
 * @param {string} [opts.filename='card.png'] — Upload filename.
 * @returns {Promise<{ok:boolean,messageId?:number,fileId?:string,error?:string}>}
 */
export async function sendSvgCard(bot, svgContent, opts = {}) {
  if (!bot?.botToken) return { ok: false, error: 'No bot token' };
  const { caption, parseMode = 'Markdown', filename = 'card.png' } = opts;
  ...
  try {
    ...
    // The caption rides along with the image — this is what removes the second send.
    if (caption) {
      parts.push({ name: 'caption', value: String(caption).substring(0, 1024) });
      if (parseMode) parts.push({ name: 'parse_mode', value: String(parseMode) });
    }
    const body = _buildMultipartBody(boundary, parts);
    ...
    return {
      ...
      // Largest rendition, not `[0]` — PhotoSize is ascending. Returned for reuse
      // only; nothing needs to re-send it, and re-sending it is the bug above.
 ...
 *
 * R-F3609 — NO PRODUCTION CALLER as of this change. Both former callers used it to
 * re-send a card that `sendSvgCard` (then `uploadSvgAsPhoto`) had ALREADY published;
 * that second send was the double-post defect. Kept because it is correctly named,
 * does exactly what it says, and is the right primitive for re-sending an
 * already-hosted file_id or a remote URL — but it is currently exercised only by
 * tests. Do not reach for it to caption a card: pass `caption` to `sendSvgCard`,
 * which sends once.
 *
...
function _buildMultipartBody(boundary, parts) {
  // R-F2903 — build a BUFFER, not a string.
  //
  // This previously did `part.data.toString('utf-8')` and joined everything into a
  // string. That is lossless for text and DESTRUCTIVE for binary: every byte that is
  // not valid UTF-8 becomes U+FFFD, so a PNG arrives at Telegram corrupted and comes
  // back as 400 IMAGE_PROCESS_FAILED. It went unnoticed because the only binary this
  // module ever sent was an SVG, which is text.
  //
  // Verified live 2026-07-23: after rasterising to PNG the upload still failed with
  // IMAGE_PROCESS_FAILED — the rasteriser was fine, the encoder was eating the image.
  const chunks = [];
  for (const part of parts) {
    ...
    if (part.data != null) {
      ...
      chunks.push(Buffer.from(String(part.value ?? ''), 'utf-8'));
    }
