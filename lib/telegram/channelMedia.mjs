...
export function generateInfographicCard(data) {
  ...
  const titleLines = _svgLines(title, 48, 96, titleSize + 8, title.length > 76 ? 38 : 32);
  const subtitleLines = shortSubtitle ? _svgLines(shortSubtitle, 48, 240, 31, 78) : '';
  const impact = bullets[0] || shortSubtitle || 'New intelligence item requires review.';
  const nextCheck = bullets[1] || bullets[2] || 'Screen the named entity, address, directors and source trail before acting.';
  ...
  <g filter="url(#shadow)">
    <rect x="42" y="34" width="${Math.min(360, _textWidth(colours.label, 14) + 58)}" height="38" rx="19" fill="#ffffff" opacity="0.08"/>
    <rect x="42" y="34" width="${Math.min(360, _textWidth(colours.label, 14) + 58)}" height="38" rx="19" fill="${colours.accent}" opacity="0.14"/>
    <text x="66" y="58" font-family="system-ui, Segoe UI, sans-serif" font-size="14" font-weight="800" letter-spacing="0" fill="${colours.accent}">${_escapeXml(colours.label)}</text>
  </g>
  ...
  <g>
    <rect x="720" y="92" width="420" height="150" rx="8" fill="#ffffff" opacity="0.08"/>
    ...
    <text x="748" y="164" font-family="system-ui, Segoe UI, sans-serif" font-size="20" font-weight="600" letter-spacing="0" fill="#f3f4f6">${_svgLines(impact, 748, 164, 27, 45)}</text>

    <rect x="720" y="270" width="420" height="150" rx="8" fill="#ffffff" opacity="0.07"/>
    <text x="748" y="306" font-family="system-ui, Segoe UI, sans-serif" font-size="13" font-weight="850" letter-spacing="0" fill="${colours.accent}">NEXT CHECK</text>
    <text x="748" y="342" font-family="system-ui, Segoe UI, sans-serif" font-size="20" font-weight="600" letter-spacing="0" fill="#f3f4f6">${_svgLines(nextCheck, 748, 342, 27, 45)}</text>
  </g>
...
function _svgLines(text, x, y, lineHeight, maxCharsPerLine) {
