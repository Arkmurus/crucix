...
function _displayUrl(url) {
  try {
    const u = new URL(url);
    const path = u.pathname.length > 34 ? `${u.pathname.slice(0, 33)}…` : u.pathname;
...
export function generateInfographicCard(data) {
  ...
  const titleLines = _svgLines(title, 48, 96, titleSize + 8, title.length > 76 ? 38 : 32, 4);
  ...
  const subtitleLines = (shortSubtitle && shortSubtitle !== impact)
    ? _svgLines(shortSubtitle, 48, 240, 31, 78)
    : '';
  ...
  <g filter="url(#shadow)">
    ...
    <text x="66" y="58" font-family="system-ui, Segoe UI, sans-serif" font-size="14" font-weight="800" letter-spacing="0" fill="${colours.accent}">${_escapeXml(badge)}</text>`;
    })()}
  </g>
  ${target ? `<text x="48" y="342" font-family="system-ui, Segoe UI, sans-serif" font-size="13" font-weight="700" letter-spacing="0" fill="#9ca3af">TARGET</text>
  <text x="48" y="374" font-family="system-ui, Segoe UI, sans-serif" font-size="19" font-weight="700" letter-spacing="0" fill="#f3f4f6">${_escapeXml(target.slice(0, 46))}</text>` : ''}

  <text x="48" y="112" font-family="system-ui, Segoe UI, sans-serif" font-size="${titleSize}" font-weight="850" letter-spacing="0" fill="#ffffff">${titleLines}</text>
  ${subtitleLines ? `<text x="48" y="240" font-family="system-ui, Segoe UI, sans-serif" font-size="23" font-weight="500" letter-spacing="0" fill="#d1d5db">${subtitleLines}</text>` : ''}
  ...
  <g>
    ...
    <text x="${mx + 18}" y="374" font-family="system-ui, Segoe UI, sans-serif" font-size="29" font-weight="850" letter-spacing="0" fill="#ffffff">${_escapeXml(m.value)}</text>
    <text x="${mx + 18}" y="400" font-family="system-ui, Segoe UI, sans-serif" font-size="12" font-weight="700" letter-spacing="0" fill="#9ca3af">${_escapeXml(m.label)}</text>`;
    }).join('')}
  </g>

  <g>
    <rect x="720" y="92" width="420" height="180" rx="8" fill="#ffffff" opacity="0.08"/>
    <text x="748" y="128" font-family="system-ui, Segoe UI, sans-serif" font-size="13" font-weight="850" letter-spacing="0" fill="${colours.accent}">WHY IT MATTERS</text>
    <text x="748" y="164" font-family="system-ui, Segoe UI, sans-serif" font-size="20" font-weight="600" letter-spacing="0" fill="#f3f4f6">${_svgLines(impact, 748, 164, 27, 45, 4)}</text>

    <rect x="720" y="292" width="420" height="140" rx="8" fill="#ffffff" opacity="0.07"/>
    <text x="748" y="328" font-family="system-ui, Segoe UI, sans-serif" font-size="13" font-weight="850" letter-spacing="0" fill="${colours.accent}">RECOMMENDED ACTION</text>
    <text x="748" y="364" font-family="system-ui, Segoe UI, sans-serif" font-size="20" font-weight="600" letter-spacing="0" fill="#f3f4f6">${_svgLines(nextCheck, 748, 364, 27, 45)}</text>
  </g>

  ${!target ? `<g>
    ${bullets.slice(0, 3).map((b, i) => `
    <circle cx="61" cy="${466 + (i * 31)}" r="4" fill="${colours.accent}"/>
    <text x="78" y="${472 + (i * 31)}" font-family="system-ui, Segoe UI, sans-serif" font-size="17" font-weight="600" letter-spacing="0" fill="#d1d5db">${_escapeXml(String(b).replace(/\s+/g, ' ').slice(0, 86))}</text>`).join('')}
  </g>` : ''}
  ...
  <g>
    ...
    <text x="744" y="${h - 150}" font-family="system-ui, Segoe UI, sans-serif" font-size="12" font-weight="850" letter-spacing="0" fill="${colours.accent}">EVIDENCE · PRIMARY SOURCE</text>
    <text x="744" y="${h - 126}" font-family="system-ui, Segoe UI, sans-serif" font-size="16" font-weight="700" letter-spacing="0" fill="#f3f4f6">${_escapeXml(evidenceHost)}</text>
    ${detectedShort ? `<text x="744" y="${h - 104}" font-family="system-ui, Segoe UI, sans-serif" font-size="13" font-weight="600" letter-spacing="0" fill="#9ca3af">Detected ${_escapeXml(detectedShort)}${corroboration ? ` · ${_escapeXml(corroboration)}` : ''}</text>` : ''}
  </g>` : ''}

  <line x1="48" y1="${h - 76}" x2="${w - 48}" y2="${h - 76}" stroke="#ffffff" stroke-opacity="0.13"/>
  <text x="48" y="${h - 44}" font-family="system-ui, Segoe UI, sans-serif" font-size="13" font-weight="700" letter-spacing="0" fill="#9ca3af">${_escapeXml(source)}</text>
  <text x="${w - 48}" y="${h - 44}" font-family="system-ui, Segoe UI, sans-serif" font-size="13" font-weight="850" letter-spacing="0" fill="#f3f4f6" text-anchor="end">ARIA Intelligence · imaria.io</text>
</svg>`;
}
