...
/** Max posts per day to avoid flooding subscribers. R-F2306 — operator directive:
 *  ONE relevant post/day, keep the noise down. Env-tunable. */
const MAX_DAILY_POSTS = Number(process.env.CHANNEL_MAX_DAILY_POSTS) || 1;
 ...
 *   - Relevance to ARIA audience (procurement, sanctions, Africa/LATAM intel)
...
export function scoreSignal(signal, opts = {}) {
  ...
  // R-F2299 — noMark: score WITHOUT consuming the signal's dedup slot. Used by
  // isBreakingSignal to classify a signal before publishBreakingSignals actually
  // posts it — otherwise classification would mark it "already posted" and the
  // routine curated pass would silently skip everything.
  const noMark = opts.noMark === true;
  ...
  // ── Urgency boost ──────────────────────────────────────────────────────────
  // R-F2315 — include 'flash': isBreakingSignal treats flash as breaking, but this
  // boost omitted it, so a flash signal could score < MIN_QUALITY_SCORE and be
  // dropped by publishSignal — silently losing a genuine breaking alert.
  if (signal.severity === 'critical' || signal.severity === 'high' || signal.severity === 'flash') {
    score += 0.2;
  ...
  const relevanceKeywords = [
    'sanction', 'procurement', 'tender', 'defence', 'defense', 'military',
    // Africa
    'africa', 'angola', 'mozambique', 'nigeria', 'kenya', 'ethiopia', 'ghana',
    'brazil', 'portugal', 'lusophone', 'cplp', 'sahel', 'drc',
    // R-F2299 — broaden BEYOND Africa (operator directive: global audience) so
    // non-African defence/procurement signals score fairly and reach the channel.
    'europe', 'nato', 'poland', 'ukraine', 'germany', 'france', 'balkans',
    'gulf', 'gcc', 'mena', 'saudi', 'uae', 'qatar', 'egypt', 'israel', 'turkey',
    'india', 'indonesia', 'philippines', 'vietnam', 'korea', 'japan', 'pakistan',
    'latam', 'mexico', 'colombia',
    'oil', 'gas', 'mining', 'critical mineral', 'rare earth',
    'export control', 'trade restriction', 'tariff', 'embargo', 'wassenaar', 'itar', 'ear',
...
// R-F2299 — breaking threshold. A signal AT/ABOVE this score is genuinely
// breaking (FLASH/critical) and bypasses the routine posting-cadence cap so it
// reaches the channel in real time (the growth driver — screenshot-worthy,
// forward-to-a-colleague). Env-tunable.
export const BREAKING_SCORE = Number(process.env.CHANNEL_BREAKING_SCORE) || 0.85;

/**
 * R-F2299 — classify a signal as breaking WITHOUT consuming its dedup slot
 * (noMark), so the caller can post it via publishBreakingSignals afterwards.
 * @param {object} signal
 * @returns {boolean}
 */
export function isBreakingSignal(signal) {
  if (!signal || typeof signal !== 'object') return false;
  const r = scoreSignal(signal, { noMark: true });
  // Never re-classify an already-posted signal as breaking (avoids repeat posts).
  if (r.reason === 'already posted (dedup)') return false;
  // The strategy's "FLASH/critical" alerts are breaking by definition; anything
  // else must earn it on score (>= BREAKING_SCORE).
  const sev = String(signal.severity || '').toLowerCase();
  return sev === 'critical' || sev === 'flash' || r.score >= BREAKING_SCORE;
}
...
export function canPostNow() {
  ...
  // R-F2306 — read the cap at CALL time so it's env-tunable without a restart.
  const maxDaily = Number(process.env.CHANNEL_MAX_DAILY_POSTS) || MAX_DAILY_POSTS;
  if (_dailyPostCount >= maxDaily) {
...
export async function publishSignal(signal, bot, opts = {}) {
  ...
  try {
    ...
    //
    // R-F3609 — the card is now BUILT here and SENT once in step 4. It used to be
    // "uploaded" here (which actually published it, bare and uncaptioned) and then
    // sent a second time with the post as caption — two copies of the same card on
    // the channel per publish, the second at thumbnail resolution.
    let cardSvg = null;
    if (generateImage) {
      try {
        const { generateInfographicCard } = await import('./channelMedia.mjs');
    ...
    // 4. Send the post (as a captioned card if one was built, otherwise text)
    let sendResult;
    if (cardSvg) {
      const { sendSvgCard } = await import('./channelMedia.mjs');
      sendResult = await sendSvgCard(bot, cardSvg, { caption: post, filename: `intel_${Date.now()}.png` });
      // Rasterisation is best-effort; fall through to text so the post is never lost.
      if (!sendResult.ok) {
        console.warn('[ChannelPublisher] card send failed, posting text-only:', sendResult.error);
        cardSvg = null;
      }
    }
    if (!cardSvg) {
