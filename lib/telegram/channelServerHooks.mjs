...
// R-F2319 — normalize the channel chat_id. A Telegram channel/supergroup id must
// carry the -100 prefix; if only a bare 10+ digit TELEGRAM_CHAT_ID is set, every
// channel send silently fails ("chat not found"). Restore the prefix defensively.
function _channelChatId(bot) {
  const raw = String(bot?.channelId || bot?.chatId || '');
  return /^\d{10,}$/.test(raw) ? `-100${raw}` : raw;
}
...
export function selectTelegramGoldenIntel(feed, opts = {}) {
  ...
  const candidates = (Array.isArray(feed.signals) ? feed.signals : []).filter(s => {
    ...
    return quality.startsWith('decision-grade')
      ...
      && !wasRecentlyPosted(dedupKey({ title: s.id || s.decision_summary || s.title, text: _signalEvidenceUrl(s) }));
  });
  ...
  try {
    ...
    if (golden) {
      ...
      if (res.ok) {
        ...
        recordPosted([dedupKey({ title: golden.id || golden.decision_summary || golden.title, text: _signalEvidenceUrl(golden) })]);
    ...
    signals = signals.filter(s => !wasRecentlyPosted(dedupKey(s)));
    ...
    if (res.ok) {
      ...
      recordPosted(signals.map(dedupKey));   // remember what we posted (cross-day)
    }
