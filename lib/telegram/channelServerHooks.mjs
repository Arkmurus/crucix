...
export function selectTelegramGoldenIntel(feed, opts = {}) {
  ...
  const blockingStale = staleReasons.filter(r => r !== 'source_failure_degraded');
...
export async function handleMorningSignalCron(currentData, bot, opts = {}) {
  ...
  try {
    ...
    if (!destination.ok) {
      console.error(`[ChannelCron] Golden Intel blocked — invalid public channel destination: ${destination.reason} type=${destination.type || 'unknown'}`);
      reportChannelOutcome('daily_golden_intel', 'failed', `${destination.reason}:${destination.type || destination.status || 'unknown'}`);
    ...
    const goldenFeed = await fetchGoldenIntelSignals({ limit: 60 });
