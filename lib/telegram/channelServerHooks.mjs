...
export async function handleMorningSignalCron(currentData, bot) {
  ...
  try {
    ...
    // R-F2310 — draw from the REAL currentData fields. The prior code read
    // currentData.sanctions / .opportunities[].title / .summary, none of which
    // exist (audit R-F2309): the sweep writes sanctions to currentData.opensanctions
    // .recent, tenders to currentData.bdIntelligence.tenders, and opportunities carry
    // no title/summary. buildDailySignals maps + projects them correctly.
    const signals = buildDailySignals(currentData);
