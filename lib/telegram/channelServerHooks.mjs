...
import { curateSignals, formatChannelPost, formatDailyBrief, canPostNow, recordPost, getSchedulerState, publishSignal } from './channelPublisher.mjs';
...
export async function runChannelSweep(currentData, bot) {
  ...
  if (!canPost) return { posted: 0, errors: 0 };
...
export async function handleMorningSignalCron(currentData, bot) {
  ...
  try {
    ...
    if (signals.length === 0) {
      signals.push({ title: 'Market Monitor', text: 'No significant signals overnight. Standard monitoring cadence active.' });
    }
