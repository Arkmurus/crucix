...
export function parseReply(text) {
  ...
  // 5. Whole-word country match (e.g. "screen angola" → Angola). R-F2315 — was
  // startsWith/endsWith, which matched short keys inside unrelated words
  // ("ukulele" → 'uk' → UK brief). Match only when the country key appears as a
  // whole word, so genuine chatter never triggers a brief.
  const _words = new Set(lower.split(/[^a-z]+/).filter(Boolean));
  const countryMatch = Object.keys(COUNTRY_KEYWORDS).find(k => _words.has(k));
  if (countryMatch) {
...
export async function handleScreen(entityName) {
  ...
  // R-F2302 — REAL fuzzy sanctions screen (never-false-clean). /sanctions/fuzzy
  // returns { screened, blocking_matches, matches, match_count, variants_tried,
  // error }. formatScreenResult below refuses to report "clear" unless the screen
  // genuinely ran (data.screened === true && !data.error).
  try {
    const res = await fetch(`${ARIA_SERVICE_URL}/api/aria/sanctions/fuzzy`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(ARIA_API_TOKEN ? { 'Authorization': `Bearer ${ARIA_API_TOKEN}` } : {}),
      },
    ...
    console.warn('[ReplyKeyword] screen HTTP', res.status);
  } catch (err) {
    console.warn('[ReplyKeyword] ARIA API screen failed:', err.message);
  }

  // NEVER-FALSE-CLEAN fallback: if the screen could not run, say so — do NOT
  // imply a clearance.
  return {
    text: `🔍 *Sanctions Screen — ${_escapeMarkdown(entityName)}*\n━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`
      + `⚠️ The screen could not be completed right now (source temporarily unavailable).\n\n`
      + `This is *NOT* a clearance — please reply \`SCREEN ${_escapeMarkdown(entityName)}\` again in a moment.\n\n`
      + `━━━━━━━━━━━━━━━━━━━━━━━━━━\n🤖 *ARIA Intelligence*`,
  };
}
...
export function handlePro() {
  // R-F2317 — real conversion path. Prefer a Stripe Payment Link (frictionless,
  // no login) if configured; else send users to imaria.io where the live checkout
  // (lib/billing) takes payment. No more "DM a human" dead-end.
  const upgradeUrl = process.env.STRIPE_PAYMENT_LINK_PROINTEL || 'https://imaria.io/signup.html';
  return {
...
export function handleHelp() {
  return {
    text: `🤖 *ARIA Intelligence — Free Consultation*\n━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`
      + `I'm ARIA — an autonomous defence due-diligence & compliance agent. Consult me here, privately, free:\n\n`
      + `🔍 \`SCREEN [company or person]\`\n`
      + `    Live sanctions & watchlist check — OFAC · UK OFSI · EU · UN · OpenSanctions.\n`
      + `    _try_ \`SCREEN Rosoboronexport\`\n\n`
      + `🌍 \`[COUNTRY]\` — defence-market brief\n`
      + `    _e.g._ \`NIGERIA\` · \`POLAND\` · \`UAE\` · \`INDIA\` · \`MEXICO\`\n\n`
      + `📋 \`TENDER [sector]\` — procurement opportunities\n`
      + `🎯 \`DEMO\` — a live example\n`
      + `💼 \`PRO\` — full DD (ownership, adverse media, financials) in under 10 min\n\n`
      + `Every answer is drawn from live sources and shown honestly — a screen that can't be verified is *never* reported as "clear".\n\n`
      + `━━━━━━━━━━━━━━━━━━━━━━━━━━\n🤖 *ARIA Intelligence* · imaria.io`,
  };
}
...
function formatScreenResult(entityName, data) {
  const name = _escapeMarkdown(entityName);
  let msg = `🔍 *Sanctions & Watchlist Screen*\n_${name}_\n━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;

  // R-F2302 — NEVER-FALSE-CLEAN: only report a verdict when the screen actually
  // RAN. `screened` is fuzzy_screen's R-F1696 flag; `error` (e.g.
  // 'sanctions_source_unavailable') means the source failed. Absence of matches
  // from an UNPERFORMED screen must NEVER read as "clear".
  const screened = !!(data && data.screened === true && !data.error);
  if (!screened) {
    msg += `⚠️ *Screen could not be completed* — the sanctions source was unavailable this run.\n\n`
      + `This is *NOT* a clearance; absence of a result does not mean the entity is clear. `
      + `Please reply \`SCREEN ${name}\` again shortly.\n`
      + `\n━━━━━━━━━━━━━━━━━━━━━━━━━━\n🤖 *ARIA Intelligence* — Screen`;
    return msg;
  }

  const blocking = Array.isArray(data.blocking_matches) ? data.blocking_matches : [];
  const total = data.match_count ?? (Array.isArray(data.matches) ? data.matches.length : 0);
  const renderList = (arr) => arr.slice(0, 5).map((m) => {
    let line = `• *${_escapeMarkdown(m.name || m.entity || 'match')}*`;
    const list = m.list || m.source || m.dataset;
    if (list) line += ` — ${_escapeMarkdown(String(list))}`;
    if (typeof m.score === 'number') line += ` (${Math.round(m.score * 100)}%)`;
    return line;
  }).join('\n');

  if (blocking.length > 0) {
    msg += `🔴 *${blocking.length} BLOCKING match(es)* on an active sanctions/watchlist:\n\n`
      + renderList(blocking)
      + `\n\n*Do not proceed* without enhanced due diligence.\n`;
  } else if (total > 0) {
    msg += `🟠 *${total} possible match(es)* below the blocking threshold — likely name overlaps, `
      + `but *enhanced DD is required* before contracting:\n\n`
      + renderList(data.matches || [])
      + `\n`;
  } else {
    msg += `🟢 *No matches* across OFAC, UK OFSI, EU, UN & OpenSanctions lists`;
    if (Array.isArray(data.variants_tried) && data.variants_tried.length) {
      msg += ` (${data.variants_tried.length} name variants tested)`;
    }
    msg += `.\n\nInformational only — reflects the lists as of this run; re-screen before contracting.\n`;
  }

  msg += `\n💼 Full DD — ownership, adverse media, financials — in under 10 min. Reply \`PRO\`.`;
  msg += `\n━━━━━━━━━━━━━━━━━━━━━━━━━━\n🤖 *ARIA Intelligence* — Screen`;
  return msg;
}
