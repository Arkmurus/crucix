...
// R-F3656 — the brain calls in this module carried no Authorization header, so
// every Bearer-gated route answered 401 and the catch fell back to the local
// LLM. Verified live 2026-08-03: /api/aria/curiosity and /api/aria/identity
// both return 401 unauthenticated. The fallback made this look like "the brain
// had nothing to say" rather than "we were never allowed to ask" — so /identity
// and /curiosity have been silently degraded to local answers. Token chain
// mirrors lib/self/learning_store.mjs:420.
const BRAIN_TOKEN   = process.env.ARIA_API_TOKEN || process.env.ARIA_INTERNAL_TOKEN || '';

/** fetch() against the brain with auth attached — use for EVERY brain call. */
...
async function ariaThinkProxy(question, context = {}, fast = false) {
  if (BRAIN_URL) {
    try {
      const r = await brainFetch('/api/aria/think', {
...
async function ariaChatProxy(message, sessionId) {
  if (BRAIN_URL) {
    try {
      const r = await brainFetch('/api/aria/chat', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
...
export async function buildArkmursBrief(chatId) {
  ...
  try {
    const [briefResp, identityResp] = await Promise.all([
      // NOTE (R-F3656): /api/brain/brief 404s on the brain — probed live
      // 2026-08-03. It is an aria-web route, not an aria-intel one, so this
      // call has never returned data. Left pointing at BRAIN_URL rather than
      // silently re-hosted: switching it is a behaviour change that needs its
      // own verification. Recorded in docs/prospector_360_sweep_2026_08_03.md.
      brainFetch('/api/brain/brief', { signal: AbortSignal.timeout(15000) }),
      brainFetch('/api/aria/identity', { signal: AbortSignal.timeout(10000) }),
    ]);
...
export async function handleAriaCommand(chatId, userId, args) {
  ...
  if (argsLower === 'status') {
    try {
      const resp = await brainFetch('/api/aria/identity', { signal: AbortSignal.timeout(10000) });
  ...
  if (argsLower === 'reflect') {
    ...
    try {
      const resp = await brainFetch('/api/aria/reflect', {
