  ...
  if (BRAIN_URL) {
    try {
      const r = await fetch(`${BRAIN_URL}/api/aria/think`, {
        ...
        body: JSON.stringify({ question, context, fast }), timeout: 90000,
      });
  ...
  if (BRAIN_URL) {
    try {
      const r = await fetch(`${BRAIN_URL}/api/aria/chat`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message, session_id: sessionId }), timeout: 30000,
      });
      if (r.ok) return await r.json();
    } catch {}
  }
...
export async function buildArkmursBrief(chatId) {
  ...
  try {
    const [briefResp, identityResp] = await Promise.all([
      fetch(`${BRAIN_URL}/api/brain/brief`, { timeout: 15000 }),
      fetch(`${BRAIN_URL}/api/aria/identity`, { timeout: 10000 }),
    ]);
...
export async function handleAriaCommand(chatId, userId, args) {
  ...
  if (argsLower === 'status') {
    try {
      const resp = await fetch(`${BRAIN_URL}/api/aria/identity`, { timeout: 10000 });
  ...
  if (argsLower === 'curiosity') {
    try {
      const resp = await fetch(`${BRAIN_URL}/api/aria/curiosity`, { timeout: 10000 });
      if (!resp.ok) throw new Error('Brain offline');
